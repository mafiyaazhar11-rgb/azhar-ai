...
module.exports = function (app, pool, requireAuth, requireRole) {
  ...
  // "Combined" means: multiple orders (e.g. AED 500 + 200 + 300) going to the SAME
  // location, on the SAME day, in the SAME temperature category (Ambient or Frozen) ride
  // on the same truck stop — so their VALUES ARE SUMMED before checking the AED 1,000
  // threshold. A drop is judged as a whole, not order-by-order. Two orders to the same
  // address on the same day but different temperature (one Frozen, one Ambient) need two
  // different trucks, so they are NOT combined — that's two separate drops. ──
  function buildCombinedDrops(rows) {
    var groups = {};
    rows.forEach(function (r) {
      if (isExcludedRow(r)) return;
      var temp = tempBucketOf(r.temperature, r.truck_type);
      var loc = r.location_id || r.customer || r.order_code;
      var key = r.date_key + '::' + loc + '::' + temp;
      if (!groups[key]) groups[key] = { date: r.date_key, location: loc, temp: temp, org: r.org || '(Unknown)', value: 0, orders: [] };
  ...
  // temperature (Ambient/Frozen — the dimension "combining" actually runs on), plus one
  // TOTAL (All BU) row. Classification happens at the COMBINED DROP level (see above):
  // a drop with 3 orders worth 500+200+300 = AED 1,000 counts as one ≥1k drop with 3
  // orders in it — the orders are not each judged against the threshold individually. ──
  function buildValueTier(rows) {
    var drops = buildCombinedDrops(rows);
    var byOrgTemp = {};
    var total = { orders_hi: 0, orders_lo: 0, value_hi: 0, value_lo: 0, drops_hi: 0, drops_lo: 0 };
    drops.forEach(function (g) {
      var key = g.org + '||' + g.temp;
      if (!byOrgTemp[key]) byOrgTemp[key] = { org: g.org, type: g.temp, orders_hi: 0, orders_lo: 0, value_hi: 0, value_lo: 0, drops_hi: 0, drops_lo: 0 };
      var b = byOrgTemp[key];
      var isHigh = g.value >= LOW_VALUE_THRESHOLD;
      ...
      else { b.orders_lo += g.orders.length; b.value_lo += g.value; b.drops_lo++; total.orders_lo += g.orders.length; total.value_lo += g.value; total.drops_lo++; }
    });
    var out = Object.keys(byOrgTemp).sort().map(function (k) {
      var b = byOrgTemp[k];
      return { org: b.org, type: b.type, orders_ge1k: b.orders_hi, orders_lt1k: b.orders_lo, drops_ge1k: b.drops_hi, drops_lt1k: b.drops_lo, value_ge1k: Math.round(b.value_hi), value_lt1k: Math.round(b.value_lo) };
    });
    return {
      rows: out,
      combined: { org: 'TOTAL (All BU)', type: 'All Types', orders_ge1k: total.orders_hi, orders_lt1k: total.orders_lo, drops_ge1k: total.drops_hi, drops_lt1k: total.drops_lo, value_ge1k: Math.round(total.value_hi), value_lt1k: Math.round(total.value_lo) }
    };
  }
  ...
  function buildRedelivery(rows, historyRows, dateFrom, dateTo) {
    ...
    return { daily: dailyRows, by_bu: buRows, by_attempt: byAttempt, total_orders_redelivered: totalOrders.size, total_estimated_cost: Math.round(totalCost), note: 'Cost is billed PER DROP, not per order — 10 re-delivered orders to the same stop on the same day is one drop charge, not ten. Rate is an ESTIMATE (temperature-based Multi-route rate) — no per-order truck-type was on file for every row.' };
  }

  // ── 5) Full drops — day-wise and BU-wise. A "drop" is a COMBINED DROP (see above):
  // one truck stop = one (date, location, temperature) combo, regardless of how many
  // orders shared that stop. ──
  ...
  async function buildSummary(q) {
    ...
    return { period: period, orgs: orgs, value_tier: valueTier, low_value: lowValue, low_value_by_customer: lowValueByCustomer, redelivery: redelivery, drops: drops, rejection_redelivery: rejectionRedelivery };
  }
