...
module.exports = function (app, pool, requireAuth, requireRole) {
  ...
  function buildCombinedDrops(rows) {
    ...
    rows.forEach(function (r) {
      ...
      if (!groups[key]) groups[key] = { date: r.date_key, location: loc, org: r.org || '(Unknown)', value: 0, orders: [], missing_value_count: 0 };
      // Flag a truly missing/blank value BEFORE the || 0 fallback swallows it — a
      // genuinely priced-at-zero order (e.g. a sample/free item) and an order with no
      // value on file at all are different things, and only the second is a data-quality
      // risk worth surfacing (it could be silently misclassifying a real high-value order
      // as "low value" purely because the price never made it into this row).
      var rawVal = r.value;
      var isMissing = rawVal === null || rawVal === undefined || rawVal === '' || isNaN(+rawVal);
      if (isMissing) groups[key].missing_value_count++;
  ...
  function buildValueTier(rows, threshold) {
    ...
    var missingValueLowDrops = 0, missingValueLowOrders = 0;
    drops.forEach(function (g) {
      ...
      if (isHigh) { b.orders_hi += g.orders.length; b.value_hi += g.value; b.drops_hi++; total.orders_hi += g.orders.length; total.value_hi += g.value; total.drops_hi++; }
        ...
        // Cross-check: a drop landed in the "low value" bucket here because its
        // combined value came out under the threshold — but if that value was built
        // partly or entirely from a missing price (defaulted to AED 0), the "low
        // value" classification for this drop cannot be verified as real. Counted
        // separately so it's visible rather than silently trusted.
        if (g.missing_value_count > 0) { missingValueLowDrops++; missingValueLowOrders += g.missing_value_count; }
    ...
    return {
      ...
      // Data-quality cross-check: how many of the "low value" drops/orders above had a
      // missing (not just zero) price on file. If this is 0, every low-value
      // classification is verified against a real number, not a data gap.
      data_quality: { low_value_drops_with_missing_price: missingValueLowDrops, low_value_orders_with_missing_price: missingValueLowOrders }
    };
  }
  ...
  // ── 3a-ii) Month-wise trend for consolidation savings vs standalone cost — same
  // numbers as buildLowValueConsolidation's combined total, split by calendar month
  // instead of by BU. Used to draw a visual trend in Excel (data bars) since ExcelJS
  // cannot write a real embedded chart object into an .xlsx file — this is the closest
  // real equivalent the library actually supports. ──
  function buildLowValueMonthlyTrend(rows, threshold) {
    threshold = threshold || LOW_VALUE_THRESHOLD;
    var byMonth = {};
    rows.forEach(function (r) {
      var m = String(r.date_key).slice(0, 7);
      if (!byMonth[m]) byMonth[m] = [];
      byMonth[m].push(r);
    });
    var months = Object.keys(byMonth).sort();
    return months.map(function (m) {
      var vt = buildValueTier(byMonth[m], threshold);
      var lvc = buildLowValueConsolidation(vt);
      return {
        month: m,
        low_value_orders: lvc.combined.low_value_orders,
        orders_saved: lvc.combined.orders_saved_by_consolidation,
        cost_saved_aed: lvc.combined.estimated_cost_saved_aed,
        pure_orders: lvc.combined.pure_low_value_orders,
        pure_cost_aed: lvc.combined.pure_drop_current_cost_aed
      };
    });
  }
  ...
  async function buildRejectionRedelivery(dateFrom, dateTo) {
    var rejR = await pool.query(
      ...
      [dateFrom, dateTo]);
    // Same other-division exclusion as everywhere else on this page.
    rejR.rows = rejR.rows.filter(function (r) { return VALID_ORGS.indexOf(String(r.org || '').toUpperCase().trim()) !== -1; });
  ...
  async function buildSummary(q) {
    ...
    var lowValueMonthlyTrend = buildLowValueMonthlyTrend(rows, customThreshold);
    ...
    return {
      period: period, orgs: orgs, value_tier: valueTier, low_value: lowValue, low_value_consolidation: lowValueConsolidation, low_value_monthly_trend: lowValueMonthlyTrend, low_value_by_customer: lowValueByCustomer,
  ...
  app.get('/api/transport-cost/export', requireAuth, async function (req, res) {
    try {
      ...
      es.addRow(['Orders Above 1,000 AED (All BU)', data.value_tier.combined.orders_ge1k]);
      es.addRow(['Orders Below 1,000 AED (All BU)', data.value_tier.combined.orders_lt1k]);
      es.addRow(['Drops Above 1,000 AED (All BU)', data.value_tier.combined.drops_ge1k]);
      es.addRow(['Drops Below 1,000 AED (All BU)', data.value_tier.combined.drops_lt1k]);
      ...
      savedCostRow.getCell(2).font = { bold: true, size: 18, color: { argb: 'FFCC0000' } };
      savedCostRow.getCell(1).font = { bold: true, size: 14 };
      savedCostRow.height = 26;
      ...
      var dq = data.value_tier.data_quality;
      var dqRow = es.addRow(['Cross-Check: Low-Value Orders With a Missing Price on File', dq.low_value_orders_with_missing_price]);
      if (dq.low_value_orders_with_missing_price > 0) {
        dqRow.getCell(2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: RED } };
        dqRow.getCell(2).font = { bold: true };
      } else {
        dqRow.getCell(2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GREEN } };
      }
      ...
      vt.addRow([]);
      headerRow(vt, ['BU', 'Type', 'Orders Above 1000', 'Orders Below 1000', 'Drops Above 1000', 'Drops Below 1000', 'Value Above 1000 (AED)', 'Value Below 1000 (AED)']);
      ...
      titleRow(lv, 'LOW-VALUE ORDERS (BELOW 1,000 AED) \u2014 ' + data.period.label, 'H');
      lv.addRow([]);
      ...
      var dqNote = lvs.addRow([data.value_tier.data_quality.low_value_orders_with_missing_price > 0
        ? '\u26a0 CROSS-CHECK: ' + data.value_tier.data_quality.low_value_orders_with_missing_price + ' order(s) counted as "low value" above had NO price on file at all (not genuinely priced low \u2014 the price field was blank/missing, which defaults to AED 0). These are NOT verified as truly under the threshold and should be checked against the source file before relying on the totals above.'
        : '\u2713 CROSS-CHECK PASSED: every order counted as "low value" above had a real price on file under the threshold \u2014 none of this total comes from a missing/blank price defaulting to AED 0.']);
      lvs.mergeCells('A' + dqNote.number + ':H' + dqNote.number);
      dqNote.font = { bold: true, size: 12, color: { argb: data.value_tier.data_quality.low_value_orders_with_missing_price > 0 ? 'FFCC0000' : 'FF1A7A4C' } };
      lvs.addRow([]);
      ...
      lvs.addRow([]);
      if (data.low_value_monthly_trend && data.low_value_monthly_trend.length > 1) {
        var trendHdr = lvs.addRow(['MONTH-BY-MONTH TREND (visual bar = relative size within its own column)']);
        lvs.mergeCells('A' + trendHdr.number + ':H' + trendHdr.number); trendHdr.font = { bold: true, size: 12 };
        var trendColHdr = lvs.addRow(['Month', 'Low-Value Orders', '', 'Cost Saved (AED)', '', 'Pure Orders', '', 'Standalone Cost (AED)']);
        trendColHdr.font = { bold: true };
        var trendFirstRow = lvs.rowCount + 1;
        data.low_value_monthly_trend.forEach(function (r) {
          var row = lvs.addRow([r.month, r.low_value_orders, '', r.cost_saved_aed, '', r.pure_orders, '', r.pure_cost_aed]);
          row.getCell(4).numFmt = '#,##0'; row.getCell(8).numFmt = '#,##0';
        });
        var trendLastRow = lvs.rowCount;
        // Data bars — the real visual "trend line" equivalent: ExcelJS cannot write a
        // native embedded chart object into .xlsx, but a data bar on each numeric
        // column gives the same at-a-glance shape across months.
        lvs.addConditionalFormatting({ ref: 'B' + trendFirstRow + ':B' + trendLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FF5B8DEE' }, minLength: 0, maxLength: 100 }] });
        lvs.addConditionalFormatting({ ref: 'D' + trendFirstRow + ':D' + trendLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FF6FCF97' }, minLength: 0, maxLength: 100 }] });
        lvs.addConditionalFormatting({ ref: 'F' + trendFirstRow + ':F' + trendLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FFE8B84C' }, minLength: 0, maxLength: 100 }] });
        lvs.addConditionalFormatting({ ref: 'H' + trendFirstRow + ':H' + trendLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FFE05C5C' }, minLength: 0, maxLength: 100 }] });
      }
