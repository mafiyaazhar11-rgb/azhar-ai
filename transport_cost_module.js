...
module.exports = function (app, pool, requireAuth, requireRole) {
  ...
  function buildLowValueConsolidation(valueTier) {
    function shape(row) {
      ...
      return {
        ...
        // with anything else at that address. This is a genuine COUNT, but there is
        // deliberately NO cost figure attached to it: a standalone drop isn't
        // automatically an extra, avoidable truck trip — the truck is very likely
        // already running its normal multi-stop route regardless, so charging the
        // full per-drop rate card to every one of these would overstate what's
        // actually incremental spend. Only the consolidation SAVINGS above (orders
        // that genuinely got removed by merging into another drop) is a defensible
        // cost number.
  ...
  app.get('/api/transport-cost/export', requireAuth, async function (req, res) {
    try {
      ...
      lvs.columns = [{ width: 16 }, { width: 14 }, { width: 18 }, { width: 16 }, { width: 22 }, { width: 20 }, { width: 22 }];
      titleRow(lvs, 'LOW-VALUE ORDER CONSOLIDATION SAVINGS \u2014 ' + data.period.label + ' (threshold: AED ' + data.value_tier.threshold.toLocaleString() + ')', 'G');
      lvs.addRow([]);
      var lvsNote = lvs.addRow(['A low-value order (below the threshold above) isn\'t automatically a separate truck stop \u2014 if two or more share the same date/route/location, they already ride on ONE drop together. "Saved by Consolidation" = orders that got folded into a shared drop \u2014 a genuine, defensible cost saving. "Pure" = orders that went out as their OWN dedicated drop, never combined at that address. This is shown as a COUNT ONLY, deliberately with no cost attached: a standalone drop is not automatically an extra/avoidable truck trip \u2014 the truck is very likely already running its normal multi-stop route regardless, so applying the per-drop rate card to every standalone order would overstate real incremental spend.']);
      lvs.mergeCells('A' + lvsNote.number + ':G' + lvsNote.number); lvsNote.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
      lvs.addRow([]);
      headerRow(lvs, ['BU', 'Type', 'Low-Value Orders', 'Low-Value Drops', 'Orders Saved by Consolidation', 'Estimated Cost Saved (AED)', 'Pure (Non-Combined) Orders']);
      data.low_value_consolidation.rows.concat([data.low_value_consolidation.combined]).forEach(function (r) {
        var isTotal = r.org.indexOf('TOTAL') !== -1;
        var row = lvs.addRow([r.org, r.type, r.low_value_orders, r.low_value_drops, r.orders_saved_by_consolidation, r.estimated_cost_saved_aed, r.pure_low_value_orders]);
        row.getCell(6).numFmt = '#,##0';
        if (r.orders_saved_by_consolidation > 0) row.getCell(6).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: GREEN } };
        if (r.pure_low_value_orders > 0) row.getCell(7).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: AMBER } };
        if (isTotal) row.eachCell(function (c) { c.font = { bold: true }; });
      });
      lvs.addRow([]);
