...
module.exports = function (app, pool, requireAuth, requireRole) {
  ...
  // These org/BU codes are OTHER divisions' transport data that ends up in the same
  // dispatch file but isn't ours to track cost against — excluded everywhere on this
  // page (value tier, low-value, re-delivery, drops, rejection re-delivery).
  var EXCLUDED_ORGS = ['APQ', 'DAD', 'DCH', 'DEV', 'DLL', 'DNN', 'DPB', 'DSD', 'HCP', 'TPW'];
  function isExcludedRow(row) {
    if (String(row.source_label || '').toUpperCase() === 'SALON') return true;
    if (EXCLUDED_ORGS.indexOf(String(row.org || '').toUpperCase()) !== -1) return true;
    return false;
  }
  ...
  // instead of chasing the number in the abstract. Ranked by low-value order COUNT first
  // (the thing driving avoidable drop cost), then by low-value AED as a tiebreak. Only
  // customers with at least one low-value order are included. ──
  function buildLowValueByCustomer(rows) {
    ...
    rows.forEach(function (r) {
      ...
      if (val < LOW_VALUE_THRESHOLD) { b.low++; b.lowValueSum += val; }
    });
  ...
  async function buildRejectionRedelivery(dateFrom, dateTo) {
    ...
    rejR.rows = rejR.rows.filter(function (r) { return EXCLUDED_ORGS.indexOf(String(r.org || '').toUpperCase()) === -1; });
