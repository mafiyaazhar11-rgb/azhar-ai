...
module.exports = function (app, pool, requireAuth, requireRole) {
  ...
  app.get('/api/transport-cost/export', requireAuth, async function (req, res) {
    try {
      ...
      lvs.columns = [{ width: 16 }, { width: 14 }, { width: 18 }, { width: 16 }, { width: 22 }, { width: 20 }];
      titleRow(lvs, 'LOW-VALUE ORDER CONSOLIDATION SAVINGS \u2014 ' + data.period.label, 'F');
      lvs.addRow([]);
      var lvsNote = lvs.addRow(['A low-value order (< AED 1,000) isn\'t automatically a separate truck stop \u2014 if two or more share the same date/route/location, they already ride on ONE drop together. This sheet shows how many low-value orders were folded into a shared drop instead of needing a drop of their own, and the estimated transport cost that saved (same per-drop rate card used for Re-attempt Cost \u2014 not a new made-up rate).']);
      lvs.mergeCells('A' + lvsNote.number + ':F' + lvsNote.number); lvsNote.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
      lvs.addRow([]);
      headerRow(lvs, ['BU', 'Type', 'Low-Value Orders', 'Low-Value Drops', 'Orders Saved by Consolidation', 'Estimated Cost Saved (AED)']);
      data.low_value_consolidation.rows.concat([data.low_value_consolidation.combined]).forEach(function (r) {
        var isTotal = r.org.indexOf('TOTAL') !== -1;
        var row = lvs.addRow([r.org, r.type, r.low_value_orders, r.low_value_drops, r.orders_saved_by_consolidation, r.estimated_cost_saved_aed]);
