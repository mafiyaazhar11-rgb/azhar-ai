...
module.exports = function (app, pool, requireAuth, requireRole, matchRateViaVehicleMaster) {
  ...
  // ── Real vehicle-type rate card (transport team's FY26 rates, same table used in
  // Truck Cost Reconciliation elsewhere in this app) — used for the Pure/standalone
  // drop cost below, instead of a flat estimate. Matches on distinctive keywords so
  // variations in how the truck-type column gets typed still resolve correctly. If a
  // drop's truck type is missing or doesn't match anything on the rate card, AED 130
  // (a rough average across the card) is used instead of leaving it uncosted. ──
  var TRUCK_RATE_CARD = [
  ...
  var MISSING_TRUCK_TYPE_FALLBACK_RATE = 130;
  function matchTruckRate(rawTruckType) {
    var u = String(rawTruckType || '').toUpperCase();
    if (!u) return null;
  ...
  // Returns { rate, is_fallback, source } — is_fallback tells the caller this drop's
  // cost is the AED 130 average (no real match at all), not a genuine rate-card or
  // Vehicle Master match, so it can be tracked separately. Tries three levels, in
  // order: (1) the drop's own truck_type text against the rate card, (2) if that's
  // missing/unrecognized, the drop's vehicle plate against the uploaded Vehicle
  // Master (same lookup already used in Truck Cost Reconciliation) — this needs how
  // many drops that same vehicle had that day, to pick Multi vs Bulk tier correctly,
  // (3) only if neither resolves anything, the AED 130 average.
  function dropCostFor(rawTruckType, vehicleNo, dropCountForVehicle) {
    var match = matchTruckRate(rawTruckType);
    if (match) return { rate: match.rate, is_fallback: false, source: 'truck_type' };
    if (vehicleNo && typeof matchRateViaVehicleMaster === 'function') {
      var viaVehicle = matchRateViaVehicleMaster(vehicleNo, dropCountForVehicle || 1);
      if (viaVehicle) return { rate: viaVehicle.rate, is_fallback: false, source: 'vehicle_master' };
    }
    return { rate: MISSING_TRUCK_TYPE_FALLBACK_RATE, is_fallback: true, source: 'fallback_average' };
  }
  ...
  async function fetchPeriodRows(dateFrom, dateTo) {
    var r = await pool.query(
      `SELECT order_code, date_key::text AS date_key, customer, value, route, org, drop_type,
         temperature, source_label, city, location_id, truck_type, vehicle_no
  ...
  function buildValueTier(rows, threshold) {
    ...
    // How many drops each vehicle handled on each date — needed by the Vehicle
    // Master fallback to pick the right rate tier (2+ drops that day = Multi-route
    // rate, 1 drop = that vehicle's own Bulk tier, per transport team's rule).
    var dropCountByVehicleDate = {};
    drops.forEach(function (g) {
      var vNo = (g.orders[0] && g.orders[0].vehicle_no) || '';
      if (!vNo) return;
      var vKey = g.date + '::' + vNo;
      dropCountByVehicleDate[vKey] = (dropCountByVehicleDate[vKey] || 0) + 1;
    });
    var byOrgTemp = {};
    var total = { orders_hi: 0, orders_lo: 0, value_hi: 0, value_lo: 0, drops_hi: 0, drops_lo: 0, pure_drops_lo: 0, pure_orders_lo: 0, pure_value_lo: 0, pure_drop_cost_lo: 0, pure_fallback_rate_count: 0, pure_vehicle_master_count: 0 };
    drops.forEach(function (g) {
      var key = g.org + '||' + g.temp;
      if (!byOrgTemp[key]) byOrgTemp[key] = { org: g.org, type: g.temp, orders_hi: 0, orders_lo: 0, value_hi: 0, value_lo: 0, drops_hi: 0, drops_lo: 0, pure_drops_lo: 0, pure_orders_lo: 0, pure_value_lo: 0, pure_drop_cost_lo: 0, pure_fallback_rate_count: 0, pure_vehicle_master_count: 0 };
      var b = byOrgTemp[key];
      ...
      if (isHigh) { b.orders_hi += g.orders.length; b.value_hi += g.value; b.drops_hi++; total.orders_hi += g.orders.length; total.value_hi += g.value; total.drops_hi++; }
        ...
        // logic at all. Order value and drop cost (real vehicle-type rate card, falling
        // back to the Vehicle Master plate lookup, then the AED 130 average only as a
        // last resort) tracked here specifically, since these are single-order drops —
        // the drop's own truck_type/vehicle_no IS that one order's.
        if (g.orders.length === 1) {
          b.pure_drops_lo++; b.pure_orders_lo++; total.pure_drops_lo++; total.pure_orders_lo++;
          b.pure_value_lo += g.value; total.pure_value_lo += g.value;
          var vNo = g.orders[0].vehicle_no || '';
          var vDropCount = vNo ? (dropCountByVehicleDate[g.date + '::' + vNo] || 1) : 1;
          var costInfo = dropCostFor(g.orders[0].truck_type, vNo, vDropCount);
          b.pure_drop_cost_lo += costInfo.rate; total.pure_drop_cost_lo += costInfo.rate;
          if (costInfo.is_fallback) { b.pure_fallback_rate_count++; total.pure_fallback_rate_count++; }
          else if (costInfo.source === 'vehicle_master') { b.pure_vehicle_master_count++; total.pure_vehicle_master_count++; }
        }
      }
    });
    var out = Object.keys(byOrgTemp).sort().map(function (k) {
      var b = byOrgTemp[k];
      return { org: b.org, type: b.type, orders_ge1k: b.orders_hi, orders_lt1k: b.orders_lo, drops_ge1k: b.drops_hi, drops_lt1k: b.drops_lo, value_ge1k: Math.round(b.value_hi), value_lt1k: Math.round(b.value_lo), pure_drops_lt1k: b.pure_drops_lo, pure_orders_lt1k: b.pure_orders_lo, pure_value_lt1k: Math.round(b.pure_value_lo), pure_drop_cost_lt1k: Math.round(b.pure_drop_cost_lo), pure_fallback_rate_count: b.pure_fallback_rate_count, pure_vehicle_master_count: b.pure_vehicle_master_count };
    });
    return {
      rows: out, threshold: threshold,
      combined: { org: 'TOTAL (All BU)', type: 'All Types', orders_ge1k: total.orders_hi, orders_lt1k: total.orders_lo, drops_ge1k: total.drops_hi, drops_lt1k: total.drops_lo, value_ge1k: Math.round(total.value_hi), value_lt1k: Math.round(total.value_lo), pure_drops_lt1k: total.pure_drops_lo, pure_orders_lt1k: total.pure_orders_lo, pure_value_lt1k: Math.round(total.pure_value_lo), pure_drop_cost_lt1k: Math.round(total.pure_drop_cost_lo), pure_fallback_rate_count: total.pure_fallback_rate_count, pure_vehicle_master_count: total.pure_vehicle_master_count }
    };
  }
  ...
  function buildLowValueConsolidation(valueTier) {
    function shape(row) {
      ...
      // Order value moving through COMBINED (non-pure) low-value drops — i.e. total
      // low-value order value minus the portion that's in standalone/pure drops.
      var combinedOrderValueAed = Math.max(0, (row.value_lt1k || 0) - (row.pure_value_lt1k || 0));
      return {
        org: row.org, type: row.type,
        ...
        combined_order_value_aed: Math.round(combinedOrderValueAed),
        // "Pure" low-value orders — sent as their own dedicated drop, never combined
        // with anything else at that address. Order value = the real sales value of
        // those orders. Drop cost = the real transport-team rate card, matched by
        // each drop's actual vehicle/truck type (AED 130 average used only where the
        // truck type is missing on file — tracked separately below so that's never
        // hidden inside the total). This is the actual cost of running these drops —
        // NOT a claim that it's extra or avoidable, since the truck may well be on
        // its normal route regardless; it's simply what these specific drops cost.
        pure_low_value_orders: row.pure_orders_lt1k || 0,
        pure_low_value_drops: row.pure_drops_lt1k || 0,
        pure_order_value_aed: row.pure_value_lt1k || 0,
        pure_drop_cost_aed: row.pure_drop_cost_lt1k || 0,
        pure_fallback_rate_count: row.pure_fallback_rate_count || 0,
        pure_vehicle_master_count: row.pure_vehicle_master_count || 0
      };
    }
    return { rows: valueTier.rows.map(shape), combined: shape(valueTier.combined) };
  }
  ...
  app.get('/api/transport-cost/export', requireAuth, async function (req, res) {
    try {
      ...
      es.addRow(['Order Value Moving Through Combined Drops (AED)', data.low_value_consolidation.combined.combined_order_value_aed]);
      ...
      es.addRow(['Pure Orders \u2014 Order Value (AED)', data.low_value_consolidation.combined.pure_order_value_aed]);
      var pureDropCostRow = es.addRow(['Pure Orders \u2014 Drop Cost, Real Vehicle-Type Rate Card (AED)', data.low_value_consolidation.combined.pure_drop_cost_aed]);
      if (data.low_value_consolidation.combined.pure_drop_cost_aed > 0) pureDropCostRow.getCell(2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: AMBER } };
      var vmRow = es.addRow(['...of which resolved via Vehicle Master plate lookup (no truck type on file)', data.low_value_consolidation.combined.pure_vehicle_master_count]);
      vmRow.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
      var fallbackRow = es.addRow(['...of which used the AED 130 fallback (no truck type or matching vehicle plate)', data.low_value_consolidation.combined.pure_fallback_rate_count]);
      fallbackRow.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
      ...
      lvs.columns = [{ width: 16 }, { width: 14 }, { width: 18 }, { width: 16 }, { width: 22 }, { width: 20 }, { width: 20 }, { width: 20 }, { width: 22 }, { width: 22 }, { width: 20 }, { width: 16 }];
      titleRow(lvs, 'LOW-VALUE ORDER CONSOLIDATION SAVINGS \u2014 ' + data.period.label + ' (threshold: AED ' + data.value_tier.threshold.toLocaleString() + ')', 'L');
      lvs.addRow([]);
      var lvsNote = lvs.addRow(['A low-value order (below the threshold above) isn\'t automatically a separate truck stop \u2014 if two or more share the same date/route/location, they already ride on ONE drop together. "Saved by Consolidation" = orders that got folded into a shared drop \u2014 a genuine, defensible transport cost saving. "Pure" = orders that went out as their OWN dedicated drop, never combined at that address \u2014 Drop Cost here tries the REAL transport-team vehicle-type rate card first (matched by the drop\'s own truck type text), then the uploaded Vehicle Master (matched by the drop\'s vehicle plate) if truck type wasn\'t on file, and only falls back to an AED 130 average if neither resolves anything \u2014 that fallback count is shown separately so it\'s never hidden inside the total. This is the real cost of running these drops \u2014 not a claim that it\'s extra or avoidable spend, since the truck may already be on its normal route regardless.']);
      lvs.mergeCells('A' + lvsNote.number + ':L' + lvsNote.number); lvsNote.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
      lvs.addRow([]);
      headerRow(lvs, ['BU', 'Type', 'Low-Value Orders', 'Low-Value Drops', 'Orders Saved by Consolidation', 'Estimated Cost Saved (AED)', 'Order Value in Combined Drops (AED)', 'Pure (Non-Combined) Orders', 'Pure Order Value (AED)', 'Pure Drop Cost (AED)', 'Via Vehicle Master', 'Via AED 130 Fallback']);
      data.low_value_consolidation.rows.concat([data.low_value_consolidation.combined]).forEach(function (r) {
        var isTotal = r.org.indexOf('TOTAL') !== -1;
        var row = lvs.addRow([r.org, r.type, r.low_value_orders, r.low_value_drops, r.orders_saved_by_consolidation, r.estimated_cost_saved_aed, r.combined_order_value_aed, r.pure_low_value_orders, r.pure_order_value_aed, r.pure_drop_cost_aed, r.pure_vehicle_master_count, r.pure_fallback_rate_count]);
        row.getCell(6).numFmt = '#,##0'; row.getCell(7).numFmt = '#,##0'; row.getCell(9).numFmt = '#,##0'; row.getCell(10).numFmt = '#,##0';
        ...
        if (r.pure_drop_cost_aed > 0) row.getCell(10).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: AMBER } };
        if (isTotal) row.eachCell(function (c) { c.font = { bold: true }; });
      });
      lvs.addRow([]);
