...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, bcrypt) {
  ...
  async function initBrandsDB() {
    try {
      ...
      await pool.query(`ALTER TABLE brand_versions ADD COLUMN IF NOT EXISTS batch_id TEXT`);
      ...
      await pool.query(`CREATE INDEX IF NOT EXISTS idx_brand_versions_batch ON brand_versions(batch_id)`);
  ...
  // ============================================================
  // BULK UPLOAD — one file, every brand. For 50 brands, uploading one
  // file per brand every cycle doesn't scale. This adds a "Brand" column
  // to each tab so many brands' rows live in one workbook; the system
  // splits it apart, validates each brand's rows independently, and
  // creates one draft per brand — reusing the exact same validation and
  // draft logic as the single-brand upload, just run once per brand found.
  // Admin-only: touching every brand's data in one action is an admin action.
  // ============================================================

  app.get('/api/brands/bulk-template', requireAuth, requireRole('superadmin'), async function (req, res) {
    try {
      ...
      const brandFill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFE8F0FE' } };
      ...
      function addSheet(name, title, headers, exampleRow, colWidths) {
        const ws = wb.addWorksheet(name);
        const full = ['Brand', ...headers];
        ws.mergeCells(1, 1, 1, full.length);
        ws.getCell(1, 1).value = title; ws.getCell(1, 1).font = titleFont;
        ws.mergeCells(2, 1, 2, full.length);
        ws.getCell(2, 1).value = 'Brand column must match a brand name exactly — see the Brand List tab. Yellow = your data.';
        ws.getCell(2, 1).font = legendFont;
        full.forEach((h, i) => {
          const c = ws.getCell(4, i + 1);
          c.value = h; c.font = headerFont; c.alignment = { horizontal: 'center', vertical: 'middle' };
          c.fill = i === 0 ? brandFill : headerFill;
          if (i === 0) c.font = { ...headerFont, color: { argb: 'FF1F7A76' } };
        });
        if (exampleRow) { ws.getCell(5, 1).value = '(Example Brand)'; ws.getCell(5, 1).font = exampleFont;
          exampleRow.forEach((v, i) => { const c = ws.getCell(5, i + 2); c.value = v; c.font = exampleFont; }); }
        for (let r = 6; r <= 40; r++) { for (let c = 1; c <= full.length; c++) { ws.getCell(r, c).fill = c === 1 ? brandFill : inputFill; } }
        ws.getColumn(1).width = 22;
        colWidths.forEach((w, i) => { ws.getColumn(i + 2).width = w; });
      }

      addSheet('Target vs Achievement', 'Multi-Brand: Target vs Achievement', ['SKU Name', 'Target (Units)', 'Achieved (Units)', 'Period'],
        ['Baby Wipes 80s', 12000, 11280, 'Jul-2026'], [26, 16, 18, 16]);
      addSheet('Distribution Plan', 'Multi-Brand: Distribution Plan', ['Region / Channel', 'Planned Stores', 'Actual Stores Reached'],
        ['Dubai HoReCa', 120, 108], [26, 18, 20]);
      addSheet('Stock Status', 'Multi-Brand: Stock Status', ['SKU Name', 'Stock Value (AED)', 'Volume (Units)', 'Ageing Bucket (0-30/31-60/61+)', 'Shelf Life Status (Healthy/Monitor/Near Expiry)'],
        ['Baby Lotion 200ml', 29750, 3960, '61+', 'Near Expiry'], [24, 18, 16, 30, 30]);

      // Reference tab — exact brand names the uploader must match
      const brandsR = await pool.query(`SELECT name FROM brands ORDER BY name`);
      const refWs = wb.addWorksheet('Brand List');
      refWs.getCell(1, 1).value = 'Exact Brand Names — copy these into the Brand column';
      refWs.getCell(1, 1).font = titleFont;
      brandsR.rows.forEach((b, i) => { refWs.getCell(3 + i, 1).value = b.name; refWs.getCell(3 + i, 1).font = { name: 'Arial', size: 11 }; });
      refWs.getColumn(1).width = 40;

      const buf = await wb.xlsx.writeBuffer();
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.setHeader('Content-Disposition', 'attachment; filename="Bulk_Brand_Data_Template.xlsx"');
  ...
  app.post('/api/brands/bulk-upload-excel', requireAuth, requireRole('superadmin'), upload.single('file'), async function (req, res) {
    try {
      if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
      const { reporting_month, reporting_year } = req.body;
      if (!reporting_month || !reporting_year) return res.status(400).json({ error: 'Reporting month and year are required' });

      const wb = XLSX.read(req.file.buffer, { type: 'buffer' });
      const sheetJson = (name) => wb.Sheets[name] ? XLSX.utils.sheet_to_json(wb.Sheets[name], { range: 3, defval: '' }) : [];

      const brandsR = await pool.query(`SELECT id, name FROM brands`);
      const brandByName = {};
      brandsR.rows.forEach(b => { brandByName[String(b.name).trim().toLowerCase()] = b; });

      // Group every sheet's rows by the Brand column
      function groupByBrand(rows) {
        const groups = {};
        rows.forEach(r => {
          const brandName = String(r['Brand'] || '').trim();
          if (!brandName) return;
          groups[brandName] = groups[brandName] || [];
          groups[brandName].push(r);
        });
        return groups;
      }
      const targetGroups = groupByBrand(sheetJson('Target vs Achievement'));
      const distGroups = groupByBrand(sheetJson('Distribution Plan'));
      const stockGroups = groupByBrand(sheetJson('Stock Status'));

      const allBrandNames = new Set([...Object.keys(targetGroups), ...Object.keys(distGroups), ...Object.keys(stockGroups)]);
      const batchId = 'batch_' + Date.now() + '_' + req.user.uid;
      const results = [];

      for (const brandName of allBrandNames) {
        const brand = brandByName[brandName.trim().toLowerCase()];
        if (!brand) { results.push({ brand: brandName, status: 'unknown_brand', error: `No brand found named "${brandName}" — check spelling against the Brand List tab` }); continue; }

        const errors = [], warnings = [];
        const tRows = (targetGroups[brandName] || []).filter(r => !isBlank(r['SKU Name']));
        const dRows = (distGroups[brandName] || []).filter(r => !isBlank(r['Region / Channel']));
        const sRows = (stockGroups[brandName] || []).filter(r => !isBlank(r['SKU Name']));
        if (tRows.length) validateTargetVsAchievement(tRows, errors, warnings);
        if (dRows.length) validateDistributionPlan(dRows, errors, warnings);
        if (sRows.length) validateStockStatus(sRows, errors, warnings);

        const excelSections = [];
        if (tRows.length) {
          const items = tRows.map(r => ({ sku: r['SKU Name'], target: Number(r['Target (Units)']) || 0, achieved: Number(r['Achieved (Units)']) || 0 }));
          ...
          const kpis = [{ label: 'Achieved / Target', value: items.reduce((a,b)=>a+b.achieved,0), compare_value: items.reduce((a,b)=>a+b.target,0), unit: 'units' }];
          if (dRows.length) kpis.push({ label: 'Store Coverage', value: dRows.reduce((a,r)=>a+(Number(r['Actual Stores Reached'])||0),0), compare_value: dRows.reduce((a,r)=>a+(Number(r['Planned Stores'])||0),0), unit: 'stores' });
        ...
        if (sRows.length) {
          const rows = sRows.map(r => ({ sku: r['SKU Name'], value: Number(r['Stock Value (AED)'])||0, volume: Number(r['Volume (Units)'])||0, ageing: r['Ageing Bucket (0-30/31-60/61+)']||'0-30', shelf_status: r['Shelf Life Status (Healthy/Monitor/Near Expiry)']||'Healthy' }));
        ...
        const liveSections = await pool.query(`SELECT template_type, section_order, chapter_tag, title, data FROM brand_sections WHERE brand_id=$1 AND section_order NOT IN (1,2,3,5)`, [brand.id]);
        const fullSectionsData = [...liveSections.rows, ...excelSections].sort((a,b)=>a.section_order-b.section_order);

        await pool.query(`UPDATE brand_versions SET status='cancelled' WHERE brand_id=$1 AND status='draft'`, [brand.id]);
        const verNum = (await pool.query(`SELECT COALESCE(MAX(version_number),0)+1 AS n FROM brand_versions WHERE brand_id=$1`, [brand.id])).rows[0].n;
        const months = ['','January','February','March','April','May','June','July','August','September','October','November','December'];
        const presentationName = `${brand.name} — ${months[reporting_month]} ${reporting_year} Review`;

        const draft = await pool.query(
          `INSERT INTO brand_versions (brand_id, version_number, presentation_name, reporting_month, reporting_year, status, sections_data, validation, source_file_name, batch_id, created_by)
           VALUES ($1,$2,$3,$4,$5,'draft',$6,$7,$8,$9,$10) RETURNING id, version_number`,
          [brand.id, verNum, presentationName, reporting_month, reporting_year, JSON.stringify(fullSectionsData), JSON.stringify({errors,warnings}), req.file.originalname, batchId, req.user.uid]
        );
        results.push({ brand: brand.name, brand_id: brand.id, status: errors.length ? 'draft_with_errors' : 'draft_ready', draft_id: draft.rows[0].id, version_number: draft.rows[0].version_number, errors: errors.length, warnings: warnings.length });
      }

      await auditLog(req.user.uid, req.user.username, 'brand_bulk_upload', `${results.length} brands, batch ${batchId}`, req.ip);
  ...
  // Publish every clean (zero-error) draft in a batch, in one action. Anything
  // with errors is left as a draft, untouched, for individual review.
  app.post('/api/brands/bulk-publish/:batchId', requireAuth, requireRole('superadmin'), async function (req, res) {
    try {
      const drafts = await pool.query(`SELECT * FROM brand_versions WHERE batch_id=$1 AND status='draft'`, [req.params.batchId]);
      const published = [], skipped = [];
      for (const version of drafts.rows) {
        if ((version.validation.errors || []).length > 0) { skipped.push({ brand_id: version.brand_id, version_id: version.id, reason: 'has errors' }); continue; }
        const client = await pool.connect();
        try {
          await client.query('BEGIN');
          await client.query(`UPDATE brand_versions SET status='superseded' WHERE brand_id=$1 AND status='published'`, [version.brand_id]);
          for (const sec of version.sections_data) {
            const existing = await client.query(`SELECT id, data, data_source FROM brand_sections WHERE brand_id=$1 AND section_order=$2`, [version.brand_id, sec.section_order]);
            if (existing.rows[0]) {
              await client.query(`INSERT INTO brand_section_history (section_id, data, data_source, replaced_by) VALUES ($1,$2,$3,$4)`, [existing.rows[0].id, existing.rows[0].data, existing.rows[0].data_source, req.user.uid]);
              await client.query(`UPDATE brand_sections SET template_type=$1, chapter_tag=$2, title=$3, data=$4, data_source='excel_upload', last_refreshed_at=NOW(), updated_by=$5, updated_at=NOW() WHERE id=$6`, [sec.template_type, sec.chapter_tag, sec.title, JSON.stringify(sec.data), req.user.uid, existing.rows[0].id]);
            } else {
              await client.query(`INSERT INTO brand_sections (brand_id, template_type, section_order, chapter_tag, title, data, data_source, last_refreshed_at, updated_by) VALUES ($1,$2,$3,$4,$5,$6,'excel_upload',NOW(),$7)`, [version.brand_id, sec.template_type, sec.section_order, sec.chapter_tag, sec.title, JSON.stringify(sec.data), req.user.uid]);
            }
          }
          await client.query(`UPDATE brand_versions SET status='published', published_by=$1, published_at=NOW() WHERE id=$2`, [req.user.uid, version.id]);
          await client.query('COMMIT');
          published.push({ brand_id: version.brand_id, version_id: version.id, version_number: version.version_number });
        } catch (e) {
          await client.query('ROLLBACK');
          skipped.push({ brand_id: version.brand_id, version_id: version.id, reason: e.message });
        } finally {
          client.release();
        }
      }
      await auditLog(req.user.uid, req.user.username, 'brand_bulk_publish', `batch ${req.params.batchId}: ${published.length} published, ${skipped.length} skipped`, req.ip);
  ...
  app.post('/api/brands/:id/upload-excel', requireAuth, requireBrandManage(), upload.single('file'), async function (req, res) {
    try {
      ...
      if (fname.endsWith('.pptx') || fname.endsWith('.ppt') || fname.endsWith('.key')) {
        return res.status(400).json({ error: 'This is a PowerPoint file. "Upload & refresh" needs the Excel data template (.xlsx) — download it below, fill in your numbers, and upload that instead.' });
      }
      if (fname.endsWith('.pdf') || fname.endsWith('.docx') || fname.endsWith('.csv')) {
        return res.status(400).json({ error: 'This needs to be the .xlsx Excel data template, not a ' + fname.split('.').pop().toUpperCase() + ' file. Download the template below.' });
      }
      if (!fname.endsWith('.xlsx')) {
        return res.status(400).json({ error: 'Please upload the .xlsx Excel data template — download it below if you need a fresh copy.' });
      }
      ...
      try {
        ...
        return res.status(400).json({ error: "Couldn't read that file as Excel data. Make sure it's the .xlsx template, not a renamed or corrupted file." });
      }
      ...
      res.status(500).json({ error: 'Something went wrong processing that file. Check it matches the template and try again.' });
    }
  });
  ...
  // ============================================================
  // POWERPOINT EXPORT — real, downloadable, native charts + tables.
  // Works for any brand, using whatever is currently live, or a specific
  // published version if ?version_id= is passed (so history entries can
  // be downloaded too, not just the current live deck).
  //
  // Honest limitation, stated once here rather than silently: a .pptx is
  // a static file. The count-up numbers, radial rings, and interactive
  // SWOT radar are JavaScript running in a browser — PowerPoint has no
  // equivalent. This export gives every section a clean static layout
  // instead (bar/pie charts stay native and editable in PowerPoint; the
  // SWOT radar becomes a plain 2x2 grid, since a static radar shape adds
  // visual complexity without adding information).
  // ============================================================
  app.get('/api/brands/:id/export/pptx', requireAuth, requireBrandView(), async function (req, res) {
    try {
      const pptxgen = require('pptxgenjs');
      const brandR = await pool.query(`SELECT name, accent_color, tagline FROM brands WHERE id=$1`, [req.params.id]);
      if (!brandR.rows[0]) return res.status(404).json({ error: 'Brand not found' });
      const brand = brandR.rows[0];

      let sections, deckLabel;
      if (req.query.version_id) {
        const vr = await pool.query(`SELECT sections_data, presentation_name FROM brand_versions WHERE id=$1 AND brand_id=$2`, [req.query.version_id, req.params.id]);
        if (!vr.rows[0]) return res.status(404).json({ error: 'Version not found' });
        sections = vr.rows[0].sections_data;
        deckLabel = vr.rows[0].presentation_name;
      } else {
        const sr = await pool.query(`SELECT template_type, section_order, chapter_tag, title, data FROM brand_sections WHERE brand_id=$1 ORDER BY section_order`, [req.params.id]);
        sections = sr.rows;
        deckLabel = brand.name;
      }

      const accent = (brand.accent_color || '#c6a15b').replace('#', '').toUpperCase();
      const INK = '1A1A1A', MUTED = '6B7280', LIGHT_BG = 'F7F9F9', LIGHT_BORDER = 'E5EBEB';
      ...
      function baseSlide(kicker, title) {
        ...
        if (kicker) s.addText(kicker.toUpperCase(), { x: 0.6, y: 0.45, w: 11, h: 0.35, fontFace: BODY_FONT, fontSize: 11, color: accent, bold: true, charSpacing: 2 });
        s.addText(title, { x: 0.6, y: 0.75, w: 11.8, h: 0.9, fontFace: TITLE_FONT, fontSize: 28, color: INK });
        return s;
      }
      function pillColor(status, kind) {
        if (kind === 'ageing') return status === '0-30' ? accent : status === '31-60' ? 'E2A63F' : 'D9705F';
        if (kind === 'shelf') return status === 'Healthy' ? accent : status === 'Monitor' ? 'E2A63F' : 'D9705F';
        return MUTED;
      }

      for (const sec of sections) {
        const d = sec.data || {};
        if (sec.template_type === 'cover') {
          const s = pres.addSlide();
          s.background = { color: accent };
          s.addText(brand.name.slice(0, 2).toUpperCase(), { x: 0.9, y: 2.4, w: 1.5, h: 1.5, fontFace: TITLE_FONT, fontSize: 30, color: 'FFFFFF', bold: true, align: 'center', valign: 'middle', shape: pres.ShapeType.roundRect, fill: { color: 'FFFFFF', transparency: 88 }, line: { color: 'FFFFFF', width: 1 } });
          if (brand.tagline) s.addText(brand.tagline, { x: 0.9, y: 4.05, w: 9, h: 0.4, fontFace: BODY_FONT, fontSize: 13, color: 'FFFFFF' });
          s.addText(brand.name, { x: 0.85, y: 4.4, w: 10, h: 1.3, fontFace: TITLE_FONT, fontSize: 50, color: 'FFFFFF' });
          s.addText(deckLabel, { x: 0.9, y: 5.55, w: 9, h: 0.5, fontFace: BODY_FONT, fontSize: 15, color: 'FFFFFF' });

        } else if (sec.template_type === 'kpi_hero') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'Key Metrics');
          (d.kpis || []).forEach((k, i) => {
            const x = 0.6 + (i % 3) * 4.1, y = 1.9 + Math.floor(i / 3) * 1.7;
            s.addText(String(k.label || '').toUpperCase(), { x, y, w: 3.8, h: 0.3, fontFace: BODY_FONT, fontSize: 10, color: MUTED, charSpacing: 1 });
            s.addText(Number(k.value).toLocaleString(), { x, y: y + 0.3, w: 3.8, h: 0.7, fontFace: TITLE_FONT, fontSize: 30, color: INK });
            if (k.compare_value) s.addText(Math.round(100 * k.value / k.compare_value) + '% of ' + Number(k.compare_value).toLocaleString(), { x, y: y + 0.95, w: 3.8, h: 0.3, fontFace: BODY_FONT, fontSize: 10.5, color: accent, bold: true });
          });

        } else if (sec.template_type === 'sku_rings') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'SKU Performance');
          const items = d.items || [];
          const chartData = [
            { name: 'Achieved', labels: items.map(i => i.sku), values: items.map(i => i.achieved) },
            { name: 'Target', labels: items.map(i => i.sku), values: items.map(i => i.target) },
          ];
          s.addChart(pres.ChartType.bar, chartData, {
            x: 0.5, y: 1.9, w: 12.3, h: 5.0, barDir: 'col', barGrouping: 'clustered',
            showLegend: true, legendPos: 'b', legendColor: MUTED, legendFontSize: 11,
            chartColors: [accent, 'D9D9D9'], showValue: true, dataLabelPosition: 'outEnd', dataLabelColor: INK, dataLabelFontSize: 10,
            catAxisLabelColor: MUTED, catAxisLabelFontSize: 10, catAxisLineShow: false,
            valAxisLabelColor: MUTED, valAxisLabelFontSize: 10, valAxisLineShow: false,
            valGridLine: { color: 'EDEDED', size: 1 }, catGridLine: { style: 'none' },
          });

        } else if (sec.template_type === 'stock_table') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'Stock Status');
          const header = ['SKU', 'Stock Value', 'Volume', 'Ageing', 'Shelf Life'].map(h => ({ text: h, options: { bold: true, color: 'FFFFFF', fill: { color: accent }, fontFace: BODY_FONT, fontSize: 12, align: 'center', valign: 'middle' } }));
          const rows = [header, ...(d.rows || []).map((row, ri) => [
            { text: row.sku, options: { fontFace: BODY_FONT, fontSize: 12, color: INK, fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: 'AED ' + Number(row.value).toLocaleString(), options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: Number(row.volume).toLocaleString(), options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: row.ageing, options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', color: pillColor(row.ageing, 'ageing'), fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: row.shelf_status, options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', color: pillColor(row.shelf_status, 'shelf'), fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
          ])];
          s.addTable(rows, { x: 0.6, y: 1.9, w: 12.1, h: Math.min(0.6 * rows.length, 4.8), colW: [3.6, 2.5, 2.0, 2.0, 2.0], border: { type: 'none' }, autoPage: false, rowH: 0.6 });

        } else if (sec.template_type === 'ranked_table') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'Performance');
          const cols = d.columns || ['Label', 'Prior', 'Current', 'Growth %', 'Contribution %'];
          const header = cols.map(h => ({ text: h, options: { bold: true, color: 'FFFFFF', fill: { color: accent }, fontFace: BODY_FONT, fontSize: 12, align: 'center', valign: 'middle' } }));
          const rows = [header, ...(d.rows || []).map((row, ri) => [
            { text: row.label, options: { fontFace: BODY_FONT, fontSize: 12, fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: Number(row.year_prior || 0).toLocaleString(), options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: Number(row.year_current || 0).toLocaleString(), options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: (row.growth_pct >= 0 ? '+' : '') + row.growth_pct + '%', options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
            { text: row.contribution_pct + '%', options: { fontFace: BODY_FONT, fontSize: 12, align: 'center', fill: { color: ri % 2 ? 'FFFFFF' : LIGHT_BG }, valign: 'middle' } },
          ])];
          s.addTable(rows, { x: 0.6, y: 1.9, w: 12.1, h: Math.min(0.6 * rows.length, 4.8), border: { type: 'none' }, autoPage: false, rowH: 0.6 });

        } else if (sec.template_type === 'swot_radar') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'SWOT Analysis');
          const quads = [
            { label: 'Strengths', items: d.strengths || [], color: accent, x: 0.6, y: 1.9 },
            { label: 'Weaknesses', items: d.weaknesses || [], color: 'E2A63F', x: 6.75, y: 1.9 },
            { label: 'Opportunities', items: d.opportunities || [], color: '3CB6AE', x: 0.6, y: 4.35 },
            { label: 'Threats', items: d.threats || [], color: 'D9705F', x: 6.75, y: 4.35 },
          ];
          quads.forEach(q => {
            s.addShape(pres.ShapeType.roundRect, { x: q.x, y: q.y, w: 5.9, h: 2.25, rectRadius: 0.06, fill: { color: LIGHT_BG }, line: { color: LIGHT_BORDER, width: 1 } });
            s.addText(q.label.toUpperCase(), { x: q.x + 0.25, y: q.y + 0.15, w: 5.4, h: 0.3, fontFace: BODY_FONT, fontSize: 11, color: q.color, bold: true, charSpacing: 1 });
            const bodyText = q.items.length ? q.items.map(t => ({ text: t, options: { bullet: { code: '2022' }, breakLine: true } })) : [{ text: 'No items yet' }];
            s.addText(bodyText, { x: q.x + 0.25, y: q.y + 0.5, w: 5.4, h: 1.6, fontFace: BODY_FONT, fontSize: 11, color: INK, valign: 'top', paraSpaceAfter: 4 });
          });

        } else if (sec.template_type === 'initiative_grid') {
          const s = baseSlide(sec.chapter_tag, sec.title || 'Initiatives');
          (d.items || []).forEach((it, i) => {
            const col = i % 2, row = Math.floor(i / 2);
            const x = 0.6 + col * 6.15, y = 1.95 + row * 1.55;
            if (y > 6.8) return;
            s.addShape(pres.ShapeType.roundRect, { x, y, w: 5.85, h: 1.35, rectRadius: 0.06, fill: { color: LIGHT_BG }, line: { color: LIGHT_BORDER, width: 1 } });
            s.addText(String(i + 1).padStart(2, '0'), { x: x + 0.2, y: y + 0.12, w: 0.5, h: 0.3, fontFace: BODY_FONT, fontSize: 10, color: accent, bold: true });
            s.addText(it.title || '', { x: x + 0.2, y: y + 0.38, w: 5.4, h: 0.35, fontFace: BODY_FONT, fontSize: 13, color: INK, bold: true });
            s.addText(it.note || '', { x: x + 0.2, y: y + 0.72, w: 5.4, h: 0.4, fontFace: BODY_FONT, fontSize: 10.5, color: MUTED });
            s.addText(String(it.status || '').toUpperCase(), { x: x + 0.2, y: y + 1.08, w: 3, h: 0.25, fontFace: BODY_FONT, fontSize: 8.5, color: accent, bold: true, charSpacing: 0.5 });
          });
        }
        // document_list intentionally skipped — files don't translate into a static slide.
      }

      const buf = await pres.write({ outputType: 'nodebuffer' });
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.presentationml.presentation');
      res.setHeader('Content-Disposition', `attachment; filename="${brand.name.replace(/\s+/g, '_')}_Presentation.pptx"`);
      res.send(buf);
    } catch (e) {
      res.status(500).json({ error: e.message });
    }
