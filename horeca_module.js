...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, bcrypt, crypto) {
  ...
  app.post('/api/horeca/master/customers/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      for (const row of rows) {
        ...
        const spName = row['New Salespersons'] || null;
  ...
  app.get('/api/horeca/customers', requireAuth, async function (req, res) {
    try {
      ...
      if (req.user.role === 'salesman') {
        const code = req.user.horeca_salesperson_code;
        if (!code) return res.json({ customers: [] });
        const r = await pool.query(
          `SELECT account_number, customer_name, customer_category, location, site_use_id
           FROM horeca_customer_sites WHERE salesperson_code=$1 AND active=true ORDER BY customer_name`,
          [code]
        );
        rows = r.rows;
      } else {
  ...
  app.get('/api/horeca/salesperson-codes', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
    try {
      const r = await pool.query(
        `SELECT DISTINCT salesperson_code, salesperson_name FROM horeca_customer_sites
         WHERE salesperson_code IS NOT NULL AND salesperson_code != '' ORDER BY salesperson_name`
      );
  ...
  app.get('/api/horeca/salesmen', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
    try {
      const r = await pool.query(
        `SELECT u.id, u.username, u.full_name, u.horeca_salesperson_code, u.horeca_salesperson_name, u.active, u.last_login, u.created_at,
                (SELECT COUNT(*) FROM horeca_customer_sites s WHERE s.salesperson_code = u.horeca_salesperson_code) AS assigned_sites,
  ...
  app.post('/api/horeca/salesmen', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
    try {
      const { username, password, full_name, salesperson_code, salesperson_name } = req.body;
      ...
      const r = await pool.query(
        `INSERT INTO users (username, password_hash, full_name, role, horeca_salesperson_code, horeca_salesperson_name, must_change_password, created_by)
         VALUES ($1,$2,$3,'salesman',$4,$5,true,$6) RETURNING id, username, full_name, horeca_salesperson_code`,
        [username.toLowerCase().trim(), hash, full_name, salesperson_code || null, salesperson_name || null, req.user.username]
      );
  ...
  app.put('/api/horeca/salesmen/:id', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
    try {
      const { full_name, salesperson_code, salesperson_name, active } = req.body;
      await pool.query(
        `UPDATE users SET
           full_name = COALESCE($1, full_name),
           horeca_salesperson_code = $2,
           horeca_salesperson_name = $3,
           active = COALESCE($4, active)
         WHERE id = $5 AND role = 'salesman'`,
        [full_name || null, salesperson_code || null, salesperson_name || null, active, req.params.id]
      );
