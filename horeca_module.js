...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, bcrypt, crypto) {
  ...
  async function initHorecaDB() {
    try {
      ...
      await pool.query(`ALTER TABLE horeca_customer_sites ADD COLUMN IF NOT EXISTS address_detail TEXT`);
  ...
  app.post('/api/horeca/master/customers/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      for (const row of rows) {
        ...
        // Address1 usually just repeats the Location label (e.g. "Food Services"), so the
        // genuinely useful street-level detail is Address2/Address3 + City — combine those
        // so a salesman can actually tell two same-named sites apart.
        ...
        if (existing.rows.length) {
          await pool.query(
            `UPDATE horeca_customer_sites SET customer_name=$1, customer_category=$2, salesperson_code=$3, salesperson_name=$4, location=$5, address_detail=$6, updated_at=NOW()
             WHERE id=$7`,
            [customerName, category, spCode, spName, location, addressDetail, existing.rows[0].id]
          );
          updated++;
        } else {
          await pool.query(
            `INSERT INTO horeca_customer_sites (account_number, customer_name, customer_category, salesperson_code, salesperson_name, location, address_detail, site_use_id)
             VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
            [accountNumber, customerName, category, spCode, spName, location, addressDetail, siteUseId || '']
          );
          inserted++;
        }
      }
      ...
      if (req.user.role === 'salesman') {
        ...
        const r = await pool.query(
          `SELECT account_number, customer_name, customer_category, location, address_detail, site_use_id
           FROM horeca_customer_sites WHERE UPPER(TRIM(salesperson_name))=UPPER(TRIM($1)) AND active=true ORDER BY customer_name`,
          [name]
        );
        rows = r.rows;
      } else {
        const r = await pool.query(
          `SELECT account_number, customer_name, customer_category, location, address_detail, site_use_id, salesperson_name
           FROM horeca_customer_sites WHERE active=true ORDER BY customer_name`
        );
        rows = r.rows;
      }
  ...
  // Normalize customer names for matching across dispatch / backlog / horeca sources
  // (mirrors cleanCustName() used in the main dashboard's backlog parser)
  function cleanCustName(name) {
    let n = String(name || '').trim();
    n = n.replace(/L\.L\.C\.?/gi, 'LLC').replace(/\bllc\b/gi, 'LLC');
    n = n.replace(/  +/g, ' ');
    n = n.replace(/[.,;]+$/, '').trim();
    return n.toUpperCase();
  }

  // Salesman: dispatch / backlog status for their own orders (last 7 days only)
  app.get('/api/horeca/orders/dispatch-status', requireAuth, requireHorecaRole('salesman'), async function (req, res) {
    try {
      const ordersR = await pool.query(
        `SELECT id, order_ref, customer_name, po_number, status, created_at
         FROM horeca_orders
         WHERE salesman_user_id = $1
      ...
      const trackR = await pool.query(
        `SELECT order_code, date_key, customer, value, route, drop_type, temperature
         FROM order_tracking WHERE date_key >= NOW() - INTERVAL '10 days'`
      );
      const tracking = trackR.rows;

      const backlogR = await pool.query(`SELECT summary FROM backlog_data ORDER BY uploaded_at DESC LIMIT 1`);
      const custMap = (backlogR.rows[0] && backlogR.rows[0].summary && backlogR.rows[0].summary.custMap) || {};
      const backlogByClean = {};
      Object.keys(custMap).forEach(function (k) { backlogByClean[cleanCustName(k)] = custMap[k]; });

      const result = orders.map(function (o) {
        const cleanName = cleanCustName(o.customer_name);
        const createdDate = new Date(o.created_at);
        const matches = tracking
          .filter(function (t) { return cleanCustName(t.customer) === cleanName && new Date(t.date_key) >= createdDate; })
          .sort(function (a, b) { return new Date(a.date_key) - new Date(b.date_key); });
        const dispatch = matches[0] || null;
        const backlog = !dispatch ? (backlogByClean[cleanName] || null) : null;

        let statusLabel, statusType;
        if (dispatch) {
          statusLabel = 'Dispatched ' + new Date(dispatch.date_key).toLocaleDateString('en-AE');
          statusType = 'dispatched';
        } else if (backlog) {
          statusLabel = 'In W/H Backlog';
          statusType = 'backlog';
        } else {
        ...
        return {
          ...
          dispatch: dispatch ? { date: dispatch.date_key, route: dispatch.route, value: dispatch.value, temperature: dispatch.temperature, order_code: dispatch.order_code } : null,
          backlog: backlog ? { count: backlog.count, value: backlog.val, topCat: backlog.topCat, oldestRSD: backlog.oldestRSD } : null
        };
      });
  ...
  app.get('/api/horeca/orders/:id/export-excel', requireAuth, requireHorecaRole('cs', 'viewer', 'subadmin'), async function (req, res) {
    try {
      ...
      ws.getCell('A2').value = 'HORECA ORDER FORM';
