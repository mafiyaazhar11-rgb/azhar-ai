...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  function buildGrvWorkbook(reqRow, lines) {
    ...
    // Oracle needs the real numeric Site Use ID when we have one — shown
    // together with the location name for readability ("20373 - Al Wuheida
    // Road D93"). If there's no valid number on file (e.g. an older
    // placeholder from before this location had a real ID), fall back to
    // just the location name text instead of a meaningless "0" or code.
    const siteIdNum = reqRow.site_use_id != null && !isNaN(reqRow.site_use_id) ? Number(reqRow.site_use_id) : null;
    const siteIdVal = (siteIdNum && siteIdNum !== 0)
      ? (reqRow.location ? `${siteIdNum} - ${reqRow.location}` : siteIdNum)
      : (reqRow.location || reqRow.site_use_id || '');
