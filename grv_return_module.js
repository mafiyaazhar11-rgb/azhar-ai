...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  function buildGrvWorkbook(reqRow, lines) {
    ...
    // Oracle needs the real numeric Site Use ID when we have one. If it's
    // missing, blank, or zero (e.g. an older synthetic placeholder from
    // before the location had a real ID on file), fall back to the location
    // name text instead so the cell is never just a meaningless "0" or code.
    const siteIdNum = reqRow.site_use_id != null && !isNaN(reqRow.site_use_id) ? Number(reqRow.site_use_id) : null;
    const siteIdVal = (siteIdNum && siteIdNum !== 0) ? siteIdNum : (reqRow.location || reqRow.site_use_id || '');
