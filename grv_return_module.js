...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
    ...
    try {
      ...
      for (const row of rows) {
        ...
        const existing = await pool.query(
          `SELECT id FROM grv_customer_sites WHERE account_number=$1 AND site_use_id=$2`,
          [accountNumber, siteUseId || '']
        );
        if (existing.rows.length) {
          await pool.query(
            `UPDATE grv_customer_sites SET customer_name=$1, customer_category=$2, salesperson_code=$3, salesperson_name=$4, location=$5, address_detail=$6, updated_at=NOW()
             WHERE id=$7`,
  ...
  // Upload Item Master (flexible columns: Item_Code/Item Code/AKI Code, Description, Brand, Category, UOM, Price, EA Barcode, Outer Barcode)
  app.post('/api/grv/master/items/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      for (const row of rows) {
        ...
        const eaBarcode = row['EA Barcode'] != null ? String(row['EA Barcode']).trim() : null;
        const outerBarcode = row['Outer Barcode'] != null ? String(row['Outer Barcode']).trim() : null;

        const existing = await pool.query(`SELECT id FROM grv_items WHERE item_code=$1`, [itemCode]);
        if (existing.rows.length) {
          await pool.query(
            `UPDATE grv_items SET description=$1, brand=$2, category=$3, uom=$4, price_per_unit=$5, ea_barcode=$6, outer_barcode=$7, updated_at=NOW() WHERE id=$8`,
            [description, brand, category, uom, price, eaBarcode || null, outerBarcode || null, existing.rows[0].id]
          );
          updated++;
        } else {
          await pool.query(
            `INSERT INTO grv_items (item_code, description, brand, category, uom, price_per_unit, ea_barcode, outer_barcode) VALUES ($1,$2,$3,$4,$5,$6,$7,$8)`,
            [itemCode, description, brand, category, uom, price, eaBarcode || null, outerBarcode || null]
          );
          inserted++;
        }
      }
    ...
    try {
      ...
      const r = await pool.query(
        `SELECT item_code, description, brand, category, uom, price_per_unit, ea_barcode, outer_barcode FROM grv_items
         WHERE active=true AND (item_code ILIKE $1 OR description ILIKE $1 OR ea_barcode ILIKE $1 OR outer_barcode ILIKE $1)
  ...
  // Salesman dropdown list — sourced from GRV's own master, not HoReCa's.
  app.get('/api/grv/salespersons', requireAuth, async function (req, res) {
    try {
      const r = await pool.query(
        `SELECT DISTINCT salesperson_name FROM grv_customer_sites
  ...
  app.post('/api/grv/requests', requireAuth, requireGrvRole('grv_sales'), async function (req, res) {
    try {
      const { account_number, customer_name, site_use_id, location, salesperson_name, warehouse, requester_email, lines } = req.body;
  ...
  app.put('/api/grv/requests/:id', requireAuth, requireGrvRole('grv_cs'), async function (req, res) {
    try {
      const { order_type, po_number, warehouse, status, assigned_to } = req.body;
      ...
      if (assignedToProvided) {
        await pool.query(
          `UPDATE grv_return_requests SET
             ...
             updated_at  = NOW()
           WHERE id = $6`,
          [order_type ?? null, po_number ?? null, warehouse ?? null, status ?? null, assignedToVal, req.params.id]
        );
      } else {
        await pool.query(
          `UPDATE grv_return_requests SET
