...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  function buildGrvWorkbook(reqRow, lines) {
    ...
    const ws = wb.addWorksheet('GRV Request');
    ws.columns = [
    ...
    ws.getCell('A2').value = 'GRV REQUEST';
    ...
    // C6 = the real numeric Site Use ID alone (what Oracle picks up).
    // D6 = the location name, in its own cell (matches the confirmed layout).
    // If there's no valid number on file (e.g. an older placeholder from
    // before this location had a real ID), C6 falls back to the location
    // name text instead of a meaningless "0", and D6 stays blank since
    // showing the name twice would be redundant in that case.
    const siteIdNum = reqRow.site_use_id != null && !isNaN(reqRow.site_use_id) ? Number(reqRow.site_use_id) : null;
    const hasRealSiteId = siteIdNum && siteIdNum !== 0;
    ws.getCell('C6').value = hasRealSiteId ? siteIdNum : (reqRow.location || reqRow.site_use_id || '');
    ws.getCell('D6').value = hasRealSiteId ? (reqRow.location || '') : '';
  ...
  app.post('/api/grv/master/salespersons/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      let skipped = 0;
      for (const row of rows) {
        const name = String(row['New Salespersons'] || row['Salesperson Name'] || row['Salesman Name'] || row['SalesMan'] || row['Salesman'] || row['Name'] || '').trim();
        if (!name) { skipped++; continue; }
        // Ensure the standard "CON-" prefix is present, without requiring
        // source files to already have it.
        const normalizedName = /^con-/i.test(name) ? name : ('CON-' + name);
        const code = row['Salesmen Number'] != null ? String(row['Salesmen Number']).trim() : (row['Code'] != null ? String(row['Code']).trim() : null);
        map.set(normalizedName, [code, normalizedName]);
      }
      const records = Array.from(map.values());
  ...
  app.get('/api/grv/items', requireAuth, async function (req, res) {
    try {
      ...
      let whereClause;
      if (field === 'code') whereClause = `(item_code ILIKE $1 OR ea_barcode ILIKE $1 OR outer_barcode ILIKE $1)`;
  ...
  app.get('/api/grv/requests/:id/export-excel', requireAuth, requireGrvRole('grv_cs'), async function (req, res) {
    try {
      ...
      await wb.xlsx.write(res);
      res.end();
      // Mark as Downloaded/Closed the moment CS actually pulls the file —
      // this is what flips the status badge from red (pending) to green.
      await pool.query(`UPDATE grv_return_requests SET status='Downloaded', updated_at=NOW() WHERE id=$1`, [req.params.id]);
    } catch (e) {
      if (!res.headersSent) res.status(500).json({ error: e.message });
    }
  });
  ...
  app.get('/api/grv/requests/export-all', requireAuth, requireGrvRole('grv_cs'), async function (req, res) {
    try {
      ...
      res.send(zipBuf);
      // Mark every request included in this batch as Downloaded/Closed too.
      const ids = rows.map(r => r.id);
      await pool.query(`UPDATE grv_return_requests SET status='Downloaded', updated_at=NOW() WHERE id = ANY($1::int[])`, [ids]);
    } catch (e) {
      if (!res.headersSent) res.status(500).json({ error: e.message });
    }
  });
