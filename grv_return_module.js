...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.post('/api/grv/master/items/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      for (const row of rows) {
        ...
        if (existing.rows.length) {
          await pool.query(
            `UPDATE grv_items SET description=$1, brand=$2, category=$3, uom=$4, price_per_unit=$5, updated_at=NOW() WHERE id=$6`,
            [description, brand, category, uom, price, existing.rows[0].id]
          );
          updated++;
        } else {
          await pool.query(
            `INSERT INTO grv_items (item_code, description, brand, category, uom, price_per_unit) VALUES ($1,$2,$3,$4,$5,$6)`,
            [itemCode, description, brand, category, uom, price]
          );
          inserted++;
        }
      }
    ...
    try {
      const r = await pool.query(
        `SELECT item_code AS "Item Code", description AS "Description", brand AS "Brand", category AS "Category",
                uom AS "UOM", price_per_unit AS "Price"
  ...
  // Item search for the merchandiser's line-item picker (code or description).
  app.get('/api/grv/items', requireAuth, async function (req, res) {
    try {
      const q = (req.query.q || '').trim();
      if (!q) return res.json({ items: [] });
      const r = await pool.query(
        `SELECT item_code, description, brand, category, uom, price_per_unit FROM grv_items
         WHERE active=true AND (item_code ILIKE $1 OR description ILIKE $1) ORDER BY item_code LIMIT 20`,
        [`%${q}%`]
      );
