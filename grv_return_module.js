  ...
  app.post('/api/grv/master/customers/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      for (const row of rows) {
        ...
        const siteUseIdRaw = String(row['Site Use Id'] || row['Site Use ID'] || '').trim();
        ...
        const siteUseId = siteUseIdRaw || (location ? 'LOC_' + String(location).trim().toUpperCase().replace(/[^A-Z0-9]+/g, '_') : `ROW_${inserted + updated + skipped + 1}`);
    ...
    try {
      ...
      const r = await pool.query(
        ...
        [ref, account_number || null, customer_name, site_use_id || null, location || null, salesperson_name,
         warehouse || 'DCF', requester_email.trim().toLowerCase(), req.user.username, paper_return_number.trim()]
      );
