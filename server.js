...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    ...
    mt.columns = [{ width: 10 }, { width: 16 }, { width: 10 }, { width: 16 }, { width: 10 }, { width: 16 }, { width: 10 }, { width: 12 }];
    var mtTitle = mt.addRow(['MONTH-BY-MONTH RETURNS — FOOD vs NON-FOOD']); styleTitleRow(mtTitle); mt.mergeCells('A1:H1');
    mt.addRow([]);
    styleSectionRow(mt.addRow(['VALUE (AED) AND % SHARE BY MONTH', '', '', '', '', '', '', '']), 8);
    var mtHdr = mt.addRow(['Month', 'Food (AED)', 'Food %', 'Non-Food (AED)', 'Non-Food %', 'Total (AED)', 'Orders', 'MoM Change']);
    styleHeaderRow(mtHdr, 8);
