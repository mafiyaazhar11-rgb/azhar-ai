...
app.post('/api/dispatch/reconcile/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
  try {
    ...
    // line to change.
    var TASK_STATUS_RULE = 'COMPLETED';

    var byDateClass = {}; // "date|class" -> Set of drop_ids
    var skippedNonCompleted = 0, totalRows = 0;
    rawRows.forEach(function (raw) {
      ...
      if (status !== TASK_STATUS_RULE) { skippedNonCompleted++; return; }
    ...
    for (var key in byDateClass) {
      ...
      await pool.query(
        `INSERT INTO transport_drop_reconciliation (date_key, drop_class, reported_drops, task_status_rule, upload_batch_id, uploaded_by)
         VALUES ($1,$2,$3,$4,$5,$6)
         ON CONFLICT (date_key, drop_class) DO UPDATE SET reported_drops=$3, task_status_rule=$4, upload_batch_id=$5, uploaded_by=$6, uploaded_at=NOW()`,
        [dateStr, cls, count, TASK_STATUS_RULE, batchId, req.user ? req.user.username : 'Admin']
      );
      upserted++;
    }
    ...
    res.json({
      ...
      task_status_rule: TASK_STATUS_RULE,
      skipped_non_completed: skippedNonCompleted,
...
app.get('/api/dispatch/reconcile', requireAuth, async function (req, res) {
  try {
    ...
    var transportRows = await pool.query(
      `SELECT date_key::text, drop_class, reported_drops, task_status_rule, uploaded_at FROM transport_drop_reconciliation ${where} ORDER BY date_key`, params
    );
    if (!transportRows.rows.length) return res.json({ has_data: false, days: [] });

    var byDate = {};
    transportRows.rows.forEach(function (r) {
      if (!byDate[r.date_key]) byDate[r.date_key] = { date: r.date_key, transport_total: 0, by_class: {}, task_status_rule: r.task_status_rule, uploaded_at: r.uploaded_at };
    ...
    var days = dateKeys.map(function (d) {
      ...
      return {
        ...
        task_status_rule: t.task_status_rule,
        variance: variance,
...
app.get('/api/dispatch/reconcile/export', requireAuth, async function (req, res) {
  try {
    ...
    if (req.query.date_to) { params.push(req.query.date_to); clauses.push('date_key <= $' + params.length); }
    var where = clauses.length ? ('WHERE ' + clauses.join(' AND ')) : '';
    var transportRows = await pool.query(`SELECT date_key::text, drop_class, reported_drops, task_status_rule FROM transport_drop_reconciliation ${where} ORDER BY date_key`, params);
    var byDate = {};
    transportRows.rows.forEach(function (r) {
      if (!byDate[r.date_key]) byDate[r.date_key] = { total: 0, classes: {} };
    ...
    ws.columns = [{ width: 14 }, { width: 16 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 12 }];
    var titleRow = ws.addRow(['TRANSPORT DROP RECONCILIATION — App Captured vs Transport Reported (COMPLETED drops only)']);
    ws.mergeCells('A' + titleRow.number + ':G' + titleRow.number);
    titleRow.font = { bold: true, size: 14 };
    ws.addRow(['Downloaded', new Date().toLocaleString('en-AE')]);
    ws.addRow([]);
    var hdr = ws.addRow(['Date', 'App Captured', 'Transport Bulk', 'Transport Multi', 'Transport Total', 'Variance', 'Variance %']);
    hdr.font = { bold: true };
    var totA = 0, totT = 0;
    dateKeys.forEach(function (d) {
      ...
      totA += appDrops || 0; totT += t.total;
      ws.addRow([d, appDrops === null ? 'No app data' : appDrops, t.classes.Bulk || 0, t.classes.Multi || 0, t.total, variance === null ? '' : variance, variancePct === null ? '' : variancePct]);
    });
    var totalRow = ws.addRow(['TOTAL', totA, '', '', totT, totA - totT, '']);
