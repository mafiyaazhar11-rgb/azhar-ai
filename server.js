...
// sometimes shows up in dispatch files instead of a driver's name. Primary source
// is the General Info roster you already upload (Unit/Acct No./Name columns) — no
// separate upload needed. driver_master is kept only as a small manual-override
// table for a code General Info doesn't have (e.g. a driver not yet in that file).
// ============================================================
...
// Lazily built from genInfoData (General Info roster) — rebuilt only when
// genInfoData has actually changed (re-uploaded), not on every lookup.
var _giAcctNoMapCache = { forData: null, map: {} };
function getGiAcctNoMap() {
  if (_giAcctNoMapCache.forData === genInfoData) return _giAcctNoMapCache.map;
  var m = {};
  if (genInfoData && genInfoData.rows) {
    genInfoData.rows.forEach(function (row) {
      var acctNo = '';
      var name = '';
      Object.keys(row).forEach(function (k) {
        var kU = k.trim().toUpperCase();
        if (kU === 'ACCT_NO' || kU === 'ACCT NO' || kU === 'ACCT NO.' || kU === 'ACCOUNT NO' || kU === 'ACCOUNT NO.' || kU === 'EMPLOYEE ID' || kU === 'EMPLOYEE CODE') acctNo = String(row[k] || '').trim();
        if (kU === 'NAME') name = String(row[k] || '').trim();
      });
      if (acctNo && name) m[acctNo.toUpperCase()] = name;
    });
  }
  _giAcctNoMapCache = { forData: genInfoData, map: m };
  return m;
}
// Resolves a raw driver identifier (name OR bare code) to its real name — checks
// the General Info roster first (already-uploaded staff list), then the manual
// driver_master override, then falls back to the raw value unchanged.
function resolveDriverName(raw) {
  var key = String(raw || '').trim().toUpperCase();
  var giMap = getGiAcctNoMap();
  if (giMap[key]) return giMap[key];
...
app.get('/api/driver-master/status', requireAuth, async function (req, res) {
  try {
    var giCount = Object.keys(getGiAcctNoMap()).length;
    var r = await pool.query('CREATE TABLE IF NOT EXISTS driver_master (driver_code TEXT PRIMARY KEY, driver_name TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())').then(function(){
      return pool.query('SELECT COUNT(*)::int AS cnt FROM driver_master');
    });
    res.json({ general_info_codes: giCount, manual_override_codes: r.rows[0].cnt, has_general_info: !!(genInfoData && genInfoData.rows && genInfoData.rows.length) });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
async function buildDriverRejectionTracker(dateFrom, dateTo) {
  ...
  // order_code -> which driver ran it, and on what date (last one wins if an order code
  // is ever re-dispatched under a different driver on a later date within the range).
  var orderCodeToDriver = {};
  // driver name -> running totals across every date in range. Keyed by the RESOLVED
  // name (via driver_master) so a bare employee code like "E111640" and that same
  // person's real name — if they ever appear separately across different uploads —
  // get clubbed together as one driver, not counted as two.
  var driverStats = {};
  dispR.rows.forEach(function (row) {
    ...
    [['inhouse_driver_details', false], ['hired_driver_details', true]].forEach(function (pair) {
      ...
      (dss[key] || []).forEach(function (drv) {
        ...
        if (!driverStats[resolvedName]) driverStats[resolvedName] = { isHired: isHired, orders: 0, drops: 0, dates: {} };
        driverStats[resolvedName].orders += drv.orders || 0;
        driverStats[resolvedName].drops += drv.drops || 0;
        driverStats[resolvedName].dates[row.date_key] = true;
        (drv.order_details || []).forEach(function (o) {
          if (o.order_code) orderCodeToDriver[o.order_code] = { driver: resolvedName, isHired: isHired, date: row.date_key };
        });
      });
    });
  });
  ...
  var matchedCount = 0;
  var driverRejections = {}; // driver name -> { count, sameDayReturns }
  var orderCodesInPeriod = rejR.rows.map(function (r) { return r.order_no; });
  var sameDayCounts = {};
  if (orderCodesInPeriod.length) {
    var sameDayR = await pool.query(
      `SELECT order_code, date_key::text AS date_key, COUNT(*) AS cnt FROM order_tracking
       WHERE order_code = ANY($1) GROUP BY order_code, date_key`,
      [orderCodesInPeriod]);
    sameDayR.rows.forEach(function (t) { sameDayCounts[t.order_code + '|' + t.date_key] = +t.cnt; });
  }
  rejR.rows.forEach(function (rej) {
    var match = orderCodeToDriver[rej.order_no];
    if (!match) return;
    matchedCount++;
    if (!driverRejections[match.driver]) driverRejections[match.driver] = { count: 0, sameDayReturns: 0 };
    driverRejections[match.driver].count++;
    // Same-day return: the rejection report for a given date describes the PREVIOUS
    // day's dispatch (today's dispatch report and tomorrow's rejection report cover
    // the same batch of orders) — so "same day" as the actual dispatch means checking
    // order_tracking on (entry_date - 1 day), not on entry_date itself.
    ...
    if ((sameDayCounts[rej.order_no + '|' + dispatchDateKey] || 0) > 1) driverRejections[match.driver].sameDayReturns++;
  });
  var inhouseRows = [], inhouseAgg = { orders: 0, drops: 0, rejections: 0, same_day_returns: 0, driver_count: 0 };
  var hiredAgg = { orders: 0, drops: 0, rejections: 0, same_day_returns: 0, driver_count: 0 };
  Object.keys(driverStats).forEach(function (name) {
    var stat = driverStats[name];
    var rej = driverRejections[name] || { count: 0, sameDayReturns: 0 };
    var agg = stat.isHired ? hiredAgg : inhouseAgg;
    agg.orders += stat.orders; agg.drops += stat.drops; agg.rejections += rej.count;
    agg.same_day_returns += rej.sameDayReturns; agg.driver_count++;
    if (!stat.isHired) {
      var daysActive = Object.keys(stat.dates).length;
      var rejPct = stat.orders ? +((rej.count / stat.orders) * 100).toFixed(2) : 0;
      inhouseRows.push({
        driver: name, orders: stat.orders, drops: stat.drops, days_active: daysActive,
...
app.get('/api/dispatch/driver-rejection-tracker/export', noCache, requireAuth, async function (req, res) {
  try {
    ...
    ws.columns = [{ width: 6 }, { width: 26 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 16 }, { width: 14 }];
    var hdr = ws.addRow(['#', 'Driver', 'Orders', 'Rejections', 'Rejection Rate %', 'Delivered %', 'Same-Day Returns', 'Avg Drops/Day']);
    hdr.font = { bold: true };
    (d.inhouse_drivers || []).forEach(function (row, i) {
      var r = ws.addRow([i + 1, row.driver, row.orders, row.rejections, row.rejection_rate_pct, row.delivered_pct, row.same_day_returns, row.avg_drops_per_day]);
