  ...
  try {
    ...
    var result = await pool.query(
      `SELECT customer_name,
       ...
       FROM rejection_rows
       WHERE customer_name IS NOT NULL AND customer_name <> '' AND customer_name ILIKE $1
       GROUP BY customer_name
       ORDER BY rejected_count DESC, customer_name ASC
       LIMIT 12`,
      ['%' + q + '%']
    );
    res.json({ customers: result.rows.map(function(r) {
...
async function buildCustomerBreakdown(name) {
  var result = await pool.query(
  ...
  var acc = {};
  BUCKETS.forEach(function(b) { acc[b] = { rejected: 0, valueAtRisk: 0, foodCount: 0, nonfoodCount: 0, byOrg: {}, byMonth: {}, byMonthOrg: {}, byRoot: {}, byRootOrg: {}, byOrgRoot: {}, byOrgArea: {}, recent: [] }; });
  ...
  rows.forEach(function(r) {
    ...
    var area = r.area || r.address || 'Unknown location';
    var targets = ['all'];
    ...
    targets.forEach(function(b) {
      ...
      if (a.recent.length < 2000) {
        a.recent.push({ date: r.entry_date, order_no: r.order_no, org: org, root_cause: root, value: val, area: r.area, address: r.address, source: srcU || null });
      }
    });
  });
  ...
  function finalizeBucket(b) {
    ...
    var orgList = Object.keys(orgKeys).map(function(k) {
      ...
      var areasForOrg = Object.keys(a.byOrgArea[k] || {}).map(function(ak) {
        ...
        return { area: ak, count: av.count, pct: o.rejected ? (av.count / o.rejected * 100) : 0, value: av.value };
      }).sort(function(x, y) { return y.count - x.count; });
    ...
    // breakdown is shown (known codes first, anything else after).
    var knownOrgOrder = ['DCV', 'DCF', 'DGC', 'DGS', 'DSN'];
    var orgColumnOrder = knownOrgOrder.filter(function(o) { return orgKeys[o]; })
      .concat(Object.keys(orgKeys).filter(function(o) { return knownOrgOrder.indexOf(o) < 0; }).sort());
...
app.get('/api/rejection/customer-detail', requireAuth, async function(req, res) {
  try {
    ...
    var breakdown = await buildCustomerBreakdown(name);
    res.json(breakdown);
  } catch (e) {
...
// lives here instead. Respects the same all/external/internal view the
// user had selected on screen (?source=). ──
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var d = await buildCustomerBreakdown(name);
    if (!d.hasData) return res.status(404).json({ error: 'No rejection rows on file for this customer' });
    ...
    // ---- Sheet 1: Overview ----
    var ov = wb.addWorksheet('Overview');
    ov.columns = [{ width: 30 }, { width: 26 }, { width: 18 }, { width: 18 }];
    var titleRow = ov.addRow(['CUSTOMER VISIT BRIEF — ' + d.customer]);
    titleRow.font = { bold: true, size: 15, color: { argb: GOLD } };
    ov.mergeCells('A1:D1');
    ov.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } };
    var genRow = ov.addRow(['Generated', null]);
    ...
    ov.addRow(['View', srcLabel]);
    if (src !== 'all') {
      var scopeNoteRow = ov.addRow(['This export contains ' + srcLabel + ' rejections ONLY — no Internal figures are included below.']);
      scopeNoteRow.font = { italic: true, color: { argb: 'FFB00000' } };
      ov.mergeCells('A' + scopeNoteRow.number + ':D' + scopeNoteRow.number);
    }
    ov.addRow([]);
    styleSectionRow(ov.addRow(['KEY METRICS']));
    ov.addRow(['Total Orders (all sources)', d.totalOrders]);
    ov.addRow(['Rejected (' + srcLabel + ')', b.rejected]);
    ov.addRow(['Rejection Rate', b.rejectionRate + '%']);
    ov.addRow(['Delivered', d.delivered]);
    ov.addRow(['Value At Risk', 'AED ' + b.valueAtRisk.toLocaleString()]);
    ov.addRow([]);
    if (src === 'all') {
      styleSectionRow(ov.addRow(['INTERNAL vs EXTERNAL (overall)']));
      ov.addRow(['External (customer-side)', d.sourceSplit.external]);
      ov.addRow(['Internal (AKI-side)', d.sourceSplit.internal]);
      ov.addRow([]);
    }
    styleSectionRow(ov.addRow(['FOOD vs NON-FOOD (' + srcLabel + ')']));
    ov.addRow(['Food', b.foodSplit.food]);
    ov.addRow(['Non-Food', b.foodSplit.nonfood]);

    // ---- Sheet 2: Branch-Wise (ORG-level — these are AKI's business-unit
    // codes, e.g. DCV/DCF/DGC/DGS/DSN, not separate physical branches).
    // Below the summary table, each branch that actually has rejections
    // gets its own detail block (highest-rejection branch first) listing
    // its root causes with counts and plain-English explanations, so the
    // full "why" for any branch is on this one sheet. ----
    var bw = wb.addWorksheet('Branch-Wise');
    bw.columns = [{ width: 20 }, { width: 12 }, { width: 12 }, { width: 20 }, { width: 55 }];
    styleHeaderRow(bw.addRow(['ORG', 'Rejected', 'Delivered', 'Value At Risk (AED)']));
    var bwOrgsWithRejections = b.byOrg.filter(function(o) { return o.rejected > 0; }); // ignore orgs with zero rejections (e.g. UNKNOWN/TPW/TPN) — already sorted highest rejection first
    bwOrgsWithRejections.forEach(function(o) { bw.addRow([o.org, o.rejected, o.delivered, Math.round(o.value || 0)]); });
    var bwTotal = bw.addRow(['TOTAL', bwOrgsWithRejections.reduce(function(s, o) { return s + o.rejected; }, 0), bwOrgsWithRejections.reduce(function(s, o) { return s + o.delivered; }, 0), Math.round(bwOrgsWithRejections.reduce(function(s, o) { return s + (o.value || 0); }, 0))]);
    styleTotalRow(bwTotal);
    bw.addRow([]);
    bw.addRow([]);
    if (bwOrgsWithRejections.length) {
      var bwDetailTitle = bw.addRow(['BRANCH DETAIL — highest rejection branch first']);
      bwDetailTitle.font = { bold: true, size: 12, color: { argb: DARKBG } };
      bw.mergeCells('A' + bwDetailTitle.number + ':E' + bwDetailTitle.number);
      bwOrgsWithRejections.forEach(function(o) {
        bw.addRow([]);
        var hdr = bw.addRow([o.org + ' — ' + o.rejected + ' rejected — AED ' + Math.round(o.value || 0).toLocaleString() + ' at risk']);
        hdr.font = { bold: true, color: { argb: GOLD } };
        hdr.eachCell(function(cell) { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } }; });
        bw.mergeCells('A' + hdr.number + ':E' + hdr.number);
        styleHeaderRow(bw.addRow(['Root Cause', 'Count', '% of Branch', '', 'What This Means (for the customer)']));
        (o.rootCauses || []).forEach(function(r) {
          bw.addRow([r.root_cause, r.count, r.pct.toFixed(1) + '%', '', r.explanation]);
        });
        // Real branch/outlet detail — this is the actual physical location
        // the rejections happened at, not just the ORG business-unit code.
        if ((o.areas || []).length) {
          bw.addRow([]);
          var areaHdr = bw.addRow(['By Area / Outlet (' + o.org + ')']);
          areaHdr.font = { bold: true, italic: true, color: { argb: DARKBG } };
          bw.mergeCells('A' + areaHdr.number + ':E' + areaHdr.number);
          styleHeaderRow(bw.addRow(['Area / Outlet', 'Count', '% of Branch', 'Value At Risk (AED)', '']));
          o.areas.forEach(function(ar) {
            bw.addRow([ar.area, ar.count, ar.pct.toFixed(1) + '%', Math.round(ar.value || 0), '']);
          });
        }
      });
    }

    // ---- Sheet 3: Month-Wise (zero-rejection months already dropped by
    // buildCustomerBreakdown; one column per ORG so the branch split is
    // visible right alongside the month trend, not just in Sheet 2) ----
    var mw = wb.addWorksheet('Month-Wise');
    var orgCols = b.orgColumnOrder || [];
    mw.columns = [{ width: 14 }, { width: 12 }, { width: 12 }, { width: 18 }].concat(orgCols.map(function() { return { width: 12 }; }));
    styleHeaderRow(mw.addRow(['Month', 'Rejected', 'Delivered', 'Value At Risk (AED)'].concat(orgCols.map(function(o) { return o + ' (Rej)'; }))));
    if (!b.byMonth.length) {
      mw.addRow(['No months with rejections in this view']);
    } else {
      b.byMonth.forEach(function(m) {
        mw.addRow([m.month, m.rejected, m.delivered, Math.round(m.value || 0)].concat(orgCols.map(function(o) { return (m.byOrg && m.byOrg[o]) || 0; })));
      });
    }

    // ---- Sheet 4: Root Causes — includes a plain-English explanation and
    // which branch(es) it happened at, so a rep can answer "which branch
    // had the stock count issue?" straight from this sheet. ----
    var rc = wb.addWorksheet('Root Causes');
    rc.columns = [{ width: 40 }, { width: 10 }, { width: 12 }, { width: 22 }, { width: 55 }];
    styleHeaderRow(rc.addRow(['Root Cause', 'Count', '% of Total', 'Branch(es) (ORG)', 'What This Means (for the customer)']));
    b.rootCauses.forEach(function(r) { rc.addRow([r.root_cause, r.count, r.pct.toFixed(1) + '%', r.topBranches, r.explanation]); });
    var rcTotal = rc.addRow(['TOTAL', b.rootCauses.reduce(function(s, r) { return s + r.count; }, 0), '100.0%', '', '']);
    styleTotalRow(rcTotal);

    // ---- Sheet 5: Order Master — the full list of every rejected order
    // in the selected view (not just a recent sample), with ORG, area, and
    // the customer's own branch/outlet address so any order can be traced
    // back to exactly where it happened. ----
    var ro = wb.addWorksheet('Order Master');
    ro.columns = [{ width: 12 }, { width: 16 }, { width: 8 }, { width: 14 }, { width: 32 }, { width: 34 }, { width: 42 }, { width: 10 }, { width: 14 }];
    var roTitle = ro.addRow(['ORDER MASTER DATA — ' + d.customer + ' — ' + srcLabel + ' (' + b.recentRejections.length + ' orders)']);
    roTitle.font = { bold: true, size: 13, color: { argb: GOLD } };
    ro.mergeCells('A1:I1');
    ro.getRow(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } };
    styleHeaderRow(ro.addRow(['Date', 'Order No', 'ORG', 'Area', 'Branch / Outlet (Address)', 'Root Cause', 'What This Means', 'Source', 'Value (AED)']));
    b.recentRejections.forEach(function(r) {
      var dt = r.date ? new Date(r.date) : null;
      ro.addRow([dt, r.order_no, r.org, r.area || '—', r.address || '—', r.root_cause, humanizeRootCause(r.root_cause), r.source || '—', Math.round(r.value || 0)]);
      if (dt) ro.getRow(ro.rowCount).getCell(1).numFmt = 'dd/mm/yyyy';
    });
    if (b.recentRejections.length) {
      var roTotal = ro.addRow(['', '', '', '', '', '', '', 'TOTAL', Math.round(b.recentRejections.reduce(function(s, r) { return s + (r.value || 0); }, 0))]);
      styleTotalRow(roTotal);
    }

    var fileName = 'Customer_Visit_' + name.replace(/[^a-z0-9]+/gi, '_').slice(0, 40) + '_' + src + '.xlsx';
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
