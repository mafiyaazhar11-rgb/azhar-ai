...
function parseDispatchFromRows(rows) {
  ...
  // Transport Cost Reconciliation only ever compares against the 5 business-unit org
  // codes that the transport team's own file could ever contain (Food = DCV+DCF,
  // Non-Food = DGC+DGS+DSN). 3PL/HCP drops are fulfilled by a third party — our own
  // transport team never delivers them — and any other org code (DPS, DPB, or anything
  // else present in a mixed dispatch file) is likewise outside what transport's file
  // covers. Counting those toward "App Captured" would always show as a false variance/
  // "missing from transport file", since transport's file structurally can't contain
  // them. This is a SEPARATE counter from total_drops below (which intentionally stays
  // all-inclusive for the Executive Summary / cost-estimate features) so nothing else
  // on the dashboard changes.
  var RECON_ELIGIBLE_ORGS = { DCV: true, DCF: true, DGC: true, DGS: true, DSN: true };
  ...
  for (var i = 0; i < rows.length; i++) {
    ...
    if (C.route && row[C.route] && !isCostExcludedRow(row)) {
      ...
      if (loc) {
        ...
        // Reconciliation-scoped count: only for the 5 org codes transport's own file could
        // ever contain (see RECON_ELIGIBLE_ORGS above). Excludes 3PL/HCP and any other org.
        if (RECON_ELIGIBLE_ORGS[org]) {
  ...
  try {
    ...
    for (var i = 0; i < orderRows.length; i += CHUNK) {
      ...
      chunk.forEach(function(r) {
        phs.push('($' + idx + ',$' + (idx+1) + ',$' + (idx+2) + ',$' + (idx+3) + ',$' + (idx+4) + ',$' + (idx+5) + ',$' + (idx+6) + ',$' + (idx+7) + ',$' + (idx+8) + ')');
        vals.push(r.order_code, dateKey, r.customer, r.value, r.route, r.org, r.drop_type, r.temperature, r.source_label || '');
        idx += 9;
      });
      await pool.query('INSERT INTO order_tracking (order_code, date_key, customer, value, route, org, drop_type, temperature, source_label) VALUES ' + phs.join(','), vals);
    }
    return true;
  } catch(e) {
...
// The transport team's own file only ever covers these 5 business-unit org codes
// (Food = DCV+DCF, Non-Food = DGC+DGS+DSN). 3PL/HCP orders are delivered by a third
// party — our own transport team never touches them — and any other org code is
// likewise outside what transport's file could contain. Every order-level "missing
// from transport file" comparison below is scoped to this list so those orders never
// show up as a false mismatch.
var RECON_ELIGIBLE_ORG_LIST = ['DCV', 'DCF', 'DGC', 'DGS', 'DSN'];
...
app.get('/api/dispatch/reconcile', noCache, requireAuth, async function (req, res) {
  try {
    ...
    var totalTransportAllDays = days.reduce(function (s, d) { return s + d.transport_reported; }, 0);
    ...
    var totalTransportMatched = days.reduce(function (s, d) { return s + (d.app_has_data ? d.transport_reported : 0); }, 0);
    var daysMatched = days.filter(function (d) { return d.app_has_data; }).length;
...
app.get('/api/dispatch/reconcile/order-diff', noCache, requireAuth, async function (req, res) {
  try {
    ...
    var appRows = await pool.query("SELECT DISTINCT order_code FROM order_tracking WHERE date_key=$1 AND org = ANY($2::text[]) AND (source_label IS DISTINCT FROM 'Salon')", [date, RECON_ELIGIBLE_ORG_LIST]);
...
app.get('/api/dispatch/reconcile/export', noCache, requireAuth, async function (req, res) {
  try {
    ...
    for (var i = 0; i < orderIdRows.rows.length; i++) {
      ...
      var appOrderRows = await pool.query("SELECT DISTINCT order_code FROM order_tracking WHERE date_key=$1 AND org = ANY($2::text[]) AND (source_label IS DISTINCT FROM 'Salon')", [dk, RECON_ELIGIBLE_ORG_LIST]);
