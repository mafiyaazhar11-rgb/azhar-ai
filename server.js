...
async function buildCustomerBreakdown(name) {
  ...
  var acc = {};
  BUCKETS.forEach(function(b) { acc[b] = { rejected: 0, valueAtRisk: 0, foodCount: 0, nonfoodCount: 0, byOrg: {}, byMonth: {}, byRoot: {}, recent: [] }; });
  ...
  function finalizeBucket(b) {
    ...
    var monthList = Object.keys(moKeys).map(function(k) {
      ...
      return { month: k, rejected: m.rejected, delivered: deliveredByMonth[k] || 0, value: m.value };
    }).sort(function(x, y) { return x.month < y.month ? -1 : (x.month > y.month ? 1 : 0); });

    var rootList = Object.keys(a.byRoot).map(function(k) {
      return { root_cause: k, count: a.byRoot[k], pct: a.rejected ? (a.byRoot[k] / a.rejected * 100) : 0 };
...
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    styleHeaderRow(bw.addRow(['Branch', 'Rejected', 'Delivered', 'Value At Risk (AED)']));
    ...
    mw.columns = [{ width: 16 }, { width: 16 }, { width: 16 }, { width: 20 }];
    styleHeaderRow(mw.addRow(['Month', 'Rejected', 'Delivered', 'Value At Risk (AED)']));
    b.byMonth.forEach(function(m) { mw.addRow([m.month, m.rejected, m.delivered, Math.round(m.value || 0)]); });

    // ---- Sheet 4: Root Causes ----
    var rc = wb.addWorksheet('Root Causes');
    rc.columns = [{ width: 44 }, { width: 14 }, { width: 14 }];
    styleHeaderRow(rc.addRow(['Root Cause', 'Count', '% of Total']));
    b.rootCauses.forEach(function(r) { rc.addRow([r.root_cause, r.count, r.pct.toFixed(1) + '%']); });

    // ---- Sheet 5: Recent Orders ----
    var ro = wb.addWorksheet('Recent Orders');
    ro.columns = [{ width: 14 }, { width: 18 }, { width: 10 }, { width: 38 }, { width: 12 }, { width: 16 }];
    styleHeaderRow(ro.addRow(['Date', 'Order No', 'Branch', 'Root Cause', 'Source', 'Value (AED)']));
    b.recentRejections.forEach(function(r) {
      var dt = r.date ? new Date(r.date) : null;
      ro.addRow([dt, r.order_no, r.org, r.root_cause, r.source || '—', Math.round(r.value || 0)]);
