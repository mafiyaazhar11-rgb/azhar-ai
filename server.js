...
async function initDB() {
  try {
    ...
    // ── Sub-admin upload guardrails: 10-hour cooldown between uploads per
    // (user, endpoint). Superadmin is never gated by this table. ──
      ...
      try {
        ...
        if (blCooldown) {
          return res.status(429).json({
            error: '\u23f3 Sub-admins can upload once every 10 hours \u2014 make sure your data is correct before uploading, you cannot re-upload for ' + blCooldown.hoursLeft + 'h ' + blCooldown.minsLeft + 'm.',
...
async function checkSubadminCooldown(userId, endpoint) {
  // Returns null if clear to upload, or {hoursLeft, minsLeft} if still cooling down.
  var r = await pool.query(`SELECT last_upload_at FROM upload_cooldowns WHERE user_id=$1 AND endpoint=$2`, [userId, endpoint]);
  if (!r.rows[0]) return null;
  var hoursSince = (Date.now() - new Date(r.rows[0].last_upload_at).getTime()) / 3600000;
  if (hoursSince >= 10) return null;
...
async function recordSubadminUpload(userId, endpoint) {
  await pool.query(
    `INSERT INTO upload_cooldowns (user_id, endpoint, last_upload_at) VALUES ($1,$2,NOW())
     ON CONFLICT (user_id, endpoint) DO UPDATE SET last_upload_at=NOW()`,
    [userId, endpoint]
  );
}
  ...
  try {
    ...
    if (req.user.role === 'subadmin') {
      try {
        ...
        if (dsCooldown) {
          return res.status(429).json({
            error: '\u23f3 Sub-admins can upload once every 10 hours \u2014 make sure your data is correct before uploading, you cannot re-upload for ' + dsCooldown.hoursLeft + 'h ' + dsCooldown.minsLeft + 'm.',
...
app.get('/api/dispatch/fleet-summary/export', noCache, requireAuth, async function (req, res) {
  try {
    ...
    ws.columns = [{ width: 14 }, { width: 16 }, { width: 14 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 16 }];
    ...
    ws.addRow([]);
    var hdr = ws.addRow(['Date', 'In-House Drivers', 'Hired Drivers', '% Hired', 'Total Orders', 'Total Value (AED)', 'Hired Value (AED)']);
    hdr.font = { bold: true };
    var totInhouse = 0, totHired = 0, totOrders = 0, totValue = 0, totHiredValue = 0;
    rows.forEach(function (r) {
      var row = ws.addRow([r.date, r.inhouse_drivers, r.hired_drivers, r.hired_pct, r.total_orders, r.total_value, r.hired_value]);
      row.getCell(4).numFmt = '0.0"%"';
      row.getCell(6).numFmt = '#,##0';
      row.getCell(7).numFmt = '#,##0';
      if (r.hired_pct >= 25) row.getCell(4).font = { color: { argb: 'FFE05C5C' }, bold: true };
      totInhouse += r.inhouse_drivers; totHired += r.hired_drivers; totOrders += r.total_orders; totValue += r.total_value; totHiredValue += r.hired_value;
    });
    ws.addRow([]);
    var avgPct = rows.length ? +((rows.reduce(function(s,r){return s+r.hired_pct;},0) / rows.length).toFixed(1)) : 0;
    var totalRow = ws.addRow(['TOTAL / AVERAGE (' + rows.length + ' days)', totInhouse, totHired, avgPct, totOrders, totValue, totHiredValue]);
    totalRow.font = { bold: true };
  ...
  try {
    ...
    if (req.user.role === 'subadmin') {
      try {
        ...
        if (cooldown) {
          return res.status(429).json({
            error: '\u23f3 Sub-admins can upload once every 10 hours \u2014 make sure your data is correct before uploading, you cannot re-upload for ' + cooldown.hoursLeft + 'h ' + cooldown.minsLeft + 'm.',
