...
app.get('/api/price-discrepancy/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var groupKey = toStr(req.query.group || '').trim().toLowerCase();
    if (!name && !groupKey) return res.status(400).json({ error: 'Customer name is required' });
    var onlyIssues = toStr(req.query.onlyIssues || 'true').toLowerCase() !== 'false';
    ...
    var exportItems = onlyIssues ? d.items.filter(function(it) { return it.discrepancy < 0; }) : d.items;
    ...
    ws.addRow([]);
    var filterRow = ws.addRow([onlyIssues
      ? 'Filter applied: showing price shortfalls only (system price higher than LPO) \u2014 ' + exportItems.length + ' of ' + fullCount + ' total rows'
      : 'Filter applied: none \u2014 showing all ' + fullCount + ' rows, including where the customer\u2019s LPO price is already higher than system price']);
    ...
    var fileName = 'Price_Discrepancy_' + d.customer.replace(/[^a-z0-9]+/gi, '_').slice(0, 40) + (onlyIssues ? '_issues-only' : '_all') + '.xlsx';
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename="' + fileName + '"');
    await wb.xlsx.write(res);
    res.end();
