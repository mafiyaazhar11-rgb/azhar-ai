    ...
    if (req.user.role === 'subadmin') {
      ...
      try {
        ...
        if (dsExisting.rows.length) {
          var dsApprovalReason = toStr(req.body.approvalReason);
          if (req.body.requestApproval === 'true' && dsApprovalReason) {
            await submitUploadApproval({
              userId: req.user.uid, username: req.user.username || req.user.full_name || 'Sub-admin',
              endpoint: 'dispatch', fileName: req.file.originalname, fileBuffer: req.file.buffer,
              blockedDates: [dateKey], reason: dsApprovalReason, meta: { dateKey: dateKey, sourceLabel: sourceLabel, uploadedBy: uploadedBy }
            });
            await recordSubadminUpload(req.user.uid, 'dispatch');
            auditLog(null, req.user.username, 'UPLOAD_APPROVAL_REQUESTED', 'Daily Dispatch (' + sourceLabel + ', ' + dateKey + '): ' + req.file.originalname + ' \u2014 reason: ' + dsApprovalReason, req.headers['x-forwarded-for'] || req.ip || '');
            return res.status(202).json({ success: false, submittedForApproval: true, message: 'Sent to the super admin for approval \u2014 you\u2019ll see it reflected once reviewed.' });
          }
          return res.status(409).json({
            error: '\u26d4 ' + sourceLabel + ' data for ' + dateKey + ' is already on the dashboard. Sub-admins can\u2019t overwrite an existing date/source directly \u2014 pick a new date, or send this to the super admin for approval.',
            cutoffDate: dateKey, subadminDateBlock: true, requiresApprovalReason: true
          });
        }
...
// ============================================================
// DRIVER ORDER DETAILS — order-by-order breakdown per driver (in-house + hired),
// across a date range: which customer, each order's value, and the final drop for
// that order. Built on top of driver_source_split.hired_driver_details /
// inhouse_driver_details, which already carry each driver's order_details array
// (added alongside the driver aggregation above) — nothing new to store per upload.
// ============================================================
async function getDriverDetailDays(dateFrom, dateTo) {
  var params = [], clauses = [];
  if (dateFrom) { params.push(dateFrom); clauses.push('date_key >= $' + params.length); }
  if (dateTo) { params.push(dateTo); clauses.push('date_key <= $' + params.length); }
  var where = clauses.length ? ('WHERE ' + clauses.join(' AND ')) : '';
  ...
  return r.rows.map(function(row) {
    var s = row.summary || {};
    var dss = s.driver_source_split || {};
    return { date: row.date_key, hired: dss.hired_driver_details || [], inhouse: dss.inhouse_driver_details || [] };
  });
}
// Flattens the per-day/per-driver structure into (a) one row per order, and (b) one
// row per driver aggregated across the whole range — the two views the export and
// the AI advice both need.
function buildDriverDetailReport(days) {
  var orderRows = [];
  var driverAgg = {}; // name -> { type, days:Set, orders, drops:Set (per day, so use date::drop), value }
  days.forEach(function(day) {
    ['inhouse', 'hired'].forEach(function(kind) {
      (day[kind] || []).forEach(function(drv) {
        var key = drv.name;
        if (!driverAgg[key]) driverAgg[key] = { name: drv.name, type: kind === 'hired' ? 'Hired' : 'In-House', days: 0, orders: 0, drops: 0, value: 0 };
        driverAgg[key].days += 1;
        driverAgg[key].orders += drv.orders || 0;
        driverAgg[key].drops += drv.drops || 0;
        driverAgg[key].value += drv.value || 0;
        (drv.order_details || []).forEach(function(o) {
          orderRows.push({
            date: day.date, driver: drv.name, type: kind === 'hired' ? 'Hired' : 'In-House',
  ...
  var driverSummary = Object.keys(driverAgg).map(function(k) {
    var d = driverAgg[k];
    return {
      name: d.name, type: d.type, days_active: d.days, total_orders: d.orders, total_drops: d.drops,
      total_value: Math.round(d.value), avg_drops_per_day: d.days ? +((d.drops / d.days).toFixed(1)) : 0,
      value_per_drop: d.drops ? Math.round(d.value / d.drops) : 0
    };
  }).sort(function(a, b) { return b.total_value - a.total_value; });
  return { orderRows: orderRows, driverSummary: driverSummary };
}
app.get('/api/dispatch/driver-details/export', noCache, requireAuth, async function (req, res) {
  try {
    var ExcelJS = require('exceljs');
    var days = await getDriverDetailDays(req.query.date_from, req.query.date_to);
    if (!days.length) return res.status(400).json({ error: 'No dispatch data in this range.' });
    var built = buildDriverDetailReport(days);
    var driverSummary = built.driverSummary, orderRows = built.orderRows;

    var hiredRows = driverSummary.filter(function(d) { return d.type === 'Hired'; });
    var inhouseRows = driverSummary.filter(function(d) { return d.type === 'In-House'; });
    var totHiredValue = hiredRows.reduce(function(s, d) { return s + d.total_value; }, 0);
    var totInhouseValue = inhouseRows.reduce(function(s, d) { return s + d.total_value; }, 0);
    var totHiredDrops = hiredRows.reduce(function(s, d) { return s + d.total_drops; }, 0);
    var totInhouseDrops = inhouseRows.reduce(function(s, d) { return s + d.total_drops; }, 0);
    // Highest value-per-drop first — the drivers/trucks whose order value is least
    // spread across drops, i.e. the ones the transport-cost email flagged (4k-6k
    // order value on only 5-8 drops).
    var leastEfficient = driverSummary.slice().sort(function(a, b) { return b.value_per_drop - a.value_per_drop; }).slice(0, 10);

    var aiAdvice = '';
    try { aiAdvice = await generateDriverEfficiencyAdvice(days.length, days[0].date, days[days.length - 1].date, driverSummary, leastEfficient); }
    ...
    es.addRow(['Range', days[0].date + ' to ' + days[days.length - 1].date + ' (' + days.length + ' day(s))']);
    es.addRow([]);
    var h1 = es.addRow(['Metric', 'Value']); h1.font = { bold: true };
    es.addRow(['Total Drivers (In-House)', inhouseRows.length]);
    es.addRow(['Total Drivers (Hired)', hiredRows.length]);
    es.addRow(['Total Orders', driverSummary.reduce(function(s, d) { return s + d.total_orders; }, 0)]);
    es.addRow(['Total Drops', totInhouseDrops + totHiredDrops]);
    es.addRow(['Total Value (AED) — In-House', totInhouseValue]);
    es.addRow(['Total Value (AED) — Hired', totHiredValue]);
    es.addRow(['Avg Value/Drop (AED) — In-House', inhouseRows.length && totInhouseDrops ? Math.round(totInhouseValue / totInhouseDrops) : 0]);
    es.addRow(['Avg Value/Drop (AED) — Hired', hiredRows.length && totHiredDrops ? Math.round(totHiredValue / totHiredDrops) : 0]);
    es.addRow([]);
    var h2 = es.addRow(['Top 10 Least Drop-Efficient Drivers (highest AED/drop)']); es.mergeCells('A' + h2.number + ':B' + h2.number); h2.font = { bold: true };
    var h2b = es.addRow(['Driver', 'AED / Drop']); h2b.font = { bold: true };
    leastEfficient.forEach(function(d) { es.addRow([d.name + ' (' + d.type + ', ' + d.total_drops + ' drops, AED ' + d.total_value.toLocaleString() + ')', d.value_per_drop]); });

    // ---- Sheet 2: AI Advice ----
    var aiSheet = wb.addWorksheet('AI Advice');
    aiSheet.columns = [{ width: 120 }];
    var aiTitle = aiSheet.addRow(['AI ADVICE — OPTIMIZING DROP CONTROL']); aiTitle.font = { bold: true, size: 14 };
    aiSheet.addRow([]);
    aiAdvice.split('\n').forEach(function(line) { aiSheet.addRow([line]); });
    aiSheet.getColumn(1).alignment = { wrapText: true, vertical: 'top' };

    // ---- Sheet 3: Driver Summary ----
    var ds = wb.addWorksheet('Driver Summary');
    ds.columns = [{ width: 26 }, { width: 10 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 14 }];
    var dsTitle = ds.addRow(['DRIVER SUMMARY — ' + days[0].date + ' to ' + days[days.length - 1].date]); ds.mergeCells('A' + dsTitle.number + ':H' + dsTitle.number); dsTitle.font = { bold: true, size: 13 };
    ds.addRow([]);
    var dsHdr = ds.addRow(['Driver', 'Type', 'Days Active', 'Total Orders', 'Total Drops', 'Avg Drops/Day', 'Total Value (AED)', 'AED/Drop']); dsHdr.font = { bold: true };
    driverSummary.forEach(function(d) {
      var row = ds.addRow([d.name, d.type, d.days_active, d.total_orders, d.total_drops, d.avg_drops_per_day, d.total_value, d.value_per_drop]);
      row.getCell(7).numFmt = '#,##0'; row.getCell(8).numFmt = '#,##0';
      if (d.type === 'Hired') row.getCell(2).font = { color: { argb: 'FFE05C5C' }, bold: true };
    });

    // ---- Sheet 4: Order Details (customer / value / final drop, per order) ----
    var od = wb.addWorksheet('Order Details');
    od.columns = [{ width: 12 }, { width: 24 }, { width: 10 }, { width: 30 }, { width: 14 }, { width: 24 }, { width: 30 }, { width: 16 }, { width: 12 }];
    var odTitle = od.addRow(['ORDER DETAILS — CUSTOMER, VALUE & FINAL DROP PER ORDER']); od.mergeCells('A' + odTitle.number + ':I' + odTitle.number); odTitle.font = { bold: true, size: 13 };
    od.addRow([]);
    var odHdr = od.addRow(['Date', 'Driver', 'Type', 'Customer', 'Order Value (AED)', 'Final Drop', 'Address', 'Order Code', 'Order Type']); odHdr.font = { bold: true };
    orderRows.forEach(function(o) {
      ...
      if (o.type === 'Hired') row.getCell(3).font = { color: { argb: 'FFE05C5C' }, bold: true };
    });
    od.autoFilter = { from: { row: odHdr.number, column: 1 }, to: { row: odHdr.number, column: 9 } };

    var buf = await wb.xlsx.writeBuffer();
    res.setHeader('Content-Disposition', 'attachment; filename="Driver_Order_Details_' + Date.now() + '.xlsx"');
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(Buffer.from(buf));
  } catch (e) { res.status(500).json({ error: 'Export failed: ' + e.message }); }
});
async function generateDriverEfficiencyAdvice(dayCount, firstDate, lastDate, driverSummary, leastEfficient) {
  var totValue = driverSummary.reduce(function(s, d) { return s + d.total_value; }, 0);
  var totDrops = driverSummary.reduce(function(s, d) { return s + d.total_drops; }, 0);
  var dataText = leastEfficient.map(function(d) {
    return d.name + ' (' + d.type + '): ' + d.total_orders + ' orders, ' + d.total_drops + ' drops over ' + d.days_active + ' day(s), AED ' + d.total_value.toLocaleString() + ' total value, AED ' + d.value_per_drop.toLocaleString() + '/drop';
  }).join('\n');
  var prompt = 'You are a senior logistics operations analyst for a UAE last-mile distribution fleet. ' +
    'Range: ' + dayCount + ' day(s) (' + firstDate + ' to ' + lastDate + '). ' +
    'Fleet-wide: ' + driverSummary.length + ' drivers, AED ' + totValue.toLocaleString() + ' total order value across ' + totDrops.toLocaleString() + ' drops (AED ' + (totDrops ? Math.round(totValue / totDrops) : 0) + '/drop average).\n\n' +
    'The specific concern raised by operations: some drivers/trucks carry a high order value (e.g. AED 4,000-6,000) across very few drops (as low as 5 to 8) — this under-utilizes the truck and drives up transport cost per drop, since the truck cost is roughly fixed per trip regardless of how many drops it does. The goal is to increase drops-per-truck for a given order value, without adding trucks.\n\n' +
    'The 10 drivers with the HIGHEST AED-per-drop (i.e. the most under-utilized trucks) in this range:\n' + dataText + '\n\n' +
    'Do a deep analysis and answer, in clear sections with short headers (no markdown tables, plain text/bullets only):\n' +
    '1) WHERE THE INEFFICIENCY IS — name the specific driver(s)/truck(s) above the fleet average AED/drop, with their numbers, and whether this looks structural (same driver every day) or a one-off day.\n' +
    '2) WHY THIS IS HAPPENING — the most likely cause given the pattern (e.g. a route with too few stops for its order value, a truck-type mismatch, a hired driver used for a single large order instead of a full multi-drop route).\n' +
    '3) HOW TO OPTIMIZE DROP CONTROL — concrete, practical steps to raise drops-per-truck without adding cost: e.g. a minimum-drop threshold before dispatching a given order-value tier, consolidating routes, or a daily drop target per truck-type (mirroring what a target of 6-7 drops per AED 4,000+ order value would look like for the drivers above) — 3 to 5 specific, actionable points tied to the actual drivers/numbers above.\n' +
    'Be direct and specific with driver names and numbers — no generic advice that could apply to any dataset.';
  var msg = await anthropic.messages.create({
    model: 'claude-haiku-4-5-20251001', max_tokens: 1500,
    system: 'You are AZHAR-AI\'s logistics analyst for a UAE distribution company. Be concise, specific, and cite actual driver names/numbers from the data given. No fluff, no generic filler.',
    messages: [{ role: 'user', content: prompt }]
  });
...
app.get('/api/dispatch/driver-details/analyze', requireAuth, async function (req, res) {
  try {
    var days = await getDriverDetailDays(req.query.date_from, req.query.date_to);
    if (!days.length) return res.status(400).json({ error: 'No dispatch data in this range to analyze.' });
    var built = buildDriverDetailReport(days);
    var leastEfficient = built.driverSummary.slice().sort(function(a, b) { return b.value_per_drop - a.value_per_drop; }).slice(0, 10);
    var advice = await generateDriverEfficiencyAdvice(days.length, days[0].date, days[days.length - 1].date, built.driverSummary, leastEfficient);
