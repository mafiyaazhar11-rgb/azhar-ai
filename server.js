...
async function buildCustomerBreakdown(name, monthsFilter) {
  var result = await pool.query(
  ...
  if (monthsFilter && monthsFilter.length) {
    ...
    if (!rows.length) return { hasData: false, customer: name, availableMonths: availableMonths };
  }
  ...
  var acc = {};
  BUCKETS.forEach(function(b) { acc[b] = { rejected: 0, valueAtRisk: 0, foodCount: 0, nonfoodCount: 0, byOrg: {}, byMonth: {}, byMonthOrg: {}, byRoot: {}, byRootOrg: {}, byOrgRoot: {}, byOrgArea: {}, byBranch: {}, byBranchRoot: {}, recent: [] }; });
  ...
  function finalizeBucket(b) {
    ...
    var branchList = Object.keys(a.byBranch).map(function(k) {
      ...
      return { name: k, count: bd.count, value: bd.value, pct: a.rejected ? (bd.count / a.rejected * 100) : 0, rootCauses: rootsForBranch };
    }).sort(function(x, y) { return y.count - x.count; });
...
app.get('/api/rejection/customer-detail', requireAuth, async function(req, res) {
  try {
    ...
    var breakdown = await buildCustomerBreakdown(name, monthsFilter);
    res.json(breakdown);
  } catch (e) {
...
// (?source=) and month selection (?months=) the user had on screen. ──
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var d = await buildCustomerBreakdown(name, monthsFilter);
    ...
    ov.addRow(['Months', monthsFilter.length ? monthsFilter.join(', ') : 'All months on file']);
    if (src !== 'all') {
