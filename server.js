...
app.post('/api/rejection/upload', upload.single('file'), async function(req, res) {
  try {
    ...
    var newDatesArr = Array.from(newDates);
    try {
...
// ── Manual correction: fix one flagged row's Root Cause (from the discrepancy
...
app.put('/api/rejection/rows/correct', requireAuth, requireRole('superadmin', 'subadmin'), async function(req, res) {
  try {
    ...
    var rebuilt = await rebuildRejectionAggregateFromDB();
    rejectionData = { uploadedAt: new Date().toISOString(), uploadedBy: (rejectionData && rejectionData.uploadedBy) || 'Admin', fileName: (rejectionData && rejectionData.fileName) || '', totalOrders: rebuilt.totalRej + rebuilt.totalDel, orgs: rebuilt.orgsOut, months: rebuilt.monthsOut };
    await dbSaveRejection(rejectionData.uploadedBy, rejectionData.fileName, rejectionData.totalOrders, rebuilt.orgsOut, rebuilt.monthsOut);
    saveJSON(REJECTION_FILE, rejectionData);
...
app.post('/api/rejection/export-excel', async function(req, res) {
  try {
    ...
    // ---- Sheet 3: Month-Wise Analysis ----
    var monthWise = body.monthWise || [];
    if (monthWise.length) {
      var mw = wb.addWorksheet('Month-Wise Analysis');
      mw.columns = [{width:10},{width:14},{width:14},{width:14},{width:14},{width:20},{width:20}];
      styleHeaderRow(mw.addRow(['Month','Delivered','Rejection','Delivered %','Rejection %','Food (of Rej.)','Non-Food (of Rej.)']));
      ...
      var noteRow = mw.addRow(['Food/Non-Food % = share of that month\u2019s rejections specifically, not of total orders. "Frozen" is not currently tracked as a separate category.']);
