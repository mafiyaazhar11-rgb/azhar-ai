...
async function buildDriverRejectionTracker(dateFrom, dateTo) {
  ...
  // driver name -> running totals across every date in range
  var driverStats = {};
  dispR.rows.forEach(function (row) {
    ...
    [['inhouse_driver_details', false], ['hired_driver_details', true]].forEach(function (pair) {
      ...
      (dss[key] || []).forEach(function (drv) {
        if (!drv.name) return;
        if (!driverStats[drv.name]) driverStats[drv.name] = { isHired: isHired, orders: 0, drops: 0, dates: {} };
        driverStats[drv.name].orders += drv.orders || 0;
        driverStats[drv.name].drops += drv.drops || 0;
        driverStats[drv.name].dates[row.date_key] = true;
        (drv.order_details || []).forEach(function (o) {
          if (o.order_code) orderCodeToDriver[o.order_code] = { driver: drv.name, isHired: isHired, date: row.date_key };
        });
      });
    });
  });
  var rejR = await pool.query(
    ...
    [dateFrom, dateTo]);
  var totalRejections = rejR.rows.length;
  var matchedCount = 0;
  var sameDayReturnDates = {}; // 'order_code|date' -> true, filled in below only if needed
  ...
  if (orderCodesInPeriod.length) {
    // Same-day return = that exact order_code appears MORE THAN ONCE in order_tracking
    // on the SAME date as the rejection — i.e. it was re-attempted and accepted later
    // that same day, not carried over to a future date.
  ...
  rejR.rows.forEach(function (rej) {
    ...
    if ((sameDayCounts[rej.order_no + '|' + rej.entry_date] || 0) > 1) driverRejections[match.driver].sameDayReturns++;
  });
  ...
  Object.keys(driverStats).forEach(function (name) {
    ...
    if (!stat.isHired) {
      ...
      inhouseRows.push({
        ...
        rejection_rate_pct: stat.orders ? +((rej.count / stat.orders) * 100).toFixed(2) : 0,
  ...
  return {
    ...
    total_rejections: totalRejections, total_rejections_matched: matchedCount,
    total_rejections_unmatched: totalRejections - matchedCount,
