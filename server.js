...
function parseDispatchFromRows(rows) {
  ...
  return {
    ...
    // Salon — dedicated visibility per Azhar's request. orders/value come from the ORG=DGC
    // rows already present in Main dispatch (these carry real order values); source_upload_orders
    // is the count from the separate Salon (DIP W/H) file specifically, which has no dollar
    // values at all and — per COST_TRACKING_EXCLUDED_SOURCES above — never feeds vehicle,
    // driver, or transport-cost tracking until Azhar has a confirmed billing model for it.
    salon_summary: {
      orders: orgStats.DGC.o,
      value: Math.round(orgStats.DGC.v),
      source_upload_orders: rows.filter(function (r) { return r.__dispatch_source === 'Salon'; }).length
    }
  };
}
