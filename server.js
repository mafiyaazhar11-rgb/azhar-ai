...
async function buildDriverRejectionTracker(dateFrom, dateTo) {
  var dispR = await pool.query(
    ...
    [dateFrom, dateTo]);
  var dispatchDates = dispR.rows.map(function (r) { return r.date_key; });
  var earliestDispatchDate = dispatchDates.length ? dispatchDates[0] : null;
  var latestDispatchDate = dispatchDates.length ? dispatchDates[dispatchDates.length - 1] : null;
  ...
  return {
    ...
    days_with_dispatch_data: dispR.rows.length,
