...
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var exSideHdr1 = ex.addRow(['REJECTIONS BY BRANCH', '', '', '', '', 'FOOD vs NON-FOOD (' + srcLabel + ')']);
    styleSectionRow(exSideHdr1);
    var exBwHdr = ex.addRow(['Branch (ORG)', 'Rejected', 'Delivered', 'Value At Risk', '% of Rejections', 'Category', 'Orders', '% of Rejections']);
    ...
    var bs = wb.addWorksheet('Branch Summary');
    bs.columns = [{ width: 20 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 14 }, { width: 18 }, { width: 14 }, { width: 18 }];
    var bsTitle = bs.addRow(['BRANCH-WISE SUMMARY \u2014 ' + srcLabel + ' Rejections']);
    styleTitleRow(bsTitle); bs.mergeCells('A1:H1');
    bs.addRow([]);
    styleSectionRow(bs.addRow(['REJECTIONS BY BRANCH (ORG)']));
    styleHeaderRow(bs.addRow(['Branch (ORG)', 'Rejected', 'Delivered', 'Total Orders', 'Rejection Rate', 'Value At Risk (AED)', 'Value At Risk %', 'Avg AED / Rejection']));
    ...
    od.columns = [{ width: 14 }, { width: 55 }, { width: 12 }, { width: 12 }, { width: 18 }];
    var odTitle = od.addRow(['OUTLET-LEVEL DETAIL \u2014 ' + srcLabel + ' Rejections']);
    styleTitleRow(odTitle); od.mergeCells('A1:E1');
    od.addRow([]);
    styleSectionRow(od.addRow(['REJECTIONS BY AREA / OUTLET (ALL BRANCHES)']));
    styleHeaderRow(od.addRow(['Branch (ORG)', 'Area / Outlet (Address)', 'Rejected', '% of Branch', 'Value At Risk (AED)']));
