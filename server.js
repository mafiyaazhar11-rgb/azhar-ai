...
async function buildDriverRejectionTracker(dateFrom, dateTo) {
  // Fetch dispatch data starting ONE DAY BEFORE dateFrom, not from dateFrom itself.
  // Why: a rejection dated dateFrom (e.g. the 1st of the month for an MTD view)
  // actually describes dispatch from the day before (per the reporting-lag logic
  // below) — which is the last day of the PREVIOUS period. Without this extra day,
  // every rejection reported right at the start of the selected period would come up
  // unmatched purely from a query-window edge, not a real data problem.
  ...
  var dispatchDatesInPeriod = dispR.rows.filter(function (r) { return r.date_key >= dateFrom; }).map(function (r) { return r.date_key; });
  ...
  var calendarDaysInPeriod = Math.round((new Date(dateTo) - new Date(dateFrom)) / 86400000) + 1;
  ...
  dispR.rows.forEach(function (row) {
    var inSelectedPeriod = row.date_key >= dateFrom; // excludes the extra boundary day added above
    ...
    [['inhouse_driver_details', false], ['hired_driver_details', true]].forEach(function (pair) {
      ...
      (dss[key] || []).forEach(function (drv) {
        ...
        if (inSelectedPeriod) {
          if (!driverStats[bucketKey]) driverStats[bucketKey] = { name: resolvedName, isHired: isHired, orders: 0, drops: 0, dates: {} };
  ...
  // status='rej' + date-range filter, no order_no requirement here) so the two
  // dashboards never disagree on "how many rejections happened this period." ──
  var rejAllR = await pool.query(
  ...
  var inhouseRows = [], inhouseAgg = { orders: 0, drops: 0, rejections: 0, same_day_returns: 0, re_attempted: 0, driver_count: 0 };
  var hiredAgg = { orders: 0, drops: 0, rejections: 0, same_day_returns: 0, re_attempted: 0, driver_count: 0 };
  Object.keys(driverStats).forEach(function (bucketKey) {
    ...
    agg.orders += stat.orders; agg.drops += stat.drops; agg.rejections += rej.count;
...
app.get('/api/dispatch/driver-rejection-tracker', requireAuth, async function (req, res) {
  try {
    ...
    var data = await buildDriverRejectionTracker(period.date_from, period.date_to);
    data.period = period;
    res.json(data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/dispatch/driver-rejection-tracker/export', noCache, requireAuth, async function (req, res) {
  try {
    var ExcelJS = require('exceljs');
    var period = resolveDriverRejPeriod(req.query);
    var d = await buildDriverRejectionTracker(period.date_from, period.date_to);
    var wb = new ExcelJS.Workbook();
    wb.creator = 'AZHAR-AI'; wb.created = new Date();
    ...
    ws.columns = [{ width: 6 }, { width: 26 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 16 }, { width: 14 }, { width: 14 }];
    var hdr = ws.addRow(['#', 'Driver', 'Orders', 'Rejections', 'Rejection Rate %', 'Delivered %', 'Same-Day Returns', 'Re-Attempted', 'Avg Drops/Day']);
    hdr.font = { bold: true };
    (d.inhouse_drivers || []).forEach(function (row, i) {
      var r = ws.addRow([i + 1, row.driver, row.orders, row.rejections, row.rejection_rate_pct, row.delivered_pct, row.same_day_returns, row.re_attempted, row.avg_drops_per_day]);
