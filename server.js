...
function autoClassifyRootCause(raw) {
  ...
  return { category: polishRootCause(s), matched: false };
}
...
function polishRootCause(raw) {
  ...
  return s.replace(/\s+/g, ' ').trim().toLowerCase().replace(/\b\w/g, function(c) { return c.toUpperCase(); });
    ...
    for (var i=0; i<rows.length; i++) {
      ...
      var hasValueSignal = (trackedVal !== null) || !!RC.value;
      ...
      // AED 0 order was never a genuine sale to begin with. Takes priority
      // over the learned/rule-based root cause classification above. Only
      // fires when the value is CONFIRMED zero, never when it's just unknown. ──
