...
async function rebuildRejectionAggregateFromDB() {
  ...
  allDbRows.forEach(function(hr) {
    ...
    accumulateRejectionRow(orgMap, monthMap, {
      ...
      mo: mo, day: day, isFood: hr.is_food, isNF: hr.is_nonfood, srcStr: (hr.source || '').toUpperCase(),
  ...
  try {
    ...
    for (var i=0; i<rows.length; i++) {
      ...
      if (forceInternalSource) {
        ...
        if (ieResult.matched) { srcStr=ieResult.binary.toUpperCase(); classifyStats.sourceAutoMatched++; }
      }
      if (d) {
        rowsToStore.push({
