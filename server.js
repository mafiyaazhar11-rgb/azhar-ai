...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    ...
    ex.columns = [{ width: 30 }, { width: 18 }, { width: 14 }, { width: 14 }];
    var exTitle = ex.addRow(['MARKET RETURNS — FULL ANALYZED REPORT']); styleTitleRow(exTitle); ex.mergeCells('A1:D1');
    var exSub = ex.addRow(['AKI Group  —  Invoice Cancellations · GRV Returns · BU-wise  —  Generated ' + genDate]);
    exSub.font = { bold: true, size: 11, color: { argb: 'FF555555' } }; ex.mergeCells('A2:D2');
    ex.addRow([]);

    styleSectionRow(ex.addRow(['KEY METRICS', '', '', '']), 4);
    var kpiDefs = [
      ['Total Returns Value (AED)', { formula: 'SUM(' + R_VAL + ')' }, '#,##0'],
      ['Invoice Cancellations (orders)', { formula: 'COUNTIF(' + R_CAT + ',"Invoice Cancellation")' }, '#,##0'],
      ['GRV Returns (orders)', { formula: 'COUNTIF(' + R_CAT + ',"GRV-Return")' }, '#,##0'],
      ['Total Orders', { formula: 'COUNTA(' + R_MONTH + ')' }, '#,##0']
    ];
    ...
    styleSectionRow(ex.addRow(['FOOD vs NON-FOOD vs ZOOZ — OVERALL SPLIT', '', '', '']), 4);
    var buHdr = ex.addRow(['Category', 'Value (AED)', '% of Total']); styleHeaderRow(buHdr, 3);
    ...
    for (var bi = buFirstRow; bi <= buLastRow; bi++) {
      ex.getCell('C' + bi).value = { formula: 'IFERROR(B' + bi + '/$B$' + totalValRow + ',0)' };
    ...
    for (var cci = catFirstRow; cci <= catLastRow; cci++) {
      ex.getCell('D' + cci).value = { formula: 'IFERROR(C' + cci + '/$B$' + totalValRow + ',0)' };
      ex.getCell('D' + cci).numFmt = '0.0%';
    }
    ex.addRow([]);
    noteRow(ex, 'All figures on this tab are calculated live from the Return Master tab and update automatically if any row is corrected. DST/DCS/DSC warehouse codes are excluded from this report entirely (data-entry noise, not a real BU).', 'D');
    ...
    mtFullMonths.forEach(function(m, mi) {
      ...
      var r0 = mt.addRow([mLabel,
        ...
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_BU + ',"Zooz")' },
        { formula: 'SUMIF(' + R_MONTH + ',' + m + ',' + R_VAL + ')' },
        '', '', '',
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Food")' },
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Non-Food")' },
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Zooz")' },
        { formula: 'COUNTIF(' + R_MONTH + ',' + m + ')' },
        '']);
      ['B','C','D','E'].forEach(function(col){ r0.getCell(col).numFmt = '#,##0'; });
      r0.getCell('F').value = { formula: 'IFERROR(B' + r0.number + '/E' + r0.number + ',0)' }; r0.getCell('F').numFmt = '0.0%';
      r0.getCell('G').value = { formula: 'IFERROR(C' + r0.number + '/E' + r0.number + ',0)' }; r0.getCell('G').numFmt = '0.0%';
      r0.getCell('H').value = { formula: 'IFERROR(D' + r0.number + '/E' + r0.number + ',0)' }; r0.getCell('H').numFmt = '0.0%';
      if (mi === 0) { r0.getCell('M').value = '—'; }
      else { r0.getCell('M').value = { formula: 'IFERROR((E' + r0.number + '-E' + (r0.number - 1) + ')/E' + (r0.number - 1) + ',0)' }; r0.getCell('M').numFmt = '0.0%'; }
      borderRow(r0, 13);
      zebra(r0, 13, mi % 2 === 1);
    });
    ...
    if (mtFullMonths.length) {
      var mtTotalRow = mt.addRow(['TOTAL',
        { formula: 'SUM(B' + mtFirstRow + ':B' + mtLastRow + ')' },
        { formula: 'SUM(C' + mtFirstRow + ':C' + mtLastRow + ')' },
        { formula: 'SUM(D' + mtFirstRow + ':D' + mtLastRow + ')' },
        { formula: 'SUM(E' + mtFirstRow + ':E' + mtLastRow + ')' },
        '', '', '',
        { formula: 'SUM(I' + mtFirstRow + ':I' + mtLastRow + ')' },
        { formula: 'SUM(J' + mtFirstRow + ':J' + mtLastRow + ')' },
        { formula: 'SUM(K' + mtFirstRow + ':K' + mtLastRow + ')' },
        { formula: 'SUM(L' + mtFirstRow + ':L' + mtLastRow + ')' },
        '']);
      ['B','C','D','E'].forEach(function(col){ mtTotalRow.getCell(col).numFmt = '#,##0'; });
      var mtTotRowNum = mtTotalRow.number;
      mtTotalRow.getCell('F').value = { formula: 'IFERROR(B' + mtTotRowNum + '/E' + mtTotRowNum + ',0)' }; mtTotalRow.getCell('F').numFmt = '0.0%';
      mtTotalRow.getCell('G').value = { formula: 'IFERROR(C' + mtTotRowNum + '/E' + mtTotRowNum + ',0)' }; mtTotalRow.getCell('G').numFmt = '0.0%';
      mtTotalRow.getCell('H').value = { formula: 'IFERROR(D' + mtTotRowNum + '/E' + mtTotRowNum + ',0)' }; mtTotalRow.getCell('H').numFmt = '0.0%';
