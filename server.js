...
// two data sources. ──
async function buildCustomerPriceDiscrepancies(name) {
  var result = await pool.query(
    `SELECT *,
...
app.get('/api/price-discrepancy/customer-detail', requireAuth, async function(req, res) {
  try {
    var name = toStr(req.query.name || '').trim();
    if (!name) return res.status(400).json({ error: 'Customer name is required' });
    var breakdown = await buildCustomerPriceDiscrepancies(name);
    res.json(breakdown);
  } catch (e) {
...
app.get('/api/price-discrepancy/customer-export-excel', requireAuth, async function(req, res) {
  try {
    var name = toStr(req.query.name || '').trim();
    if (!name) return res.status(400).json({ error: 'Customer name is required' });
    var d = await buildCustomerPriceDiscrepancies(name);
    ...
    ws.columns = [{ width: 20 }, { width: 40 }, { width: 14 }, { width: 14 }, { width: 14 }, { width: 12 }, { width: 14 }, { width: 40 }, { width: 22 }];
    ...
    ws.mergeCells('A2:I2');
    ws.addRow([]);
    var kpiRow = ws.addRow(['Open', d.open, 'Resolved', d.resolved, 'Total Value At Risk (Open)', 'AED ' + d.totalOpenValue.toLocaleString()]);
    kpiRow.font = { bold: true };
    ws.addRow([]);
    var hdr = ws.addRow(['SKU Code', 'SKU Description', 'System Price', 'LPO Price', 'Discrepancy', 'Status', 'Times Reported', 'Remarks', 'Reported By']);
    hdr.eachCell(function(cell) {
      cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } };
      cell.font = { bold: true, color: { argb: GOLD }, size: 11 };
    });
    d.items.forEach(function(it) {
      var row = ws.addRow([it.sku_code, it.sku_description, it.system_price, it.lpo_price, it.discrepancy, it.status, it.times_reported, it.remarks, it.reported_by]);
      if (it.status === 'Open') {
        row.getCell(6).font = { color: { argb: 'FFB00000' }, bold: true };
      } else {
        row.eachCell(function(cell) { cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: LIGHTGOLD } }; });
      }
    });

    var fileName = 'Price_Discrepancy_' + name.replace(/[^a-z0-9]+/gi, '_').slice(0, 40) + '.xlsx';
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.setHeader('Content-Disposition', 'attachment; filename="' + fileName + '"');
    await wb.xlsx.write(res);
    res.end();
