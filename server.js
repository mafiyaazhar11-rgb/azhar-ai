...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    ...
    function styleTitleRow(row) { row.font = { bold: true, size: 14, color: { argb: GOLD } }; row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } }; row.height = 26; row.alignment = { vertical: 'middle' }; }
    function noteRow(sheet, text, span) {
      var r = sheet.addRow([text]);
      r.font = { italic: true, size: 9, color: { argb: 'FF666666' } };
      sheet.mergeCells('A' + r.number + ':' + span + r.number);
    ...
    exSub.font = { bold: true, size: 11, color: { argb: 'FF555555' } }; ex.mergeCells('A2:E2');
    ex.addRow([]);
    ...
    kpiDefs.forEach(function(k, ki) {
      ...
      r0.getCell(1).font = { bold: true, size: 11, color: { argb: DARKBG } };
      r0.getCell(2).font = { bold: true, size: 14, color: { argb: ki === 0 ? 'FFB00000' : DARKBG } };
    ...
    var wh = wb.addWorksheet('By Warehouse', { views: [{ showGridLines: false, state: 'frozen', ySplit: 3 }] });
    ...
    [1,2,3].forEach(function(ci){ var c=arHdr.getCell(ci); c.fill={type:'pattern',pattern:'solid',fgColor:{argb:DARKBG}}; c.font={bold:true,color:{argb:GOLD},size:11}; c.border=CELL_BORDER; });
    [5,6,7].forEach(function(ci){ var c=arHdr.getCell(ci); c.fill={type:'pattern',pattern:'solid',fgColor:{argb:DARKBG}}; c.font={bold:true,color:{argb:GOLD},size:11}; c.border=CELL_BORDER; });
    arHdr.height = 20;
