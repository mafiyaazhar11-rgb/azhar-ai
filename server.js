...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    ...
    rows.forEach(function(r) {
      ...
      if (r.customer) {
        if (!custAgg[r.customer]) custAgg[r.customer] = { orders: 0, inv: 0, grv: 0, val: 0 };
        custAgg[r.customer].orders++;
        if (r.category === 'Invoice Cancellation') custAgg[r.customer].inv++;
        else if (r.category === 'GRV-Return') custAgg[r.customer].grv++;
        custAgg[r.customer].val += v;
      }
      if (r.warehouse) {
        if (!whAgg[r.warehouse]) whAgg[r.warehouse] = { orders: 0, inv: 0, grv: 0, val: 0 };
        whAgg[r.warehouse].orders++;
        if (r.category === 'Invoice Cancellation') whAgg[r.warehouse].inv++;
        else if (r.category === 'GRV-Return') whAgg[r.warehouse].grv++;
    ...
    wh.columns = [{ width: 14 }, { width: 12 }, { width: 14 }, { width: 14 }, { width: 16 }, { width: 12 }];
    var whTitle = wh.addRow(['RETURNS BY WAREHOUSE']); styleTitleRow(whTitle); wh.mergeCells('A1:F1');
    warningRow(wh, JAN_MAR_DCV_WARNING, 'F');
    var whHdr = wh.addRow(['Warehouse', 'Orders', 'Invoice Cancel', 'GRV Return', 'Value (AED)', '% of Total']);
    styleHeaderRow(whHdr, 6);
    ...
    for (var whi = whFirstRow; whi <= whLastRow; whi++) {
      ...
      wh.getCell('F' + whi).value = { formula: 'IFERROR(E' + whi + '/SUM($E$' + whFirstRow + ':$E$' + whLastRow + '),0)', result: pct(whAgg[whRowW].val, whTotalVal) };
    ...
    var whTotalRow = wh.addRow(['TOTAL',
      ...
      { formula: 'SUM(D' + whFirstRow + ':D' + whLastRow + ')', result: whSumGrv },
      { formula: 'SUM(E' + whFirstRow + ':E' + whLastRow + ')', result: whTotalVal }, 1]);
    whTotalRow.getCell(5).numFmt = '#,##0'; whTotalRow.getCell(6).numFmt = '0.0%';
    styleTotalRow(whTotalRow, 6);
    ...
    tc.columns = [{ width: 34 }, { width: 12 }, { width: 14 }, { width: 14 }, { width: 16 }, { width: 12 }];
    var tcTitle = tc.addRow(['RETURNS BY CUSTOMER']); styleTitleRow(tcTitle); tc.mergeCells('A1:F1');
    var tcHdr = tc.addRow(['Customer', 'Orders', 'Invoice Cancel', 'GRV Return', 'Value (AED)', '% of Total']);
    styleHeaderRow(tcHdr, 6);
    ...
    for (var tci = tcFirstRow; tci <= tcLastRow; tci++) {
      ...
      tc.getCell('F' + tci).value = { formula: 'IFERROR(E' + tci + '/SUM($E$' + tcFirstRow + ':$E$' + tcLastRow + '),0)', result: pct(custAgg[tcCustName].val, custTotalVal) };
      tc.getCell('F' + tci).numFmt = '0.0%';
    }
    if (custKeys.length) {
      ...
      var tcTotalRow = tc.addRow(['TOTAL',
        ...
        { formula: 'SUM(D' + tcFirstRow + ':D' + tcLastRow + ')', result: tcSumGrv },
        { formula: 'SUM(E' + tcFirstRow + ':E' + tcLastRow + ')', result: custTotalVal }, 1]);
      tcTotalRow.getCell(5).numFmt = '#,##0'; tcTotalRow.getCell(6).numFmt = '0.0%';
      styleTotalRow(tcTotalRow, 6);
    }
    tc.autoFilter = { from: { row: 2, column: 1 }, to: { row: 2, column: 6 } };
    noteRow(tc, 'Calculated live from the Return Master tab. Every customer on file, not just the dashboard\'s top 10.', 'F');
