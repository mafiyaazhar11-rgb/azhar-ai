...
app.get('/api/backlog/export', noCache, requireAuth, async function(req, res) {
  try {
    ...
    exec.columns = [{ width: 30 }, { width: 16 }, { width: 18 }, { width: 12 }, { width: 16 }];
    ...
    function kpiRow(label, count, val, fillArgb) {
      ...
      if (fillArgb) r.getCell(1).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: fillArgb } };
    }
    kpiRow('Total Pool Orders', s.totalOrders || 0, s.totalVal || 0, GOLD);
    kpiRow('WH Backlog Pure Risk (Before Cut-Off)', whBL.count, whBL.val, RED);
    kpiRow('Advance Orders (RSD > 30 Days)', advOrders.count, advOrders.val);
    kpiRow('Credit Hold (Auto-Booked After Cutoff \u2192 Late Release)', creditHold.list.length, creditHold.totalVal, GOLD);
    kpiRow('Action Required Today', whBL.count, whBL.val, RED);
    exec.addRow([]);
