...
async function loadInvSalesFromDB() {
  try {
    await pool.query(`CREATE TABLE IF NOT EXISTS invsales_data (
      ...
      uploaded_by TEXT, file_name TEXT, total_orders INT, summary JSONB
    )`);
    var r = await pool.query('SELECT * FROM invsales_data ORDER BY uploaded_at DESC LIMIT 1');
    if (r.rows[0]) {
      invSalesData = { uploadedAt: r.rows[0].uploaded_at, fileName: r.rows[0].file_name, totalOrders: r.rows[0].total_orders, summary: r.rows[0].summary };
...
app.post('/api/invsales/upload', requireAuth, requireRole('superadmin','subadmin'), async function(req, res) {
  try {
    ...
    try {
      ...
      console.log('InvSales saved to DB, size:', JSON.stringify(summary).length, 'bytes');
    } catch(dbErr) { console.error('InvSales DB save error:', dbErr.message); }
