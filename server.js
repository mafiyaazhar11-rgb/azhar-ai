...
async function buildCustomerBreakdown(name, monthsFilter, groupKey) {
  var result;
...
app.get('/api/rejection/customer-detail', requireAuth, async function(req, res) {
  try {
    ...
    var breakdown = await buildCustomerBreakdown(name, monthsFilter, groupKey || null);
    res.json(breakdown);
  } catch (e) {
...
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var d = await buildCustomerBreakdown(name, monthsFilter, groupKey || null);
...
app.get('/api/price-discrepancy/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    d.items.forEach(function(it) {
      ...
      row.getCell(5).font = { color: { argb: it.discrepancy < 0 ? 'FFB00000' : 'FF1A8A3A' }, bold: true };
      if (it.status === 'Open') {
