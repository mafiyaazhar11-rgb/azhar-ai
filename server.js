...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    ...
    var whAgg = {}, monthAgg = {}, areaMap = {}, reasonMap = {};
    rows.forEach(function(r) {
    ...
    var GOLD = 'FFC9A84C', DARKBG = 'FF1A1E26', LIGHTGOLD = 'FFF5E9C8', BORDER_COLOR = 'FFD0D0D0';
    ...
    ex.addRow([]);
    noteRow(ex, 'All figures on this tab are calculated live from the Return Master tab and update automatically if any row is corrected. DST/DCS/DSC warehouse codes are excluded from this report entirely (data-entry noise, not a real BU).', 'E');
    ...
    noteRow(wh, 'Calculated live from the Return Master tab. DST/DCS/DSC excluded (data-entry noise).', 'F');
    ...
    noteRow(ar, 'Calculated live from the Return Master tab. Reason column depends on the source upload including a Return Reason field. DST/DCS/DSC excluded.', 'G');
    ...
    var rmTitle = rm.addRow(['RETURN MASTER DATA (' + rows.length + ' rows — DST/DCS/DSC excluded)']); styleTitleRow(rmTitle); rm.mergeCells('A1:I1');
    var rmHdr = rm.addRow(['Month', 'Day', 'Warehouse', 'Category', 'BU', 'Value (AED)', 'Area', 'Order No', 'Return Reason']);
    styleHeaderRow(rmHdr, 9);
    rows.forEach(function(r, ri) {
      ...
      r0.getCell(6).numFmt = '#,##0';
      for (var ci = 1; ci <= 9; ci++) r0.getCell(ci).border = { bottom: THIN };
      if (ri % 2 === 1) { for (var ci2 = 1; ci2 <= 9; ci2++) r0.getCell(ci2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F5EF' } }; }
    });
    rm.autoFilter = { from: { row: 2, column: 1 }, to: { row: 2, column: 9 } };
