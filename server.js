...
async function buildCustomerBreakdown(name) {
  ...
  var acc = {};
  BUCKETS.forEach(function(b) { acc[b] = { rejected: 0, valueAtRisk: 0, foodCount: 0, nonfoodCount: 0, byOrg: {}, byMonth: {}, byMonthOrg: {}, byRoot: {}, byRootOrg: {}, byOrgRoot: {}, recent: [] }; });
  ...
  function finalizeBucket(b) {
    ...
    var orgList = Object.keys(orgKeys).map(function(k) {
      ...
      return { org: k, rejected: o.rejected, delivered: deliveredByOrg[k] || 0, value: o.value, rootCauses: rootsForOrg };
...
app.get('/api/rejection/customer-export-excel', requireAuth, async function(req, res) {
  try {
    ...
    // ---- Sheet 5: Recent Orders — full detail: date, order, ORG, area,
    // root cause + plain explanation, source, value ----
    var ro = wb.addWorksheet('Recent Orders');
    ro.columns = [{ width: 12 }, { width: 16 }, { width: 8 }, { width: 16 }, { width: 34 }, { width: 42 }, { width: 10 }, { width: 14 }];
    styleHeaderRow(ro.addRow(['Date', 'Order No', 'ORG', 'Area', 'Root Cause', 'What This Means', 'Source', 'Value (AED)']));
    b.recentRejections.forEach(function(r) {
      var dt = r.date ? new Date(r.date) : null;
      ro.addRow([dt, r.order_no, r.org, r.area || '—', r.root_cause, humanizeRootCause(r.root_cause), r.source || '—', Math.round(r.value || 0)]);
