...
// ============================================================
// DRIVER ORDER DETAILS — one driver/truck is a DAILY assignment, not a persistent
// identity (a "Hired Driver" phone number can be a completely different truck from
// one day to the next), so this report always reflects a SINGLE day — the most
// recent date in whatever range is picked — rather than summing a driver's numbers
// across multiple days, which would blend unrelated trucks/routes together into a
// meaningless total. Built on driver_source_split.hired_driver_details /
// inhouse_driver_details, which already carry each driver's order_details array
// (customer, order value, final drop per order) — nothing new to store per upload.
// ============================================================
async function getDriverDetailSnapshot(dateFrom, dateTo) {
  ...
  var dss = s.driver_source_split || {};
  return { date: r.rows[0].date_key, hired: dss.hired_driver_details || [], inhouse: dss.inhouse_driver_details || [] };
}
// One day's hired + in-house drivers, each already carrying its own orders/drops/
// value/value_per_drop (computed per day in parseDispatchFromRows) — flattened into
// (a) a per-driver summary list and (b) a per-order list, no cross-day math at all.
function buildDriverDetailReport(snapshot) {
  var driverSummary = [], orderRows = [];
  ['inhouse', 'hired'].forEach(function(kind) {
    (snapshot[kind] || []).forEach(function(drv) {
      var type = kind === 'hired' ? 'Hired Truck' : 'In-House';
      driverSummary.push({
        name: drv.name, type: type, total_orders: drv.orders || 0, total_drops: drv.drops || 0,
        total_value: Math.round(drv.value || 0), value_per_drop: drv.value_per_drop || 0
      });
  ...
  driverSummary.sort(function(a, b) { return b.total_value - a.total_value; });
  return { orderRows: orderRows, driverSummary: driverSummary };
}
// Efficiency color bands for AED/drop — red flags the drivers/trucks the transport-
// cost concern is actually about (high order value spread over very few drops).
function efficiencyFill(valuePerDrop, fleetAvg) {
  if (!fleetAvg) return null;
  if (valuePerDrop >= fleetAvg * 1.5) return 'FFF8D7DA'; // red — well above average, flag
  if (valuePerDrop >= fleetAvg) return 'FFFDF2CE'; // amber — above average
  return 'FFDCF5E3'; // green — at or below average, well-utilized
}
...
app.get('/api/dispatch/driver-details/export', noCache, requireAuth, async function (req, res) {
  try {
    ...
    var built = buildDriverDetailReport(snapshot);
    var driverSummary = built.driverSummary, orderRows = built.orderRows;

    var hiredRows = driverSummary.filter(function(d) { return d.type === 'Hired Truck'; });
    ...
    var fleetAvgPerDrop = (totHiredDrops + totInhouseDrops) ? Math.round((totHiredValue + totInhouseValue) / (totHiredDrops + totInhouseDrops)) : 0;
    // Highest value-per-drop first — the trucks whose order value is least spread
    // across drops, i.e. what the transport-cost email flagged (4k-6k order value
    // on only 5-8 drops).
    var leastEfficient = driverSummary.slice().sort(function(a, b) { return b.value_per_drop - a.value_per_drop; }).slice(0, 10);

    var aiAdvice = '';
    try { aiAdvice = await generateDriverEfficiencyAdvice(snapshot.date, driverSummary, leastEfficient, fleetAvgPerDrop); }
    ...
    es.addRow(['Date', snapshot.date + ' (single day — see note below)']);
    es.addRow([]);
    var h1 = es.addRow(['Metric', 'Value']); h1.font = { bold: true };
    es.addRow(['Total Drivers (In-House)', inhouseRows.length]);
    es.addRow(['Total Trucks (Hired)', hiredRows.length]);
    ...
    es.addRow(['Fleet Avg AED/Drop (all)', fleetAvgPerDrop]);
    es.addRow([]);
    var noteRow = es.addRow(['Note: figures are for ' + snapshot.date + ' only — a driver/truck (especially a hired one) is a daily assignment, so its numbers are never combined across days.']);
    es.mergeCells('A' + noteRow.number + ':B' + noteRow.number); noteRow.font = { italic: true, size: 10, color: { argb: 'FF888888' } };
    es.addRow([]);
    var h2 = es.addRow(['Top 10 Least Drop-Efficient (highest AED/drop, ' + snapshot.date + ')']); es.mergeCells('A' + h2.number + ':B' + h2.number); h2.font = { bold: true };
    var h2b = es.addRow(['Driver / Truck', 'AED / Drop']); h2b.font = { bold: true };
    leastEfficient.forEach(function(d) {
      var row = es.addRow([d.name + ' (' + d.type + ', ' + d.total_drops + ' drops, AED ' + d.total_value.toLocaleString() + ')', d.value_per_drop]);
      var fill = efficiencyFill(d.value_per_drop, fleetAvgPerDrop);
    ...
    var aiTitle = aiSheet.addRow(['AI ADVICE — OPTIMIZING DROP CONTROL (' + snapshot.date + ')']); aiTitle.font = { bold: true, size: 14 };
    aiSheet.addRow([]);
    ...
    ds.columns = [{ width: 30 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 16 }, { width: 12 }];
    var dsTitle = ds.addRow(['DRIVER / TRUCK SUMMARY — ' + snapshot.date]); ds.mergeCells('A' + dsTitle.number + ':F' + dsTitle.number); dsTitle.font = { bold: true, size: 13 };
    ds.addRow([]);
    var dsHdr = ds.addRow(['Driver / Truck', 'Type', 'Orders', 'Drops', 'Total Value (AED)', 'AED/Drop']); dsHdr.font = { bold: true };
    driverSummary.forEach(function(d) {
      var row = ds.addRow([d.name, d.type, d.total_orders, d.total_drops, d.total_value, d.value_per_drop]);
      row.getCell(5).numFmt = '#,##0'; row.getCell(6).numFmt = '#,##0';
      row.getCell(2).font = { bold: true, color: { argb: d.type === 'Hired Truck' ? 'FFC0392B' : 'FF1E7E34' } };
      var fill = efficiencyFill(d.value_per_drop, fleetAvgPerDrop);
...
// Kept short and plain on purpose — 3 tight bullets, no markdown symbols, so it reads
// cleanly both on screen and dropped into an Excel cell. "Hired" entries are hired
// TRUCKS engaged for that one day (identified only by a phone number, not a name) —
// not permanent staff — so a hired flag is a same-day trucking decision, not a
// person's track record.
async function generateDriverEfficiencyAdvice(date, driverSummary, leastEfficient, fleetAvgPerDrop) {
  var dataText = leastEfficient.map(function(d) {
    return d.name + ' (' + d.type + '): ' + d.total_orders + ' orders, ' + d.total_drops + ' drops, AED ' + d.total_value.toLocaleString() + ' total, AED ' + d.value_per_drop.toLocaleString() + '/drop';
  }).join('\n');
  var prompt = 'UAE last-mile distribution fleet, single day: ' + date + '. ' +
    driverSummary.length + ' drivers/trucks, fleet average AED ' + fleetAvgPerDrop.toLocaleString() + '/drop.\n\n' +
    '"Hired" below means a hired TRUCK engaged just for this one day (phone number only, no name on file) — not a permanent employee.\n\n' +
    'The concern: some trucks carry high order value (e.g. AED 4,000-6,000) across very few drops (5 to 8) — this wastes truck capacity and raises cost per drop. Goal: more drops per truck for the same order value, no extra trucks.\n\n' +
    'The 10 highest AED/drop today (most under-utilized):\n' + dataText + '\n\n' +
    'Reply in PLAIN TEXT only — no #, no **, no markdown of any kind. Keep it SHORT: 3 short sections, each 1-2 sentences plus at most 2 bullet lines, using actual names/numbers from above:\n' +
    'GAP: which truck(s) today are most under-utilized and by how much.\n' +
    'LIKELY CAUSE: the most probable reason (single big order, no consolidation, wrong truck size).\n' +
    'FIX: 2-3 concrete actions to raise drops per truck tomorrow.';
  var msg = await anthropic.messages.create({
    model: 'claude-haiku-4-5-20251001', max_tokens: 500,
    system: 'You are AZHAR-AI\'s logistics analyst for a UAE distribution company. Be brief and concrete — plain text only, no markdown symbols, cite real names/numbers, no filler.',
    messages: [{ role: 'user', content: prompt }]
  });
...
app.get('/api/dispatch/driver-details/analyze', requireAuth, async function (req, res) {
  try {
    ...
    var built = buildDriverDetailReport(snapshot);
    var totValue = built.driverSummary.reduce(function(s, d) { return s + d.total_value; }, 0);
    var totDrops = built.driverSummary.reduce(function(s, d) { return s + d.total_drops; }, 0);
    var fleetAvgPerDrop = totDrops ? Math.round(totValue / totDrops) : 0;
    var leastEfficient = built.driverSummary.slice().sort(function(a, b) { return b.value_per_drop - a.value_per_drop; }).slice(0, 10);
    var advice = await generateDriverEfficiencyAdvice(snapshot.date, built.driverSummary, leastEfficient, fleetAvgPerDrop);
