...
// ============================================================
// DRIVER MASTER — resolves a bare employee code (e.g. "E111640") that sometimes
// shows up in dispatch files instead of a driver's name, using a source file
// (Employee ID + Name) uploaded once. Every downstream driver report (Driver
// Rejection Tracker, driver leaderboards) clubs a code and its real name together
// as one person once this is uploaded, instead of counting them as two drivers.
// ============================================================
var driverMasterMap = {}; // driver_code (uppercased, trimmed) -> real driver_name
async function refreshDriverMasterMap() {
  try {
    var r = await pool.query('SELECT driver_code, driver_name FROM driver_master');
    var m = {};
    r.rows.forEach(function (row) { m[String(row.driver_code).trim().toUpperCase()] = row.driver_name; });
    driverMasterMap = m;
  } catch (e) { console.error('Driver Master: could not load map:', e.message); }
}
refreshDriverMasterMap();
// Resolves a raw driver identifier (name OR bare code) to its real name if a match
// exists in driver_master; otherwise returns the raw value unchanged.
function resolveDriverName(raw) {
  var key = String(raw || '').trim().toUpperCase();
  return driverMasterMap[key] || raw;
}
...
app.get('/api/driver-master/status', requireAuth, async function (req, res) {
  try {
    var r = await pool.query('SELECT COUNT(*)::int AS cnt FROM driver_master');
    res.json({ total_drivers: r.rows[0].cnt });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
