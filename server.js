...
async function dbLoadDispatch() {
  try {
    // No practical cap — this only loads the lightweight per-date summary
    // JSON (not raw rows), so even several years of daily data is small.
    // The old LIMIT 60 here silently dropped older months from memory
    // (though never from the database) once total dispatch days passed 60.
    const res = await pool.query(`SELECT date_key::text, uploaded_at, uploaded_by, summary, csv_text FROM dispatch_data ORDER BY date_key DESC`);
    return res.rows;
  } catch(e) {
