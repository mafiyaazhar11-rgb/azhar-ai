...
function parseDispatchFromRows(rows) {
  ...
  var C = {
    ...
    orderCode: findCol('ORDER CODE', 'ORDER_CODE') || findExactCol('ORDER', 'ORDER NUMBER', 'ORDER_NUMBER') || findCol('ORDER ') || findCol('TASK_ID', 'TASK ID'),
...
function extractOrderRowsFromRows(rows) {
...
app.post('/api/dispatch/upload', upload.single('file'), async function(req, res) {
  try {
    ...
    try {
      ...
      var orderRows = [];
      allSourcesRes.rows.forEach(function (s) {
        orderRows = orderRows.concat(extractOrderRowsFromRows(s.rows_json));
      });
    ...
    for (var i = 0; i < dateRows.rows.length; i++) {
      ...
      try {
        ...
        if (sourcesRes.rows.length) {
          ...
          // existed) — re-parse from the combined CSV that was already stored.
          var legacyRes = await pool.query('SELECT csv_text FROM dispatch_data WHERE date_key=$1', [dateKey]);
          if (legacyRes.rows.length && legacyRes.rows[0].csv_text) {
            var legacyWb = XLSX.read(legacyRes.rows[0].csv_text, { type: 'string', raw: true });
            var legacyRows = XLSX.utils.sheet_to_json(legacyWb.Sheets[legacyWb.SheetNames[0]], { defval: '', raw: true });
...
app.get('/api/dispatch/reconcile/order-diff', requireAuth, async function (req, res) {
  try {
    ...
    var appRows = await pool.query('SELECT DISTINCT order_code FROM order_tracking WHERE date_key=$1 AND org = ANY($2::text[])', [date, RECON_ELIGIBLE_ORG_LIST]);
...
app.get('/api/dispatch/reconcile/export', requireAuth, async function (req, res) {
  try {
    ...
    for (var i = 0; i < orderIdRows.rows.length; i++) {
      ...
      var appOrderRows = await pool.query('SELECT DISTINCT order_code FROM order_tracking WHERE date_key=$1 AND org = ANY($2::text[])', [dk, RECON_ELIGIBLE_ORG_LIST]);
