...
app.post('/api/dispatch/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function(req, res) {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file received' });
...
app.post('/api/rejection/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function(req, res) {
  try {
    if (!req.file) return res.status(400).json({ error:'No file received' });
    ...
    if (req.user.role === 'subadmin') {
      try {
        var cdRes = await pool.query(
          `SELECT last_upload_at FROM upload_cooldowns WHERE user_id=$1 AND endpoint='rejection'`,
          [req.user.uid]
        );
        if (cdRes.rows[0]) {
          var hoursSince = (Date.now() - new Date(cdRes.rows[0].last_upload_at).getTime()) / 3600000;
          if (hoursSince < 10) {
            var minsLeft = Math.ceil((10 - hoursSince) * 60);
            return res.status(429).json({
              error: '\u23f3 Sub-admins can upload once every 10 hours. You already uploaded recently \u2014 please try again in ' + Math.floor(minsLeft / 60) + 'h ' + (minsLeft % 60) + 'm.',
              cooldownActive: true
            });
          }
        }
        ...
        try {
          ...
          if (maxDateRaw) {
            ...
            if (blockedDates.length) {
              return res.status(409).json({
                error: '\u26d4 This file includes date(s) already on the dashboard (data exists through ' + maxDateStr + '): ' + blockedDates.slice(0, 5).join(', ') + (blockedDates.length > 5 ? ', \u2026' : '') + '. Sub-admins can only upload NEW dates after ' + maxDateStr + ' \u2014 existing dates can\u2019t be overwritten this way. Ask a super-admin if old data genuinely needs replacing.',
                blockedDates: blockedDates, cutoffDate: maxDateStr, subadminDateBlock: true
              });
            }
          }
    ...
    if (req.user.role === 'subadmin') {
      try {
        await pool.query(
          `INSERT INTO upload_cooldowns (user_id, endpoint, last_upload_at) VALUES ($1,'rejection',NOW())
