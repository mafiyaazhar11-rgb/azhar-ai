...
app.post('/api/delivery/classify', requireAuth, requireRole('superadmin','subadmin'), upload.single('file'), async function(req, res) {
  try {
    ...
    var skippedOrg = 0, skippedStatus = 0;
    ...
    data.forEach(function(row) {
      ...
      if (month) {
        if (!monthData[month]) monthData[month] = { scheduled:0, oos:0, noSched:0, total:0, cat:{ Ambient:{scheduled:0,oos:0,noSched:0,total:0}, Frozen:{scheduled:0,oos:0,noSched:0,total:0} } };
    ...
    var summary = {
      ...
      skippedOrg:skippedOrg, skippedStatus:skippedStatus,
    ...
    console.log('DS Classify complete:', total, 'orders — Scheduled:', sp + '%', 'OOS:', op + '%', 'NoSched:', np + '%',
      '| Excluded — org:', skippedOrg, 'status:', skippedStatus, 'of', data.length, 'rows in file');
...
app.delete('/api/delivery/reset-all', requireAuth, requireRole('superadmin','subadmin'), async function(req, res) {
  try {
    await pool.query('DELETE FROM delivery_schedule');
    await pool.query('DELETE FROM delivery_data');
    deliveryScheduleLookup = null;
