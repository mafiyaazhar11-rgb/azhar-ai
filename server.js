  ...
  try {
    ...
    var result = await pool.query(
      `SELECT customer_name,
              COUNT(*) FILTER (WHERE UPPER(status) IN ('REJECTION','REJECTED','R/D','HOLD','RD')) AS rejected_count,
...
app.get('/api/rejection/customer-detail', requireAuth, async function(req, res) {
  try {
    ...
    if (!rows.length) return res.json({ hasData: false, customer: name });

    function isRejStatus(s) { s = String(s || '').toUpperCase(); return s === 'REJECTION' || s === 'REJECTED' || s === 'R/D' || s === 'HOLD' || s === 'RD'; }
    function isDelStatus(s) { s = String(s || '').toUpperCase(); return s === 'DELIVERED' || s.indexOf('DELIVER') >= 0 || s === 'D'; }

    var totalRej = 0, totalDel = 0, valueAtRisk = 0, foodCount = 0, nonfoodCount = 0, extCount = 0, intCount = 0;
    var byOrg = {}, byMonth = {}, byRoot = {}, recent = [];

    rows.forEach(function(r) {
      var rej = isRejStatus(r.status), del = isDelStatus(r.status);
      var val = parseFloat(r.value) || 0;
      var org = r.org || 'UNKNOWN';
      var mo = r.month_key || 'Unknown';

      if (!byOrg[org]) byOrg[org] = { org: org, rejected: 0, delivered: 0, value: 0 };
      if (!byMonth[mo]) byMonth[mo] = { month: mo, rejected: 0, delivered: 0, value: 0 };

      if (rej) {
        totalRej++; valueAtRisk += val;
        if (r.is_food) foodCount++; else if (r.is_nonfood) nonfoodCount++;
        var srcU = String(r.source || '').toUpperCase();
        if (srcU === 'EXTERNAL') extCount++; else if (srcU === 'INTERNAL') intCount++;

        byOrg[org].rejected++; byOrg[org].value += val;
        byMonth[mo].rejected++; byMonth[mo].value += val;
        ...
        if (recent.length < 60) {
          recent.push({
            date: r.entry_date, order_no: r.order_no, org: org, root_cause: root,
            value: val, area: r.area, address: r.address, source: srcU || null
          });
        }
      } else if (del) {
        totalDel++;
        byOrg[org].delivered++;
        byMonth[mo].delivered++;
      }
    });

    var totalOrders = totalRej + totalDel;
    var rejectionRate = totalOrders > 0 ? (totalRej / totalOrders * 100) : 0;

    var orgList = Object.keys(byOrg).map(function(k) { return byOrg[k]; }).sort(function(a, b) { return b.rejected - a.rejected; });
    var monthList = Object.keys(byMonth).map(function(k) { return byMonth[k]; }).sort(function(a, b) { return a.month < b.month ? -1 : (a.month > b.month ? 1 : 0); });
    var rootList = Object.keys(byRoot).map(function(k) {
      return { root_cause: k, count: byRoot[k], pct: totalRej ? (byRoot[k] / totalRej * 100) : 0 };
    ...
    res.json({
      ...
      foodSplit: { food: foodCount, nonfood: nonfoodCount },
      sourceSplit: { internal: intCount, external: extCount },
      byOrg: orgList,
      byMonth: monthList,
