...
async function initDB() {
  try {
    ...
    // CREATE TABLE IF NOT EXISTS only fires for a table that isn't there yet
    // — it silently does nothing to a table that already exists with an
    // older column set. This table shipped once already without entry_date,
    // so an explicit migration is needed to add it to any DB that already
    // has the table (safe / idempotent — no-ops once the column is there).
    await pool.query(`ALTER TABLE returns_rows ADD COLUMN IF NOT EXISTS entry_date DATE`);
    await pool.query(`CREATE INDEX IF NOT EXISTS idx_returns_rows_date ON returns_rows(entry_date)`);
    ...
    await pool.query(`ALTER TABLE order_tracking ADD COLUMN IF NOT EXISTS vehicle_no TEXT`);
    ...
    // Driver Master — resolves raw driver identifiers that show up in dispatch files as
    // a bare employee code (e.g. "E111640") instead of a name, using a source file
    // (Employee ID + Name) uploaded once. Used by the Driver Rejection Tracker to club
    // every rejection/order under the driver's real name instead of leaving them split
    // across a code and a name as if they were two different people.
    await pool.query(`CREATE TABLE IF NOT EXISTS driver_master (
      driver_code TEXT PRIMARY KEY,
      driver_name TEXT NOT NULL,
      updated_at TIMESTAMPTZ DEFAULT NOW()
    )`);
    // Driver Fleet Override — some dispatch files label a driver "Hired Driver-Asghar
    // 568052636" right in the raw data even when that person is actually in-house
    // staff (a process/template mistake on the transport team's side, not something
    // this app can infer from the text alone). This table lets a specific driver be
    // manually corrected to the right fleet type, keyed by the exact raw identifier
    // as it appears in the dispatch file, so the override survives however that name
    // gets resolved/displayed elsewhere.
    await pool.query(`CREATE TABLE IF NOT EXISTS driver_fleet_override (
...
function extractOrderRowsFromRows(rows, sourceLabel) {
  ...
  var C = {
    ...
    location: findCol('LOCATION_ID', 'LOCATION'),
    truckType: findCol('TRUCK TYPE', 'TRUCK_TYPE', 'VEHICLE TYPE', 'VEHICLE_TYPE', 'DROP TYPE', 'DROP_TYPE'),
    vehicleNo: findCol('VEHICLE NO', 'VEHICLE_NO', 'VEHICLE PLATE', 'VEHICLE_PLATE', 'PLATE NO', 'PLATE_NO', 'VEHICLE')
  };
  ...
  rows.forEach(function(row) {
    ...
    out.push({
      ...
      vehicle_no: C.vehicleNo ? toStr(row[C.vehicleNo]) : ''
    });
  });
  return out;
}
  ...
  try {
    ...
    for (var i = 0; i < orderRows.length; i += CHUNK) {
      ...
      chunk.forEach(function(r) {
        phs.push('($' + idx + ',$' + (idx+1) + ',$' + (idx+2) + ',$' + (idx+3) + ',$' + (idx+4) + ',$' + (idx+5) + ',$' + (idx+6) + ',$' + (idx+7) + ',$' + (idx+8) + ',$' + (idx+9) + ',$' + (idx+10) + ',$' + (idx+11) + ',$' + (idx+12) + ')');
        vals.push(r.order_code, dateKey, r.customer, r.value, r.route, r.org, r.drop_type, r.temperature, r.source_label || '', r.city || '', r.location_id || '', r.truck_type || '', r.vehicle_no || '');
        idx += 13;
      });
      await pool.query('INSERT INTO order_tracking (order_code, date_key, customer, value, route, org, drop_type, temperature, source_label, city, location_id, truck_type, vehicle_no) VALUES ' + phs.join(','), vals);
    }
    return true;
  } catch(e) {
...
// ============================================================
// DRIVER NAME RESOLUTION — resolves a bare employee code (e.g. "E111640") that
// sometimes shows up in dispatch files instead of a driver's name. General Info is
// deliberately NOT used as a source here — it's an incomplete, still-being-built
// file, not a reliable master. Only Dispatch and Rejection YTD data are used for
// the tracker's actual matching logic; this override table is a small optional
// manual mapping (code -> real name) for display purposes only, uploaded once if
// wanted — nothing about it feeds the rejection/driver matching itself.
// ============================================================
var driverMasterMap = {}; // manual-override: driver_code (uppercased) -> real driver_name
async function refreshDriverMasterMap() {
  try {
    var r = await pool.query('CREATE TABLE IF NOT EXISTS driver_master (driver_code TEXT PRIMARY KEY, driver_name TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())').then(function(){
      return pool.query('SELECT driver_code, driver_name FROM driver_master');
    });
    var m = {};
    r.rows.forEach(function (row) { m[String(row.driver_code).trim().toUpperCase()] = row.driver_name; });
    driverMasterMap = m;
  } catch (e) { console.error('Driver Master: could not load override map:', e.message); }
}
refreshDriverMasterMap();
// Resolves a raw driver identifier (name OR bare code) to its real name — checks
// only the manual driver_master override (never General Info); falls back to the
// raw value unchanged if there's no override on file for it.
function resolveDriverName(raw) {
  try {
    var key = String(raw || '').trim().toUpperCase();
    if (driverMasterMap[key]) return driverMasterMap[key];
    return raw;
  } catch (e) { return raw; }
}
// ============================================================
// DRIVER FLEET OVERRIDE — manually correct a specific driver's fleet type when the
// source dispatch file's own label was wrong (e.g. "Hired Driver-Asghar 568052636"
// for someone who's actually in-house staff). Keyed by the exact raw driver
// identifier as it appears in the dispatch file (case/whitespace-insensitive).
// ============================================================
var driverFleetOverrideMap = {}; // normalized raw driver key -> 'inhouse' | 'hired'
async function refreshDriverFleetOverrideMap() {
  try {
    var r = await pool.query('CREATE TABLE IF NOT EXISTS driver_fleet_override (driver_key TEXT PRIMARY KEY, fleet_type TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())').then(function(){
      return pool.query('SELECT driver_key, fleet_type FROM driver_fleet_override');
    });
    var m = {};
    r.rows.forEach(function (row) { m[String(row.driver_key).trim().toUpperCase()] = row.fleet_type; });
    driverFleetOverrideMap = m;
  } catch (e) { console.error('Driver Fleet Override: could not load map:', e.message); }
}
refreshDriverFleetOverrideMap();
// Returns true/false to force isHired, or null if there's no override for this raw name.
function getFleetOverride(rawName) {
  var key = String(rawName || '').trim().toUpperCase();
  var t = driverFleetOverrideMap[key];
  if (t === 'inhouse') return false;
  if (t === 'hired') return true;
  return null;
}
app.post('/api/driver-fleet-override/set', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
  try {
    var driverKey = String(req.body.driver_key || '').trim();
    var fleetType = String(req.body.fleet_type || '').trim().toLowerCase();
    if (!driverKey) return res.status(400).json({ error: 'driver_key is required' });
    if (fleetType !== 'inhouse' && fleetType !== 'hired') return res.status(400).json({ error: 'fleet_type must be "inhouse" or "hired"' });
    await pool.query(
      `INSERT INTO driver_fleet_override (driver_key, fleet_type) VALUES ($1,$2)
       ON CONFLICT (driver_key) DO UPDATE SET fleet_type=$2, updated_at=NOW()`,
      [driverKey.toUpperCase(), fleetType]);
    await refreshDriverFleetOverrideMap();
    await auditLog(req.user.uid, req.user.username, 'DRIVER_FLEET_OVERRIDE_SET', driverKey + ' -> ' + fleetType, '');
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/driver-fleet-override/list', requireAuth, async function (req, res) {
  try {
    var r = await pool.query('SELECT driver_key, fleet_type, updated_at FROM driver_fleet_override ORDER BY updated_at DESC');
    res.json({ overrides: r.rows });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.delete('/api/driver-fleet-override/:key', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
  try {
    await pool.query('DELETE FROM driver_fleet_override WHERE driver_key=$1', [String(req.params.key).trim().toUpperCase()]);
    await refreshDriverFleetOverrideMap();
    res.json({ success: true });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.post('/api/driver-master/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
  try {
    if (!req.file) return res.status(400).json({ error: 'No file uploaded' });
    ...
    // Flexible header detection — accepts common variants of "code" and "name" columns.
    var cols = Object.keys(rows[0]);
    function findCol() {
      var names = Array.prototype.slice.call(arguments);
      return cols.find(function (c) { return names.some(function (n) { return c.trim().toUpperCase() === n.toUpperCase(); }); }) || null;
    }
    var codeCol = findCol('Employee ID', 'Employee Code', 'Driver Code', 'Driver ID', 'Emp ID', 'Emp Code', 'Code', 'ID');
    var nameCol = findCol('Driver Name', 'Employee Name', 'Name', 'Full Name');
    if (!codeCol || !nameCol) {
      return res.status(400).json({ error: 'Could not find a code column and a name column in this file. Expected headers like "Employee ID" and "Driver Name".' });
    }
    ...
    for (var i = 0; i < rows.length; i++) {
      ...
      var existing = await pool.query('SELECT driver_code FROM driver_master WHERE driver_code=$1', [code.toUpperCase()]);
      if (existing.rows.length) {
        await pool.query('UPDATE driver_master SET driver_name=$1, updated_at=NOW() WHERE driver_code=$2', [name, code.toUpperCase()]);
        updated++;
      } else {
        await pool.query('INSERT INTO driver_master (driver_code, driver_name) VALUES ($1,$2)', [code.toUpperCase(), name]);
        inserted++;
      }
    }
    await refreshDriverMasterMap();
    await auditLog(req.user.uid, req.user.username, 'DRIVER_MASTER_UPLOAD', 'inserted=' + inserted + ' updated=' + updated + ' skipped=' + skipped, '');
    res.json({ success: true, inserted: inserted, updated: updated, skipped: skipped, total_rows: rows.length });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/driver-master/status', requireAuth, async function (req, res) {
  try {
    var r = await pool.query('CREATE TABLE IF NOT EXISTS driver_master (driver_code TEXT PRIMARY KEY, driver_name TEXT NOT NULL, updated_at TIMESTAMPTZ DEFAULT NOW())').then(function(){
      return pool.query('SELECT COUNT(*)::int AS cnt FROM driver_master');
    });
    res.json({ manual_override_codes: r.rows[0].cnt });
  } catch (e) { res.status(500).json({ error: e.message }); }
});

// ============================================================
// DRIVER REJECTION TRACKER — which IN-HOUSE driver's orders get rejected most,
// Daily / MTD / YTD, in-house vs hired compared, plus same-day-return detection
// (a rejected order that shows up dispatched again on that SAME date — i.e. it went
// back out and was accepted before end of day, rather than sitting rejected).
//
// Matches rejection_rows.order_no against the order_code already captured per driver
// in dispatch_data.summary.driver_source_split (inhouse_driver_details / hired_driver_
// details -> order_details -> order_code) — same exact-match approach already used by
// Transport Cost Control's Rejection -> Re-delivery section. If Rejection YTD's order
// numbers use a different scheme than the dispatch file's Order Code, this will show
// few or no matches rather than wrong numbers (same caveat as that existing feature).
// ============================================================
function resolveDriverRejPeriod(q) {
  var today = new Date().toISOString().split('T')[0];
  var period = q.period || 'daily';
  if (period === 'custom') {
    var year = parseInt(q.year) || parseInt(today.slice(0, 4));
    var months = (q.months || '').split(',').map(function (m) { return parseInt(m); }).filter(function (m) { return m >= 1 && m <= 12; });
    var days = (q.days || '').split(',').map(function (d) { return parseInt(d); }).filter(function (d) { return d >= 1 && d <= 31; });
    if (!months.length) months = [parseInt(today.slice(5, 7))];
    var dates = [];
    months.forEach(function (m) {
      var daysInMonth = new Date(year, m, 0).getDate();
      var applicableDays = days.length ? days.filter(function (d) { return d <= daysInMonth; }) : Array.from({ length: daysInMonth }, function (_, i) { return i + 1; });
      applicableDays.forEach(function (d) {
        var dateStr = year + '-' + String(m).padStart(2, '0') + '-' + String(d).padStart(2, '0');
        if (dateStr <= today) dates.push(dateStr);
      });
    });
    dates.sort();
    var mNames = ['', 'Jan', 'Feb', 'Mar', 'Apr', 'May', 'Jun', 'Jul', 'Aug', 'Sep', 'Oct', 'Nov', 'Dec'];
    var label = months.map(function (m) { return mNames[m]; }).join(', ') + (days.length ? (' (days: ' + days.join(', ') + ')') : '') + ' ' + year;
    return { type: 'custom', dates: dates, label: label, months: months, days: days, year: year };
  }
  if (period === 'mtd') {
    var d = q.date || today;
    return { type: 'mtd', date_from: d.slice(0, 7) + '-01', date_to: d, label: 'Month to Date' };
  }
  if (period === 'ytd') {
    var d2 = q.date || today;
    return { type: 'ytd', date_from: d2.slice(0, 4) + '-01-01', date_to: d2, label: 'Year to Date' };
  }
  var d3 = q.date || today;
  return { type: 'daily', date_from: d3, date_to: d3, label: d3 };
}
async function buildDriverRejectionTracker(dateFrom, dateTo, explicitDates) {
  var isCustomDates = Array.isArray(explicitDates) && explicitDates.length > 0;
  // Fetch dispatch data starting ONE DAY BEFORE each selected date, not from the
  // date itself. Why: a rejection dated D (e.g. the 1st of the month for an MTD
  // view) actually describes dispatch from the day before (per the reporting-lag
  // logic below) — which can fall just outside the selected range/dates. Without
  // this extra day, every rejection reported right at the start of a selected
  // period would come up unmatched purely from a query-window edge, not a real
  // data problem.
  var explicitDatesSet = isCustomDates ? new Set(explicitDates) : null;
  var dispR;
  if (isCustomDates) {
    var expandedDates = new Set(explicitDates);
    explicitDates.forEach(function (ds) {
      var d = new Date(ds + 'T00:00:00'); d.setDate(d.getDate() - 1);
      expandedDates.add(d.toISOString().split('T')[0]);
    });
    dispR = await pool.query(
      `SELECT date_key::text AS date_key, summary FROM dispatch_data WHERE date_key = ANY($1::date[]) ORDER BY date_key ASC`,
      [Array.from(expandedDates)]);
  } else {
    var dispQueryFrom = new Date(dateFrom + 'T00:00:00');
    dispQueryFrom.setDate(dispQueryFrom.getDate() - 1);
    var dispQueryFromKey = dispQueryFrom.toISOString().split('T')[0];
    dispR = await pool.query(
      `SELECT date_key::text AS date_key, summary FROM dispatch_data WHERE date_key >= $1 AND date_key <= $2 ORDER BY date_key ASC`,
      [dispQueryFromKey, dateTo]);
  }
  function isInSelectedPeriod(dateKey) {
    return isCustomDates ? explicitDatesSet.has(dateKey) : (dateKey >= dateFrom);
  }
  var dispatchDatesInPeriod = dispR.rows.filter(function (r) { return isInSelectedPeriod(r.date_key); }).map(function (r) { return r.date_key; });
  var earliestDispatchDate = dispatchDatesInPeriod.length ? dispatchDatesInPeriod[0] : null;
  var latestDispatchDate = dispatchDatesInPeriod.length ? dispatchDatesInPeriod[dispatchDatesInPeriod.length - 1] : null;
  var calendarDaysInPeriod = isCustomDates ? explicitDates.length : (Math.round((new Date(dateTo) - new Date(dateFrom)) / 86400000) + 1);
  // order_code -> EVERY dispatch occurrence across the period (driver, date), not just
  // the last one. An order can be dispatched, rejected, and re-attempted multiple times
  // (2nd/3rd/4th+ attempt) — collapsing to "last one wins" would attribute a rejection
  // to whichever driver happened to touch that code last, not the driver who actually
  // had it on the day it was rejected. Keeping every occurrence lets us pick the right
  // one and also count how many attempts an order actually took.
  var orderCodeOccurrences = {};
  // driver name -> running totals across every date in range. Keyed by resolved name
  // PLUS fleet type together (never just the name) — this is deliberate: if a name
  // resolution ever collided between a hired and an in-house entry, keying by name
  // alone would silently merge two different people into one bucket and let whichever
  // was seen first lock in the fleet classification for both. Keeping them apart here
  // means a bad resolution can misname someone, but can never miscount which fleet
  // they belong to.
  var driverStats = {};
  var daysWithInhouseData = 0, daysWithHiredData = 0;
  var daysWithDispatchDataInPeriod = 0;
  var sampleHiredNamesRaw = [];
  dispR.rows.forEach(function (row) {
    var inSelectedPeriod = isInSelectedPeriod(row.date_key); // excludes the extra boundary day added above
    var dss = (row.summary && row.summary.driver_source_split) || {};
    if (inSelectedPeriod) {
      daysWithDispatchDataInPeriod++;
      if ((dss.inhouse_driver_details || []).length) daysWithInhouseData++;
      if ((dss.hired_driver_details || []).length) daysWithHiredData++;
      if (sampleHiredNamesRaw.length < 8) {
        (dss.hired_driver_details || []).forEach(function (drv) {
          if (drv.name && sampleHiredNamesRaw.length < 8 && sampleHiredNamesRaw.indexOf(drv.name) === -1) sampleHiredNamesRaw.push(drv.name);
        });
      }
    }
    [['inhouse_driver_details', false], ['hired_driver_details', true]].forEach(function (pair) {
      var key = pair[0], isHiredFromSource = pair[1];
      (dss[key] || []).forEach(function (drv) {
        if (!drv.name) return;
        var override = getFleetOverride(drv.name);
        var isHired = override !== null ? override : isHiredFromSource;
        var resolvedName = resolveDriverName(drv.name);
        var bucketKey = resolvedName + '||' + (isHired ? 'H' : 'I');
        // Order-code occurrences are collected from EVERY fetched row (including the
        // boundary day) since they're only used for matching. Driver totals (orders,
        // drops, active dates) are only accumulated for rows actually inside the
        // selected period, so the boundary day never inflates a driver's numbers.
        if (inSelectedPeriod) {
          if (!driverStats[bucketKey]) driverStats[bucketKey] = { name: resolvedName, isHired: isHired, orders: 0, drops: 0, value: 0, dates: {} };
          driverStats[bucketKey].orders += drv.orders || 0;
          driverStats[bucketKey].drops += drv.drops || 0;
          driverStats[bucketKey].value += drv.value || 0;
          driverStats[bucketKey].dates[row.date_key] = true;
        }
        (drv.order_details || []).forEach(function (o) {
          if (o.order_code) {
            var normCode = String(o.order_code).trim().toUpperCase();
            if (normCode) {
              if (!orderCodeOccurrences[normCode]) orderCodeOccurrences[normCode] = [];
              orderCodeOccurrences[normCode].push({ driver: bucketKey, isHired: isHired, date: row.date_key });
            }
          }
        });
      });
  ...
  // ── Rejection totals — matches Rejection YTD's own headline count exactly (same
  // status='rej' + date filter, no order_no requirement here) so the two
  // dashboards never disagree on "how many rejections happened this period." ──
  var rejAllR, rejR;
  if (isCustomDates) {
    rejAllR = await pool.query(
      `SELECT COUNT(*)::int AS cnt FROM rejection_rows WHERE status = 'rej' AND entry_date = ANY($1::date[])`,
      [explicitDates]);
    rejR = await pool.query(
      `SELECT order_no, entry_date::text AS entry_date FROM rejection_rows
       WHERE status = 'rej' AND entry_date = ANY($1::date[]) AND order_no IS NOT NULL AND order_no <> ''`,
      [explicitDates]);
  } else {
    rejAllR = await pool.query(
      `SELECT COUNT(*)::int AS cnt FROM rejection_rows WHERE status = 'rej' AND entry_date >= $1 AND entry_date <= $2`,
      [dateFrom, dateTo]);
    rejR = await pool.query(
      `SELECT order_no, entry_date::text AS entry_date FROM rejection_rows
  ...
  var totalRejectionsAll = rejAllR.rows[0].cnt;
  var totalWithOrderNo = rejR.rows.length;
  var matchedCount = 0;
  var driverRejections = {}; // bucketKey -> { count, sameDayReturns, reAttempted }
  var sampleUnmatchedRejOrderNos = [];
  var sampleDispatchOrderCodes = Object.keys(orderCodeOccurrences).slice(0, 5);
  var totalReAttempted = 0;
  rejR.rows.forEach(function (rej) {
    var normOrderNo = String(rej.order_no).trim().toUpperCase();
    var occurrences = orderCodeOccurrences[normOrderNo];
    if (!occurrences || !occurrences.length) {
      if (sampleUnmatchedRejOrderNos.length < 5) sampleUnmatchedRejOrderNos.push(rej.order_no);
      return;
    }
    matchedCount++;
    // The rejection report for a given date describes the PREVIOUS day's dispatch
    // (today's dispatch report and tomorrow's rejection report cover the same batch
    // of orders) — so the attempt that was actually rejected is the one dispatched on
    // (entry_date - 1 day). Prefer that exact occurrence; if it's missing for some
    // reason, fall back to the earliest recorded occurrence rather than losing the
    // match entirely.
    var dispatchDateForRej = new Date(rej.entry_date + 'T00:00:00');
    dispatchDateForRej.setDate(dispatchDateForRej.getDate() - 1);
    var dispatchDateKey = dispatchDateForRej.toISOString().split('T')[0];
    var sameDateOccs = occurrences.filter(function (o) { return o.date === dispatchDateKey; });
    var primaryOcc = sameDateOccs[0] || occurrences[0];
    var bucketKeyMatched = primaryOcc.driver;
    if (!driverRejections[bucketKeyMatched]) driverRejections[bucketKeyMatched] = { count: 0, sameDayReturns: 0, reAttempted: 0 };
    driverRejections[bucketKeyMatched].count++;
    // Same-day return: more than one dispatch occurrence of this order code recorded
    // on the exact day it was rejected — it went back out and was accepted before end
    // of day, not carried over.
    if (sameDateOccs.length > 1) driverRejections[bucketKeyMatched].sameDayReturns++;
    // Re-attempted (any number of times, including a 2nd/3rd/4th+ attempt on a LATER
    // date, not just same-day): this order code has more than one dispatch occurrence
    // total across the whole period, on top of the one that got rejected.
    if (occurrences.length > 1) { driverRejections[bucketKeyMatched].reAttempted++; totalReAttempted++; }
  });
  var inhouseRows = [], inhouseAgg = { orders: 0, drops: 0, value: 0, rejections: 0, same_day_returns: 0, re_attempted: 0, driver_count: 0 };
  var hiredAgg = { orders: 0, drops: 0, value: 0, rejections: 0, same_day_returns: 0, re_attempted: 0, driver_count: 0 };
  Object.keys(driverStats).forEach(function (bucketKey) {
    var stat = driverStats[bucketKey];
    var rej = driverRejections[bucketKey] || { count: 0, sameDayReturns: 0, reAttempted: 0 };
    var agg = stat.isHired ? hiredAgg : inhouseAgg;
    agg.orders += stat.orders; agg.drops += stat.drops; agg.value += stat.value; agg.rejections += rej.count;
    agg.same_day_returns += rej.sameDayReturns; agg.re_attempted += rej.reAttempted; agg.driver_count++;
    if (!stat.isHired) {
      var daysActive = Object.keys(stat.dates).length;
      var rejPct = stat.orders ? +((rej.count / stat.orders) * 100).toFixed(2) : 0;
      inhouseRows.push({
        driver: stat.name, orders: stat.orders, drops: stat.drops, days_active: daysActive,
        avg_drops_per_day: daysActive ? +(stat.drops / daysActive).toFixed(1) : 0,
        avg_value_per_drop: stat.drops ? +(stat.value / stat.drops).toFixed(0) : 0,
        rejections: rej.count,
        rejection_rate_pct: rejPct,
        delivered_pct: +(100 - rejPct).toFixed(2),
        same_day_returns: rej.sameDayReturns,
        re_attempted: rej.reAttempted
      });
    }
  });
  inhouseAgg.avg_value_per_drop = inhouseAgg.drops ? +(inhouseAgg.value / inhouseAgg.drops).toFixed(0) : 0;
  hiredAgg.avg_value_per_drop = hiredAgg.drops ? +(hiredAgg.value / hiredAgg.drops).toFixed(0) : 0;
  inhouseAgg.rejection_rate_pct = inhouseAgg.orders ? +((inhouseAgg.rejections / inhouseAgg.orders) * 100).toFixed(2) : 0;
  hiredAgg.rejection_rate_pct = hiredAgg.orders ? +((hiredAgg.rejections / hiredAgg.orders) * 100).toFixed(2) : 0;
  inhouseAgg.delivered_pct = +(100 - inhouseAgg.rejection_rate_pct).toFixed(2);
  hiredAgg.delivered_pct = +(100 - hiredAgg.rejection_rate_pct).toFixed(2);
  inhouseRows.sort(function (a, b) { return b.rejections - a.rejections || b.rejection_rate_pct - a.rejection_rate_pct; });
  return {
    inhouse_summary: inhouseAgg, hired_summary: hiredAgg, inhouse_drivers: inhouseRows,
    // The full funnel, so a low match count is never a mystery:
    // total_rejections_all (matches Rejection YTD's own number)
    //   -> total_with_order_no (some rejection rows have no order number captured at all)
    //     -> total_rejections_matched (of those, how many landed on a driver we have dispatch data for)
    total_rejections_all: totalRejectionsAll,
    total_rejections_missing_order_no: totalRejectionsAll - totalWithOrderNo,
    total_rejections_with_order_no: totalWithOrderNo,
    total_rejections_matched: matchedCount,
    total_rejections_unmatched: totalWithOrderNo - matchedCount,
    total_rejections_reattempted: totalReAttempted,
    days_with_dispatch_data: daysWithDispatchDataInPeriod,
    days_with_inhouse_driver_data: daysWithInhouseData,
    days_with_hired_driver_data: daysWithHiredData,
    calendar_days_in_period: calendarDaysInPeriod,
    earliest_dispatch_date: earliestDispatchDate,
    latest_dispatch_date: latestDispatchDate,
    // Real sample values from both sides, so a low match rate can be visually
    // checked instead of taken on faith — if these look like clearly different
    // formats (different length, prefix, suffix), that's the actual explanation.
    sample_rejection_order_numbers: sampleUnmatchedRejOrderNos,
    sample_dispatch_order_codes: sampleDispatchOrderCodes,
    // Raw (unresolved) hired-driver names actually stored for this period — if these
    // look like bare phone numbers instead of real names, that's very likely why an
    // entire period shows 0 in-house / all-hired: the app treats "no name, just a
    // number" as the signal for a hired driver, so an in-house dispatch file that
    // only recorded phone numbers that period would get misclassified wholesale.
    sample_hired_driver_names: sampleHiredNamesRaw
  };
}
app.get('/api/dispatch/driver-rejection-tracker', requireAuth, async function (req, res) {
  try {
    var period = resolveDriverRejPeriod(req.query);
    var data = await buildDriverRejectionTracker(period.date_from, period.date_to, period.dates);
    data.period = period;
    res.json(data);
  } catch (e) { res.status(500).json({ error: e.message }); }
});
app.get('/api/dispatch/driver-rejection-tracker/export', noCache, requireAuth, async function (req, res) {
  try {
    var ExcelJS = require('exceljs');
    var period = resolveDriverRejPeriod(req.query);
    var d = await buildDriverRejectionTracker(period.date_from, period.date_to, period.dates);
    var wb = new ExcelJS.Workbook();
    wb.creator = 'AZHAR-AI'; wb.created = new Date();

    // ── Sheet 1: Summary + full match funnel ──
    var sum = wb.addWorksheet('Summary');
    sum.columns = [{ width: 34 }, { width: 20 }];
    var t = sum.addRow(['DRIVER REJECTION TRACKER']); t.font = { bold: true, size: 14 };
    sum.mergeCells('A' + t.number + ':B' + t.number);
    sum.addRow(['Period', period.label + ' (' + period.date_from + ' to ' + period.date_to + ')']);
    sum.addRow(['Downloaded', new Date().toLocaleString('en-AE')]);
    sum.addRow([]);
    var fh = sum.addRow(['MATCH FUNNEL']); fh.font = { bold: true };
    sum.addRow(['Total rejections this period (matches Rejection YTD)', d.total_rejections_all]);
    sum.addRow(['— missing an order number entirely', d.total_rejections_missing_order_no]);
    sum.addRow(['— with an order number to check', d.total_rejections_with_order_no]);
    sum.addRow(['—— matched to a driver', d.total_rejections_matched]);
    sum.addRow(['—— unmatched (different numbering scheme, or no dispatch data)', d.total_rejections_unmatched]);
    sum.addRow([]);
    var ch = sum.addRow(['DISPATCH DATA COVERAGE']); ch.font = { bold: true };
    sum.addRow(['Calendar days in period', d.calendar_days_in_period]);
    sum.addRow(['Days with dispatch data', d.days_with_dispatch_data]);
    sum.addRow(['Earliest dispatch date on file', d.earliest_dispatch_date || 'n/a']);
    sum.addRow(['Latest dispatch date on file', d.latest_dispatch_date || 'n/a']);
    sum.addRow([]);
    var fleetH = sum.addRow(['FLEET COMPARISON', 'In-House', 'Hired']); fleetH.font = { bold: true };
    sum.addRow(['Rejections', d.inhouse_summary.rejections, d.hired_summary.rejections]);
    sum.addRow(['Rejection Rate %', d.inhouse_summary.rejection_rate_pct, d.hired_summary.rejection_rate_pct]);
    sum.addRow(['Delivered %', d.inhouse_summary.delivered_pct, d.hired_summary.delivered_pct]);
    sum.addRow(['Same-Day Returns', d.inhouse_summary.same_day_returns, d.hired_summary.same_day_returns]);
    sum.addRow(['Re-Attempted (any day)', d.inhouse_summary.re_attempted, d.hired_summary.re_attempted]);
    sum.addRow(['Driver Count', d.inhouse_summary.driver_count, d.hired_summary.driver_count]);
    sum.addRow(['Total Orders', d.inhouse_summary.orders, d.hired_summary.orders]);
    sum.addRow(['Total Drops', d.inhouse_summary.drops, d.hired_summary.drops]);

    // ── Sheet 2: In-house drivers ranked ──
    var ws = wb.addWorksheet('In-House Drivers');
    ws.columns = [{ width: 6 }, { width: 26 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 16 }, { width: 14 }, { width: 14 }, { width: 16 }];
    var hdr = ws.addRow(['#', 'Driver', 'Orders', 'Rejections', 'Rejection Rate %', 'Delivered %', 'Same-Day Returns', 'Re-Attempted', 'Avg Drops/Day', 'Avg Value/Drop (AED)']);
    hdr.font = { bold: true };
    (d.inhouse_drivers || []).forEach(function (row, i) {
      var r = ws.addRow([i + 1, row.driver, row.orders, row.rejections, row.rejection_rate_pct, row.delivered_pct, row.same_day_returns, row.re_attempted, row.avg_drops_per_day, row.avg_value_per_drop]);
      r.getCell(5).numFmt = '0.00"%"'; r.getCell(6).numFmt = '0.00"%"'; r.getCell(10).numFmt = '#,##0';
      if (row.rejection_rate_pct >= 10) { r.getCell(4).font = { color: { argb: 'FFE05C5C' }, bold: true }; r.getCell(5).font = { color: { argb: 'FFE05C5C' }, bold: true }; }
      else if (row.rejection_rate_pct >= 5) { r.getCell(5).font = { color: { argb: 'FFC9A84C' }, bold: true }; }
    });

    var buf = await wb.xlsx.writeBuffer();
    res.setHeader('Content-Disposition', 'attachment; filename="Driver_Rejection_Tracker_' + period.type + '_' + period.date_to + '.xlsx"');
    res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
    res.send(Buffer.from(buf));
  } catch (e) { res.status(500).json({ error: 'Export failed: ' + e.message }); }
});
...
// ============================================================
// COLUMN DIAGNOSTIC — for a given date, shows exactly which raw column header the
// dispatch parser picked for "driver," every other header available in that file,
// and a handful of real raw values from that column with what they'd resolve to
// (name vs "Hired Driver (ID: ...)"). Built specifically to answer: is a period
// showing all-hired/no-in-house because the upload that day used a different header
// name than usual (so the parser missed the real name column and fell back to a
// phone-number-only column), or because the file genuinely only ever had numbers?
// Same findCol() fuzzy-substring logic as the real dispatch parser, so what this
// reports is exactly what a real upload would have used, not an approximation.
// ============================================================
app.get('/api/dispatch/column-diagnostic', requireAuth, async function (req, res) {
  try {
    var dateKey = req.query.date;
    if (!dateKey) return res.status(400).json({ error: 'date is required (YYYY-MM-DD)' });
    var sourcesRes = await pool.query('SELECT source_label, rows_json FROM dispatch_data_sources WHERE date_key=$1 ORDER BY source_label', [dateKey]);
    if (!sourcesRes.rows.length) {
      return res.json({ found: false, note: 'No raw dispatch rows stored for this date (this date may predate the multi-source upload feature, or nothing was uploaded).' });
    }
    var results = sourcesRes.rows.map(function (src) {
      var rows = src.rows_json;
      if (!rows || !rows.length) return { source_label: src.source_label, headers: [], driver_column: null, samples: [] };
      var headers = Object.keys(rows[0]);
      function findCol() {
        var names = Array.prototype.slice.call(arguments);
        return headers.find(function (k) { return names.some(function (n) { return k.toUpperCase().indexOf(n.toUpperCase()) !== -1; }); }) || null;
      }
      var driverCol = findCol('DRIVER CONTACT DETAILS', 'DRIVERS NAME', 'DRIVER NAME', 'DRIVER CONTACT', 'DRIVER_CONTACT', 'DRIVER_ID');
      var samples = [];
      if (driverCol) {
        var seen = {};
        for (var i = 0; i < rows.length && samples.length < 8; i++) {
          var raw = rows[i][driverCol];
          if (raw === null || raw === undefined || raw === '') continue;
          var rawStr = String(raw).trim();
          if (seen[rawStr]) continue;
          seen[rawStr] = true;
          var extracted = extractDriverName(rawStr);
          samples.push({ raw_value: rawStr, extracted_name: extracted, would_be_classified_as: /^hired driver/i.test(extracted) ? 'Hired' : 'In-House' });
        }
      }
      return { source_label: src.source_label, headers: headers, driver_column_detected: driverCol, samples: samples, row_count: rows.length };
    });
    res.json({ found: true, date: dateKey, sources: results });
  } catch (e) { res.status(500).json({ error: e.message }); }
});
...
// Parses the M/D/YYYY-style date string the frontend already extracts
// per row (dateStr) into a real DATE. Falls back to null (row still
// stores fine, just won't be covered by per-date replace logic).
function parseReturnsDateStr(s) {
  s = toStr(s);
  if (!s) return null;
...
// Rebuilds the returns_data summary from EVERY row currently in
// returns_rows (all uploads combined) — mirrors buildReturnsSummary() on
// the frontend. Called after any incremental upload so the dashboard
// always reflects the full cumulative dataset, not just whichever file
// was uploaded most recently (same principle as rejection_rows' aggregate
// rebuild).
async function rebuildReturnsAggregate() {
  var allRes = await pool.query('SELECT * FROM returns_rows');
  var rows = allRes.rows || [];
  var FOOD_WHS = ['DCV', 'DCF'], NF_WHS = ['DGC', 'DGS', 'DGN', 'DSN'];
  ...
  rows.forEach(function(r) {
    var tot = parseFloat(r.value) || 0;
    var cat = r.category, bu = r.bu, wh = r.warehouse, month = r.month, day = r.day, area = r.area, cust = r.customer, reason = r.reason;
    totalVal += tot;
    if (cat === 'Invoice Cancellation') { invCount++; invVal += tot; }
    else if (cat === 'GRV-Return') { grvCount++; grvVal += tot; }
    if (wh) {
      whMap[wh] = (whMap[wh] || 0) + tot;
      if (FOOD_WHS.indexOf(wh) !== -1) whFoodMap[wh] = (whFoodMap[wh] || 0) + tot;
      if (NF_WHS.indexOf(wh) !== -1) whNFMap[wh] = (whNFMap[wh] || 0) + tot;
    }
    if (cat) { if (!catMap[cat]) catMap[cat] = { c: 0, v: 0 }; catMap[cat].c++; catMap[cat].v += tot; }
    if (bu) buMap[bu] = (buMap[bu] || 0) + tot;
    if (month) {
      if (!monthMap[month]) monthMap[month] = { val: 0, count: 0, food: 0, nonfood: 0 };
      monthMap[month].val += tot; monthMap[month].count++;
      if (bu === 'Food') monthMap[month].food += tot;
      else if (bu === 'Non-Food') monthMap[month].nonfood += tot;
    }
    if (month && day) {
      var dk = (MNAMES[month] || month) + ' ' + day;
      if (!dateMap[dk]) dateMap[dk] = { val: 0, count: 0, month: month, day: day };
      dateMap[dk].val += tot; dateMap[dk].count++;
    }
    if (area) areaMap[area] = (areaMap[area] || 0) + tot;
    if (cust) custMap[cust] = (custMap[cust] || 0) + tot;
    if (reason) { if (!reasonMap[reason]) reasonMap[reason] = { c: 0, v: 0 }; reasonMap[reason].c++; reasonMap[reason].v += tot; }
  });
  return {
    version: 'v3', totalVal: totalVal, invCount: invCount, invVal: invVal, grvCount: grvCount, grvVal: grvVal,
    whMap: whMap, whFoodMap: whFoodMap, whNFMap: whNFMap, catMap: catMap, buMap: buMap,
    monthMap: monthMap, dateMap: dateMap, areaMap: areaMap, custMap: custMap, reasonMap: reasonMap,
    totalOrders: rows.length
  };
}

app.post('/api/returns/upload', requireAuth, requireRole('superadmin', 'subadmin'), async function(req, res) {
  try {
    var uploadedBy = req.body.uploadedBy || req.user.username || 'Admin';
    ...
    if (!rawRows.length) return res.status(400).json({ error: 'No rows in upload' });

    // ── Per-date storage, same principle as Rejection: figure out the
    // exact set of dates THIS file covers and replace only those dates in
    // returns_rows. Uploading June, then later July, accumulates — it
    // does not wipe June. Rows whose date can't be parsed fall back to
    // being appended (never overwritten by date), so nothing silently
    // vanishes just because a date was unreadable. ──
    var newDatesSet = {};
    rawRows.forEach(function(r) {
      var ds = parseReturnsDateStr(r.dateStr);
      if (ds) newDatesSet[ds] = true;
    });
    var newDatesArr = Object.keys(newDatesSet).sort();

    if (newDatesArr.length) {
      await pool.query('DELETE FROM returns_rows WHERE entry_date = ANY($1::date[])', [newDatesArr]);
    }
    ...
    for (var ci = 0; ci < rawRows.length; ci += chunkSize) {
      ...
      await pool.query(
        'INSERT INTO returns_rows (entry_date, warehouse, category, bu, value, month, day, date_str, customer, area, order_no, reason) VALUES ' + vals.join(','),
        params
      );
      rowsSaved += chunk.length;
    }

    // Rebuild the cached summary from the FULL cumulative dataset (all
    // uploads combined), not just this file — this is what the dashboard
    // and export both read.
    var summary = await rebuildReturnsAggregate();
    returnsData = {
    ...
    var dbOk = await dbSaveReturns(uploadedBy, fileName, summary.totalOrders, summary);
    saveJSON(RETURNS_FILE, returnsData);
    auditLog(req.user.uid, req.user.username, 'RETURNS_UPLOAD', fileName + ' — ' + rowsSaved + ' rows, dates: ' + (newDatesArr.slice(0,5).join(', ') || 'n/a') + (newDatesArr.length > 5 ? ', …' : ''), req.headers['x-forwarded-for'] || req.ip || '');
    console.log('Returns saved:', rowsSaved, 'rows for', newDatesArr.length, 'date(s),', summary.totalOrders, 'total rows on file', dbOk ? '(DB+file)' : '(file only)');
    res.json({ success: true, rowsSaved: rowsSaved, totalOnFile: summary.totalOrders, datesCovered: newDatesArr });
  } catch(e) {
...
// ── MARKET RETURNS: raw row-level data, in the same shape the frontend's
// own upload parser produces (wh/cat/bu/tot/month/day/dateStr/cust/area/
// orderNo/reason). Fetched on page load so filtering (Food/Non-Food, month,
// warehouse, category, area — in any combination) always runs against exact
// per-row matches, the same as right after an upload, instead of a plain
// page reload falling back to an approximate ratio-based estimate that
// breaks down on combined filters. requireAuth only (not role-gated) since
// this is read-only and every logged-in user can already see the dashboard
// numbers this data underlies. ──
app.get('/api/returns/raw-rows', requireAuth, async function(req, res) {
  try {
    var r = await pool.query('SELECT warehouse, category, bu, value, month, day, date_str, customer, area, order_no, reason FROM returns_rows');
    var rows = r.rows.map(function(row) {
      return {
        wh: row.warehouse, cat: row.category, bu: row.bu,
        tot: parseFloat(row.value) || 0, month: row.month, day: row.day,
        dateStr: row.date_str, cust: row.customer, area: row.area,
        orderNo: row.order_no, reason: row.reason
      };
    });
    res.json({ rows: rows });
  } catch (e) {
    console.error('Returns raw-rows error:', e.message);
...
app.get('/api/returns/export-excel', requireAuth, async function(req, res) {
  try {
    var rowsRes = await pool.query('SELECT * FROM returns_rows ORDER BY entry_date NULLS LAST, month, day');
    ...
    // Full aggregate computed up front so every formula cell below can
    // carry a cached `result` alongside its formula. Without a cached
    // result, ExcelJS writes a formula with nothing to display until Excel
    // recalculates it — and Excel deliberately does NOT recalculate while
    // a downloaded file is in Protected View, so every formula cell shows
    // blank until the person clicks "Enable Editing". Attaching the actual
    // computed value here means the report displays correctly immediately,
    // Protected View or not — the live formula is still there underneath
    // for when the file is edited or a row is corrected.
    var totalVal = 0, invCount = 0, grvCount = 0, totalOrders = rows.length;
    var catValMap = {}, buValMap = { 'Food': 0, 'Non-Food': 0, 'Zooz': 0 };
    // BU x Category cross-tab — how much of each Food/Non-Food/Zooz total
    // is Invoice Cancellation vs GRV-Return.
    var buCatMap = { 'Food': { inv: 0, grv: 0 }, 'Non-Food': { inv: 0, grv: 0 }, 'Zooz': { inv: 0, grv: 0 } };
    var whAgg = {}, custAgg = {}, monthAgg = {}, areaMap = {}, reasonMap = {};
    rows.forEach(function(r) {
      var v = parseFloat(r.value) || 0;
      totalVal += v;
      if (r.category === 'Invoice Cancellation') invCount++;
      else if (r.category === 'GRV-Return') grvCount++;
      catValMap[r.category] = (catValMap[r.category] || 0) + v;
      if (buValMap.hasOwnProperty(r.bu)) buValMap[r.bu] += v;
      if (buCatMap.hasOwnProperty(r.bu)) {
        if (r.category === 'Invoice Cancellation') buCatMap[r.bu].inv += v;
        else if (r.category === 'GRV-Return') buCatMap[r.bu].grv += v;
      }
      if (r.customer) {
        if (!custAgg[r.customer]) custAgg[r.customer] = { orders: 0, inv: 0, grv: 0, val: 0, invVal: 0, grvVal: 0 };
        custAgg[r.customer].orders++;
        if (r.category === 'Invoice Cancellation') { custAgg[r.customer].inv++; custAgg[r.customer].invVal += v; }
        else if (r.category === 'GRV-Return') { custAgg[r.customer].grv++; custAgg[r.customer].grvVal += v; }
        custAgg[r.customer].val += v;
      }
      if (r.warehouse) {
        if (!whAgg[r.warehouse]) whAgg[r.warehouse] = { orders: 0, inv: 0, grv: 0, val: 0, invVal: 0, grvVal: 0 };
        whAgg[r.warehouse].orders++;
        if (r.category === 'Invoice Cancellation') { whAgg[r.warehouse].inv++; whAgg[r.warehouse].invVal += v; }
        else if (r.category === 'GRV-Return') { whAgg[r.warehouse].grv++; whAgg[r.warehouse].grvVal += v; }
        whAgg[r.warehouse].val += v;
      }
      if (r.month) {
        if (!monthAgg[r.month]) monthAgg[r.month] = { food: 0, nonfood: 0, zooz: 0, total: 0, foodOrd: 0, nfOrd: 0, zoozOrd: 0, totalOrd: 0, inv: 0, grv: 0, invOrd: 0, grvOrd: 0 };
        var ma = monthAgg[r.month];
        ma.total += v; ma.totalOrd++;
        if (r.bu === 'Food') { ma.food += v; ma.foodOrd++; }
        else if (r.bu === 'Non-Food') { ma.nonfood += v; ma.nfOrd++; }
        else if (r.bu === 'Zooz') { ma.zooz += v; ma.zoozOrd++; }
        if (r.category === 'Invoice Cancellation') { ma.inv += v; ma.invOrd++; }
        else if (r.category === 'GRV-Return') { ma.grv += v; ma.grvOrd++; }
      }
      if (r.area) areaMap[r.area] = (areaMap[r.area] || 0) + v;
      if (r.reason) reasonMap[r.reason] = (reasonMap[r.reason] || 0) + v;
    });
    var monthSet = {};
    Object.keys(monthAgg).forEach(function(m) { monthSet[m] = true; });
    var months = Object.keys(monthSet).map(Number).sort(function(a, b) { return a - b; });
    var whKeys = Object.keys(whAgg).sort(function(a, b) { return whAgg[b].val - whAgg[a].val; });
    var whTotalVal = whKeys.reduce(function(s, w) { return s + whAgg[w].val; }, 0);
    ...
    function pct(v, denom) { return denom > 0 ? (v / denom) : 0; }

    var GOLD = 'FFC9A84C', DARKBG = 'FF000000', LIGHTGOLD = 'FFF5E9C8', BORDER_COLOR = 'FFD0D0D0';
    var THIN = { style: 'thin', color: { argb: BORDER_COLOR } };
    var CELL_BORDER = { top: THIN, left: THIN, bottom: THIN, right: THIN };
    function borderRow(row, colCount) {
      for (var i = 1; i <= colCount; i++) row.getCell(i).border = CELL_BORDER;
    }
    function styleHeaderRow(row, colCount) {
      row.eachCell(function(cell) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } };
        cell.font = { bold: true, color: { argb: GOLD }, size: 16 };
        cell.alignment = { vertical: 'middle' };
        cell.border = CELL_BORDER;
      });
      row.height = 26;
    }
    function styleSectionRow(row, span) {
      row.font = { bold: true, color: { argb: DARKBG }, size: 16 };
      row.height = 24;
    ...
    function styleTotalRow(row, colCount) {
      row.eachCell(function(cell) {
        cell.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: LIGHTGOLD } };
        cell.font = { bold: true, color: { argb: DARKBG }, size: 16 };
        cell.border = CELL_BORDER;
      });
      row.height = 22;
    }
    function styleTitleRow(row) { row.font = { bold: true, size: 20, color: { argb: GOLD } }; row.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: DARKBG } }; row.height = 32; row.alignment = { vertical: 'middle' }; }
    // Red data-caveat row — used specifically for the DCV/DGC Jan-Mar warning below,
    // and anywhere else a real classification-boundary issue needs to be called out
    // loudly rather than buried in a quiet gray footnote.
    var JAN_MAR_DCV_WARNING = '⚠ JAN–MAR 2026: Many DCV brands were still classified under Food during this period. Only from APRIL 2026 onward were DCV Non-Food brands moved to DGC. This means Jan/Feb/Mar Food & Non-Food figures are MIXED and run higher than Apr onward for DCV — they are not directly comparable month-to-month across that boundary.';
    function warningRow(sheet, text, span) {
      var r = sheet.addRow([text]);
      r.font = { bold: true, size: 16, color: { argb: 'FFCC0000' } };
      r.height = 34;
    ...
    function noteRow(sheet, text, span) {
      var r = sheet.addRow([text]);
      r.font = { italic: true, size: 16, color: { argb: 'FF444444' } }; r.height = 22;
      sheet.mergeCells('A' + r.number + ':' + span + r.number);
      return r;
    }
    function zebra(row, colCount, isEven) {
      if (!isEven) return;
      for (var i = 1; i <= colCount; i++) row.getCell(i).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F5EF' } };
    }
    ...
    var R_CUST = RMS + '$H$' + RM_START + ':$H$' + RM_END;
    var R_REASON = RMS + '$J$' + RM_START + ':$J$' + RM_END;
    ...
    // ══════════════════════ Sheet 1: Executive Summary ══════════════════════
    var ex = wb.addWorksheet('Executive Summary', { views: [{ showGridLines: false }] });
    ex.columns = [{ width: 30 }, { width: 18 }, { width: 15 }, { width: 14 }, { width: 12 }];
    var exTitle = ex.addRow(['MARKET RETURNS — FULL ANALYZED REPORT']); styleTitleRow(exTitle); ex.mergeCells('A1:E1');
    var exSub = ex.addRow(['AKI Group  —  Invoice Cancellations · GRV Returns · BU-wise  —  Generated ' + genDate]);
    exSub.font = { bold: true, size: 16, color: { argb: 'FF444444' } }; ex.mergeCells('A2:E2'); exSub.height = 22;
    warningRow(ex, JAN_MAR_DCV_WARNING, 'E');
    ex.addRow([]);

    styleSectionRow(ex.addRow(['KEY METRICS', '', '', '', '']), 5);
    var kpiDefs = [
      ['Total Returns Value (AED)', { formula: 'SUM(' + R_VAL + ')', result: totalVal }, '#,##0'],
      ['Invoice Cancellations (orders)', { formula: 'COUNTIF(' + R_CAT + ',"Invoice Cancellation")', result: invCount }, '#,##0'],
      ['GRV Returns (orders)', { formula: 'COUNTIF(' + R_CAT + ',"GRV-Return")', result: grvCount }, '#,##0'],
      ['Total Orders', { formula: 'COUNTA(' + R_MONTH + ')', result: totalOrders }, '#,##0']
    ];
    var totalValRow = null;
    kpiDefs.forEach(function(k, ki) {
      var r0 = ex.addRow([k[0], k[1]]);
      r0.height = 24;
      r0.getCell(1).font = { bold: true, size: 16, color: { argb: DARKBG } };
      r0.getCell(2).font = { bold: true, size: 18, color: { argb: ki === 0 ? 'FFB00000' : DARKBG } };
      r0.getCell(2).numFmt = k[2];
      r0.getCell(2).alignment = { horizontal: 'right' };
      borderRow(r0, 2);
      zebra(r0, 2, ki % 2 === 1);
      if (ki === 0) totalValRow = r0.number;
    });
    ex.addRow([]);

    styleSectionRow(ex.addRow(['FOOD vs NON-FOOD vs ZOOZ — OVERALL SPLIT, WITH INVOICE CANCEL vs GRV BREAKDOWN', '', '', '', '']), 5);
    var buHdr = ex.addRow(['Category', 'Invoice Cancel (AED)', 'GRV Return (AED)', 'Total (AED)', '% of Total']); styleHeaderRow(buHdr, 5);
    var buFirstRow = ex.rowCount + 1;
    ['Food', 'Non-Food', 'Zooz'].forEach(function(b, bidx) {
      var bc = buCatMap[b];
      var bv = buValMap[b] || 0;
      var r0 = ex.addRow([b,
        { formula: 'SUMIFS(' + R_VAL + ',' + R_BU + ',A' + (ex.rowCount + 1) + ',' + R_CAT + ',"Invoice Cancellation")', result: bc.inv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_BU + ',A' + (ex.rowCount + 1) + ',' + R_CAT + ',"GRV-Return")', result: bc.grv },
        { formula: 'SUMIF(' + R_BU + ',A' + (ex.rowCount + 1) + ',' + R_VAL + ')', result: bv },
        '']);
      r0.getCell(2).numFmt = '#,##0'; r0.getCell(3).numFmt = '#,##0'; r0.getCell(4).numFmt = '#,##0';
      borderRow(r0, 5);
      zebra(r0, 5, bidx % 2 === 1);
    });
    var buLastRow = ex.rowCount;
    for (var bi = buFirstRow; bi <= buLastRow; bi++) {
      var buRowVal = buValMap[['Food', 'Non-Food', 'Zooz'][bi - buFirstRow]] || 0;
      ex.getCell('E' + bi).value = { formula: 'IFERROR(D' + bi + '/$B$' + totalValRow + ',0)', result: pct(buRowVal, totalVal) };
      ex.getCell('E' + bi).numFmt = '0.0%';
    }
    var buSumInv = (buCatMap['Food'].inv || 0) + (buCatMap['Non-Food'].inv || 0) + (buCatMap['Zooz'].inv || 0);
    var buSumGrv = (buCatMap['Food'].grv || 0) + (buCatMap['Non-Food'].grv || 0) + (buCatMap['Zooz'].grv || 0);
    var buSumVal = (buValMap['Food'] || 0) + (buValMap['Non-Food'] || 0) + (buValMap['Zooz'] || 0);
    var buTotalRow = ex.addRow(['TOTAL',
      { formula: 'SUM(B' + buFirstRow + ':B' + buLastRow + ')', result: buSumInv },
      { formula: 'SUM(C' + buFirstRow + ':C' + buLastRow + ')', result: buSumGrv },
      { formula: 'SUM(D' + buFirstRow + ':D' + buLastRow + ')', result: buSumVal },
      { formula: 'SUM(E' + buFirstRow + ':E' + buLastRow + ')', result: pct(buSumVal, totalVal) }]);
    buTotalRow.getCell(2).numFmt = '#,##0'; buTotalRow.getCell(3).numFmt = '#,##0'; buTotalRow.getCell(4).numFmt = '#,##0'; buTotalRow.getCell(5).numFmt = '0.0%';
    styleTotalRow(buTotalRow, 5);
    ex.addRow([]);

    styleSectionRow(ex.addRow(['BY CATEGORY', '', '', '']), 4);
    var catHdr2 = ex.addRow(['Category', 'Orders', 'Value (AED)', '% of Total']);
    styleHeaderRow(catHdr2, 4);
    var catFirstRow = ex.rowCount + 1;
    ['Invoice Cancellation', 'GRV-Return'].forEach(function(c, cidx) {
      var cCount = c === 'Invoice Cancellation' ? invCount : grvCount;
      var cVal = catValMap[c] || 0;
      var r0 = ex.addRow([c,
        { formula: 'COUNTIF(' + R_CAT + ',A' + (ex.rowCount + 1) + ')', result: cCount },
        { formula: 'SUMIF(' + R_CAT + ',A' + (ex.rowCount + 1) + ',' + R_VAL + ')', result: cVal }, '']);
      r0.getCell(3).numFmt = '#,##0';
      borderRow(r0, 4);
      zebra(r0, 4, cidx % 2 === 1);
    });
    var catLastRow = ex.rowCount;
    var catOrder = ['Invoice Cancellation', 'GRV-Return'];
    for (var cci = catFirstRow; cci <= catLastRow; cci++) {
      var cVal2 = catValMap[catOrder[cci - catFirstRow]] || 0;
      ex.getCell('D' + cci).value = { formula: 'IFERROR(C' + cci + '/$B$' + totalValRow + ',0)', result: pct(cVal2, totalVal) };
      ex.getCell('D' + cci).numFmt = '0.0%';
    }
    ex.addRow([]);
    noteRow(ex, 'All figures on this tab are calculated live from the Return Master tab and update automatically if any row is corrected.', 'E');

    // ══════════════════════ Sheet 2: Monthly Trend (Food vs Non-Food vs Zooz) ══════════════════════
    // Columns: Month | Food/Non-Food/Zooz/Total AED (grouped, with data
    // bars for an at-a-glance chart look) | Food/Non-Food/Zooz % share |
    // Food/Non-Food/Zooz order counts | Total Orders | MoM Change.
    // Runs Jan through the latest month actually on file, filling any gap
    // month with zero rather than skipping it, so Jose sees the full-year
    // timeline instead of just the months a file happened to include.
    var mt = wb.addWorksheet('Monthly Trend', { views: [{ showGridLines: false, state: 'frozen', ySplit: 5 }] });
    mt.columns = [
      { width: 9 },  { width: 14 }, { width: 14 }, { width: 12 }, { width: 14 }, // A-E: Month, Food/NF/Zooz/Total AED
      { width: 9 },  { width: 9 },  { width: 9 },                                // F-H: Food/NF/Zooz %
      { width: 9 },  { width: 9 },  { width: 9 },  { width: 10 },                // I-L: Food/NF/Zooz/Total Orders
      { width: 12 }                                                              // M: MoM Change
    ];
    var mtTitle = mt.addRow(['MONTH-BY-MONTH RETURNS — FOOD vs NON-FOOD vs ZOOZ']); styleTitleRow(mtTitle); mt.mergeCells('A1:M1');
    warningRow(mt, JAN_MAR_DCV_WARNING, 'M');
    mt.addRow([]);
    styleSectionRow(mt.addRow(['VALUE (AED), % SHARE, AND ORDER COUNT BY MONTH — Jan through latest month on file']), 13);
    var mtHdr = mt.addRow(['Month', 'Food (AED)', 'Non-Food (AED)', 'Zooz (AED)', 'Total (AED)', 'Food %', 'Non-Food %', 'Zooz %', 'Food Ord.', 'Non-Food Ord.', 'Zooz Ord.', 'Total Ord.', 'MoM Change']);
    styleHeaderRow(mtHdr, 13);
    var mtMaxMonth = months.length ? Math.max.apply(null, months) : 12;
    var mtFullMonths = [];
    for (var mfm = 1; mfm <= mtMaxMonth; mfm++) mtFullMonths.push(mfm);
    var mtFirstRow = mt.rowCount + 1;
    var mtPrevTotal = null;
    mtFullMonths.forEach(function(m, mi) {
      var mLabel = MONTH_NAMES[m] || m;
      var isJanMarBoundary = m === 1 || m === 2 || m === 3;
      var ma = monthAgg[m] || { food: 0, nonfood: 0, zooz: 0, total: 0, foodOrd: 0, nfOrd: 0, zoozOrd: 0, totalOrd: 0, inv: 0, grv: 0, invOrd: 0, grvOrd: 0 };
      var r0 = mt.addRow([isJanMarBoundary ? mLabel + ' ⚠*' : mLabel,
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_BU + ',"Food")', result: ma.food },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_BU + ',"Non-Food")', result: ma.nonfood },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_BU + ',"Zooz")', result: ma.zooz },
        { formula: 'SUMIF(' + R_MONTH + ',' + m + ',' + R_VAL + ')', result: ma.total },
        '', '', '',
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Food")', result: ma.foodOrd },
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Non-Food")', result: ma.nfOrd },
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_BU + ',"Zooz")', result: ma.zoozOrd },
        { formula: 'COUNTIF(' + R_MONTH + ',' + m + ')', result: ma.totalOrd },
        '']);
      if (isJanMarBoundary) { r0.getCell('A').font = { bold: true, color: { argb: 'FFCC0000' } }; }
      ['B','C','D','E'].forEach(function(col){ r0.getCell(col).numFmt = '#,##0'; });
      r0.getCell('F').value = { formula: 'IFERROR(B' + r0.number + '/E' + r0.number + ',0)', result: pct(ma.food, ma.total) }; r0.getCell('F').numFmt = '0.0%';
      r0.getCell('G').value = { formula: 'IFERROR(C' + r0.number + '/E' + r0.number + ',0)', result: pct(ma.nonfood, ma.total) }; r0.getCell('G').numFmt = '0.0%';
      r0.getCell('H').value = { formula: 'IFERROR(D' + r0.number + '/E' + r0.number + ',0)', result: pct(ma.zooz, ma.total) }; r0.getCell('H').numFmt = '0.0%';
      if (mi === 0) { r0.getCell('M').value = '—'; }
      else {
        var momResult = mtPrevTotal > 0 ? (ma.total - mtPrevTotal) / mtPrevTotal : 0;
        r0.getCell('M').value = { formula: 'IFERROR((E' + r0.number + '-E' + (r0.number - 1) + ')/E' + (r0.number - 1) + ',0)', result: momResult }; r0.getCell('M').numFmt = '0.0%';
      }
      mtPrevTotal = ma.total;
      borderRow(r0, 13);
      zebra(r0, 13, mi % 2 === 1);
    });
    var mtLastRow = mt.rowCount;
    if (mtFullMonths.length) {
      var mtSumFood = 0, mtSumNF = 0, mtSumZooz = 0, mtSumTotal = 0, mtSumFoodOrd = 0, mtSumNFOrd = 0, mtSumZoozOrd = 0, mtSumTotalOrd = 0;
      mtFullMonths.forEach(function(m) {
        var ma = monthAgg[m] || { food: 0, nonfood: 0, zooz: 0, total: 0, foodOrd: 0, nfOrd: 0, zoozOrd: 0, totalOrd: 0, inv: 0, grv: 0, invOrd: 0, grvOrd: 0 };
        mtSumFood += ma.food; mtSumNF += ma.nonfood; mtSumZooz += ma.zooz; mtSumTotal += ma.total;
        mtSumFoodOrd += ma.foodOrd; mtSumNFOrd += ma.nfOrd; mtSumZoozOrd += ma.zoozOrd; mtSumTotalOrd += ma.totalOrd;
      });
      var mtTotalRow = mt.addRow(['TOTAL',
        { formula: 'SUM(B' + mtFirstRow + ':B' + mtLastRow + ')', result: mtSumFood },
        { formula: 'SUM(C' + mtFirstRow + ':C' + mtLastRow + ')', result: mtSumNF },
        { formula: 'SUM(D' + mtFirstRow + ':D' + mtLastRow + ')', result: mtSumZooz },
        { formula: 'SUM(E' + mtFirstRow + ':E' + mtLastRow + ')', result: mtSumTotal },
        '', '', '',
        { formula: 'SUM(I' + mtFirstRow + ':I' + mtLastRow + ')', result: mtSumFoodOrd },
        { formula: 'SUM(J' + mtFirstRow + ':J' + mtLastRow + ')', result: mtSumNFOrd },
        { formula: 'SUM(K' + mtFirstRow + ':K' + mtLastRow + ')', result: mtSumZoozOrd },
        { formula: 'SUM(L' + mtFirstRow + ':L' + mtLastRow + ')', result: mtSumTotalOrd },
        '']);
      ['B','C','D','E'].forEach(function(col){ mtTotalRow.getCell(col).numFmt = '#,##0'; });
      var mtTotRowNum = mtTotalRow.number;
      mtTotalRow.getCell('F').value = { formula: 'IFERROR(B' + mtTotRowNum + '/E' + mtTotRowNum + ',0)', result: pct(mtSumFood, mtSumTotal) }; mtTotalRow.getCell('F').numFmt = '0.0%';
      mtTotalRow.getCell('G').value = { formula: 'IFERROR(C' + mtTotRowNum + '/E' + mtTotRowNum + ',0)', result: pct(mtSumNF, mtSumTotal) }; mtTotalRow.getCell('G').numFmt = '0.0%';
      mtTotalRow.getCell('H').value = { formula: 'IFERROR(D' + mtTotRowNum + '/E' + mtTotRowNum + ',0)', result: pct(mtSumZooz, mtSumTotal) }; mtTotalRow.getCell('H').numFmt = '0.0%';
      styleTotalRow(mtTotalRow, 13);
    }
    mt.addRow([]);

    // In-cell data bars on the three AED columns (B/C/D) — this is the
    // "chart look" for the trend: each column bars out relative to its own
    // min/max across the months, giving an at-a-glance visual read of how
    // Food, Non-Food, and Zooz are moving month to month, without needing a
    // native Excel chart object (not supported by the library generating
    // this workbook — data bars are the reliable, professional-looking
    // native alternative that renders correctly in real Excel).
    if (mtFullMonths.length) {
      mt.addConditionalFormatting({ ref: 'B' + mtFirstRow + ':B' + mtLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FFE84B4B' }, minLength: 0, maxLength: 100 }] });
      mt.addConditionalFormatting({ ref: 'C' + mtFirstRow + ':C' + mtLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FF5B8DEE' }, minLength: 0, maxLength: 100 }] });
      mt.addConditionalFormatting({ ref: 'D' + mtFirstRow + ':D' + mtLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FFC9A84C' }, minLength: 0, maxLength: 100 }] });
    }
    noteRow(mt, 'All figures calculated live from the Return Master tab (SUMIFS/COUNTIFS by Month and BU). Months with no data on file show as zero, not skipped. The most recent month may be partial — data on file through ' + genDate + '. The colored bars in the Food/Non-Food/Zooz AED columns are relative to that column\'s own range across the months shown.', 'M');
    var mtStarNote = mt.addRow(['⚠* = see the DCV/DGC reclassification warning at the top of this sheet.']);
    mtStarNote.font = { bold: true, size: 16, color: { argb: 'FFCC0000' } }; mt.mergeCells('A' + mtStarNote.number + ':M' + mtStarNote.number); mtStarNote.height = 22;
    mt.addRow([]);

    // ── Second table on the same sheet: month-by-month Invoice Cancellation vs
    // GRV Return — same live-formula, same-shape structure as the table above,
    // just split by Category instead of BU. ──
    styleSectionRow(mt.addRow(['VALUE (AED), % SHARE, AND ORDER COUNT BY MONTH — INVOICE CANCELLATION vs GRV RETURN']), 13);
    var mtCatHdr = mt.addRow(['Month', 'Invoice Cancel (AED)', 'GRV Return (AED)', '', 'Total (AED)', 'Invoice %', 'GRV %', '', 'Invoice Ord.', 'GRV Ord.', '', 'Total Ord.', 'MoM Change']);
    styleHeaderRow(mtCatHdr, 13);
    var mtCatFirstRow = mt.rowCount + 1;
    var mtCatPrevTotal = null;
    mtFullMonths.forEach(function(m, mi) {
      var mLabel = MONTH_NAMES[m] || m;
      var ma = monthAgg[m] || { total: 0, totalOrd: 0, inv: 0, grv: 0, invOrd: 0, grvOrd: 0 };
      var r0 = mt.addRow([mLabel,
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_CAT + ',"Invoice Cancellation")', result: ma.inv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_MONTH + ',' + m + ',' + R_CAT + ',"GRV-Return")', result: ma.grv },
        '',
        { formula: 'SUMIF(' + R_MONTH + ',' + m + ',' + R_VAL + ')', result: ma.total },
        '', '', '',
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_CAT + ',"Invoice Cancellation")', result: ma.invOrd },
        { formula: 'COUNTIFS(' + R_MONTH + ',' + m + ',' + R_CAT + ',"GRV-Return")', result: ma.grvOrd },
        '',
        { formula: 'COUNTIF(' + R_MONTH + ',' + m + ')', result: ma.totalOrd },
        '']);
      ['B','C','E'].forEach(function(col){ r0.getCell(col).numFmt = '#,##0'; });
      r0.getCell('F').value = { formula: 'IFERROR(B' + r0.number + '/E' + r0.number + ',0)', result: pct(ma.inv, ma.total) }; r0.getCell('F').numFmt = '0.0%';
      r0.getCell('G').value = { formula: 'IFERROR(C' + r0.number + '/E' + r0.number + ',0)', result: pct(ma.grv, ma.total) }; r0.getCell('G').numFmt = '0.0%';
      if (mi === 0) { r0.getCell('M').value = '—'; }
      else {
        var momResult = mtCatPrevTotal > 0 ? (ma.total - mtCatPrevTotal) / mtCatPrevTotal : 0;
        r0.getCell('M').value = { formula: 'IFERROR((E' + r0.number + '-E' + (r0.number - 1) + ')/E' + (r0.number - 1) + ',0)', result: momResult }; r0.getCell('M').numFmt = '0.0%';
      }
      mtCatPrevTotal = ma.total;
      borderRow(r0, 13);
      zebra(r0, 13, mi % 2 === 1);
    });
    var mtCatLastRow = mt.rowCount;
    if (mtFullMonths.length) {
      var mtSumInv = 0, mtSumGrv = 0, mtSumCatTotal = 0, mtSumInvOrd = 0, mtSumGrvOrd = 0, mtSumCatTotalOrd = 0;
      mtFullMonths.forEach(function(m) {
        var ma = monthAgg[m] || { total: 0, totalOrd: 0, inv: 0, grv: 0, invOrd: 0, grvOrd: 0 };
        mtSumInv += ma.inv; mtSumGrv += ma.grv; mtSumCatTotal += ma.total;
        mtSumInvOrd += ma.invOrd; mtSumGrvOrd += ma.grvOrd; mtSumCatTotalOrd += ma.totalOrd;
      });
      var mtCatTotalRow = mt.addRow(['TOTAL',
        { formula: 'SUM(B' + mtCatFirstRow + ':B' + mtCatLastRow + ')', result: mtSumInv },
        { formula: 'SUM(C' + mtCatFirstRow + ':C' + mtCatLastRow + ')', result: mtSumGrv },
        '',
        { formula: 'SUM(E' + mtCatFirstRow + ':E' + mtCatLastRow + ')', result: mtSumCatTotal },
        '', '', '',
        { formula: 'SUM(I' + mtCatFirstRow + ':I' + mtCatLastRow + ')', result: mtSumInvOrd },
        { formula: 'SUM(J' + mtCatFirstRow + ':J' + mtCatLastRow + ')', result: mtSumGrvOrd },
        '',
        { formula: 'SUM(L' + mtCatFirstRow + ':L' + mtCatLastRow + ')', result: mtSumCatTotalOrd },
        '']);
      ['B','C','E'].forEach(function(col){ mtCatTotalRow.getCell(col).numFmt = '#,##0'; });
      var mtCatTotRowNum = mtCatTotalRow.number;
      mtCatTotalRow.getCell('F').value = { formula: 'IFERROR(B' + mtCatTotRowNum + '/E' + mtCatTotRowNum + ',0)', result: pct(mtSumInv, mtSumCatTotal) }; mtCatTotalRow.getCell('F').numFmt = '0.0%';
      mtCatTotalRow.getCell('G').value = { formula: 'IFERROR(C' + mtCatTotRowNum + '/E' + mtCatTotRowNum + ',0)', result: pct(mtSumGrv, mtSumCatTotal) }; mtCatTotalRow.getCell('G').numFmt = '0.0%';
      styleTotalRow(mtCatTotalRow, 13);
      if (mtFullMonths.length) {
        mt.addConditionalFormatting({ ref: 'B' + mtCatFirstRow + ':B' + mtCatLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FF5B8DEE' }, minLength: 0, maxLength: 100 }] });
        mt.addConditionalFormatting({ ref: 'C' + mtCatFirstRow + ':C' + mtCatLastRow, rules: [{ type: 'dataBar', cfvo: [{ type: 'min' }, { type: 'max' }], color: { argb: 'FFC9A84C' }, minLength: 0, maxLength: 100 }] });
      }
    }
    noteRow(mt, 'Same source data as the table above (Return Master tab), split by Category instead of BU. "Invoice Cancel" = order cancelled before dispatch; "GRV Return" = physically returned after dispatch, pending goods-return voucher.', 'M');

    // ══════════════════════ Sheet 3: By Warehouse ══════════════════════
    var wh = wb.addWorksheet('By Warehouse', { views: [{ showGridLines: false, state: 'frozen', ySplit: 4 }] });
    wh.columns = [{ width: 14 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 12 }, { width: 16 }, { width: 12 }];
    var whTitle = wh.addRow(['RETURNS BY WAREHOUSE']); styleTitleRow(whTitle); wh.mergeCells('A1:J1');
    warningRow(wh, JAN_MAR_DCV_WARNING, 'J');
    var whHdr = wh.addRow(['Warehouse', 'Orders', 'Invoice Cancel (Orders)', 'Invoice Cancel (AED)', 'Invoice %', 'GRV Return (Orders)', 'GRV Return (AED)', 'GRV %', 'Total (AED)', 'Total %']);
    styleHeaderRow(whHdr, 10);
    var whFirstRow = wh.rowCount + 1;
    whKeys.forEach(function(w, wi) {
      var wa = whAgg[w];
      var r0 = wh.addRow([w,
        { formula: 'COUNTIF(' + R_WH + ',A' + (wh.rowCount + 1) + ')', result: wa.orders },
        { formula: 'COUNTIFS(' + R_WH + ',A' + (wh.rowCount + 1) + ',' + R_CAT + ',"Invoice Cancellation")', result: wa.inv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_WH + ',A' + (wh.rowCount + 1) + ',' + R_CAT + ',"Invoice Cancellation")', result: wa.invVal }, '',
        { formula: 'COUNTIFS(' + R_WH + ',A' + (wh.rowCount + 1) + ',' + R_CAT + ',"GRV-Return")', result: wa.grv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_WH + ',A' + (wh.rowCount + 1) + ',' + R_CAT + ',"GRV-Return")', result: wa.grvVal }, '',
        { formula: 'SUMIF(' + R_WH + ',A' + (wh.rowCount + 1) + ',' + R_VAL + ')', result: wa.val }, '']);
      r0.getCell(4).numFmt = '#,##0'; r0.getCell(7).numFmt = '#,##0'; r0.getCell(9).numFmt = '#,##0';
      borderRow(r0, 10);
      zebra(r0, 10, wi % 2 === 1);
    });
    var whLastRow = wh.rowCount;
    var whInvTotalVal = whKeys.reduce(function(s, w) { return s + (whAgg[w].invVal || 0); }, 0);
    var whGrvTotalVal = whKeys.reduce(function(s, w) { return s + (whAgg[w].grvVal || 0); }, 0);
    for (var whi = whFirstRow; whi <= whLastRow; whi++) {
      var whRowW = whKeys[whi - whFirstRow];
      var whWa = whAgg[whRowW];
      wh.getCell('E' + whi).value = { formula: 'IFERROR(D' + whi + '/SUM($D$' + whFirstRow + ':$D$' + whLastRow + '),0)', result: pct(whWa.invVal, whInvTotalVal) };
      wh.getCell('E' + whi).numFmt = '0.0%';
      wh.getCell('H' + whi).value = { formula: 'IFERROR(G' + whi + '/SUM($G$' + whFirstRow + ':$G$' + whLastRow + '),0)', result: pct(whWa.grvVal, whGrvTotalVal) };
      wh.getCell('H' + whi).numFmt = '0.0%';
      wh.getCell('J' + whi).value = { formula: 'IFERROR(I' + whi + '/SUM($I$' + whFirstRow + ':$I$' + whLastRow + '),0)', result: pct(whWa.val, whTotalVal) };
      wh.getCell('J' + whi).numFmt = '0.0%';
    }
    var whSumOrders = whKeys.reduce(function(s, w) { return s + whAgg[w].orders; }, 0);
    var whSumInv = whKeys.reduce(function(s, w) { return s + whAgg[w].inv; }, 0);
    var whSumGrv = whKeys.reduce(function(s, w) { return s + whAgg[w].grv; }, 0);
    var whTotalRow = wh.addRow(['TOTAL',
      { formula: 'SUM(B' + whFirstRow + ':B' + whLastRow + ')', result: whSumOrders },
      { formula: 'SUM(C' + whFirstRow + ':C' + whLastRow + ')', result: whSumInv },
      { formula: 'SUM(D' + whFirstRow + ':D' + whLastRow + ')', result: whInvTotalVal }, 1,
      { formula: 'SUM(F' + whFirstRow + ':F' + whLastRow + ')', result: whSumGrv },
      { formula: 'SUM(G' + whFirstRow + ':G' + whLastRow + ')', result: whGrvTotalVal }, 1,
      { formula: 'SUM(I' + whFirstRow + ':I' + whLastRow + ')', result: whTotalVal }, 1]);
    whTotalRow.getCell(4).numFmt = '#,##0'; whTotalRow.getCell(5).numFmt = '0.0%';
    whTotalRow.getCell(7).numFmt = '#,##0'; whTotalRow.getCell(8).numFmt = '0.0%';
    whTotalRow.getCell(9).numFmt = '#,##0'; whTotalRow.getCell(10).numFmt = '0.0%';
    styleTotalRow(whTotalRow, 10);
    noteRow(wh, 'Calculated live from the Return Master tab. Invoice Cancel and GRV Return are kept as separate value/% columns — "Invoice %" is that warehouse\'s share of the Invoice Cancel total, "GRV %" is their share of the GRV Return total, and "Total %" is their share of the combined total. These three percentages are calculated against different bases and will not add up to each other.', 'J');

    // ══════════════════════ Sheet: Top Customers ══════════════════════
    // Every customer with a return on file (not just top 10 like the
    // on-screen dashboard panel), each with its own Invoice Cancel / GRV
    // Return order-count breakdown — same pattern as By Warehouse.
    var custKeys = Object.keys(custAgg).sort(function(a, b) { return custAgg[b].val - custAgg[a].val; });
    var custTotalVal = custKeys.reduce(function(s, c) { return s + custAgg[c].val; }, 0);
    var tc = wb.addWorksheet('Top Customers', { views: [{ showGridLines: false, state: 'frozen', ySplit: 3 }] });
    tc.columns = [{ width: 34 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 12 }, { width: 14 }, { width: 16 }, { width: 12 }, { width: 16 }, { width: 12 }];
    var tcTitle = tc.addRow(['RETURNS BY CUSTOMER']); styleTitleRow(tcTitle); tc.mergeCells('A1:J1');
    var tcHdr = tc.addRow(['Customer', 'Orders', 'Invoice Cancel (Orders)', 'Invoice Cancel (AED)', 'Invoice %', 'GRV Return (Orders)', 'GRV Return (AED)', 'GRV %', 'Total (AED)', 'Total %']);
    styleHeaderRow(tcHdr, 10);
    var tcFirstRow = tc.rowCount + 1;
    custKeys.forEach(function(c, ci) {
      var ca = custAgg[c];
      var r0 = tc.addRow([c,
        { formula: 'COUNTIF(' + R_CUST + ',A' + (tc.rowCount + 1) + ')', result: ca.orders },
        { formula: 'COUNTIFS(' + R_CUST + ',A' + (tc.rowCount + 1) + ',' + R_CAT + ',"Invoice Cancellation")', result: ca.inv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_CUST + ',A' + (tc.rowCount + 1) + ',' + R_CAT + ',"Invoice Cancellation")', result: ca.invVal }, '',
        { formula: 'COUNTIFS(' + R_CUST + ',A' + (tc.rowCount + 1) + ',' + R_CAT + ',"GRV-Return")', result: ca.grv },
        { formula: 'SUMIFS(' + R_VAL + ',' + R_CUST + ',A' + (tc.rowCount + 1) + ',' + R_CAT + ',"GRV-Return")', result: ca.grvVal }, '',
        { formula: 'SUMIF(' + R_CUST + ',A' + (tc.rowCount + 1) + ',' + R_VAL + ')', result: ca.val }, '']);
      r0.getCell(4).numFmt = '#,##0'; r0.getCell(7).numFmt = '#,##0'; r0.getCell(9).numFmt = '#,##0';
      borderRow(r0, 10);
      zebra(r0, 10, ci % 2 === 1);
    });
    var tcLastRow = tc.rowCount;
    var custInvTotalVal = custKeys.reduce(function(s, c) { return s + (custAgg[c].invVal || 0); }, 0);
    var custGrvTotalVal = custKeys.reduce(function(s, c) { return s + (custAgg[c].grvVal || 0); }, 0);
    for (var tci = tcFirstRow; tci <= tcLastRow; tci++) {
      var tcCustName = custKeys[tci - tcFirstRow];
      var tcCa = custAgg[tcCustName];
      tc.getCell('E' + tci).value = { formula: 'IFERROR(D' + tci + '/SUM($D$' + tcFirstRow + ':$D$' + tcLastRow + '),0)', result: pct(tcCa.invVal, custInvTotalVal) };
      tc.getCell('E' + tci).numFmt = '0.0%';
      tc.getCell('H' + tci).value = { formula: 'IFERROR(G' + tci + '/SUM($G$' + tcFirstRow + ':$G$' + tcLastRow + '),0)', result: pct(tcCa.grvVal, custGrvTotalVal) };
      tc.getCell('H' + tci).numFmt = '0.0%';
      tc.getCell('J' + tci).value = { formula: 'IFERROR(I' + tci + '/SUM($I$' + tcFirstRow + ':$I$' + tcLastRow + '),0)', result: pct(tcCa.val, custTotalVal) };
      tc.getCell('J' + tci).numFmt = '0.0%';
    }
    if (custKeys.length) {
      var tcSumOrders = custKeys.reduce(function(s, c) { return s + custAgg[c].orders; }, 0);
      var tcSumInv = custKeys.reduce(function(s, c) { return s + custAgg[c].inv; }, 0);
      var tcSumGrv = custKeys.reduce(function(s, c) { return s + custAgg[c].grv; }, 0);
      var tcTotalRow = tc.addRow(['TOTAL',
        { formula: 'SUM(B' + tcFirstRow + ':B' + tcLastRow + ')', result: tcSumOrders },
        { formula: 'SUM(C' + tcFirstRow + ':C' + tcLastRow + ')', result: tcSumInv },
        { formula: 'SUM(D' + tcFirstRow + ':D' + tcLastRow + ')', result: custInvTotalVal }, 1,
        { formula: 'SUM(F' + tcFirstRow + ':F' + tcLastRow + ')', result: tcSumGrv },
        { formula: 'SUM(G' + tcFirstRow + ':G' + tcLastRow + ')', result: custGrvTotalVal }, 1,
        { formula: 'SUM(I' + tcFirstRow + ':I' + tcLastRow + ')', result: custTotalVal }, 1]);
      tcTotalRow.getCell(4).numFmt = '#,##0'; tcTotalRow.getCell(5).numFmt = '0.0%';
      tcTotalRow.getCell(7).numFmt = '#,##0'; tcTotalRow.getCell(8).numFmt = '0.0%';
      tcTotalRow.getCell(9).numFmt = '#,##0'; tcTotalRow.getCell(10).numFmt = '0.0%';
      styleTotalRow(tcTotalRow, 10);
    }
    tc.autoFilter = { from: { row: 2, column: 1 }, to: { row: 2, column: 10 } };
    noteRow(tc, 'Calculated live from the Return Master tab. Every customer on file, not just the dashboard\'s top 10. Invoice Cancel and GRV Return are kept as separate value/% columns — "Invoice %" is that customer\'s share of the Invoice Cancel total, "GRV %" is their share of the GRV Return total, and "Total %" is their share of the combined total. These three percentages are calculated against different bases and will not add up to each other.', 'J');

    // ══════════════════════ Sheet 4: By Area & Reason ══════════════════════
    var ar = wb.addWorksheet('Area & Reason', { views: [{ showGridLines: false, state: 'frozen', ySplit: 3 }] });
    ar.columns = [{ width: 14 }, { width: 14 }, { width: 12 }, { width: 4 }, { width: 36 }, { width: 14 }, { width: 12 }];
    var arTitle = ar.addRow(['RETURNS BY EMIRATE / AREA  &  BY RETURN REASON']); styleTitleRow(arTitle); ar.mergeCells('A1:G1');
    var arHdr = ar.addRow(['Area', 'Value (AED)', '% of Total', '', 'Return Reason', 'Value (AED)', '% of Total']);
    // Style the two header groups separately (columns A-C and E-G) —
    // column D is a blank visual gap between the two tables and should
    // stay untouched, not dark-filled like a real header cell.
    [1,2,3].forEach(function(ci){ var c=arHdr.getCell(ci); c.fill={type:'pattern',pattern:'solid',fgColor:{argb:DARKBG}}; c.font={bold:true,color:{argb:GOLD},size:16}; c.border=CELL_BORDER; });
    [5,6,7].forEach(function(ci){ var c=arHdr.getCell(ci); c.fill={type:'pattern',pattern:'solid',fgColor:{argb:DARKBG}}; c.font={bold:true,color:{argb:GOLD},size:16}; c.border=CELL_BORDER; });
    arHdr.height = 26;
    ...
    for (var ai = 0; ai < arRowsN; ai++) {
      ...
      if (areaKeys[ai]) {
        var aVal = areaMap[areaKeys[ai]] || 0;
        r0.getCell(2).value = { formula: 'SUMIF(' + R_AREA + ',A' + r0.number + ',' + R_VAL + ')', result: aVal }; r0.getCell(2).numFmt = '#,##0';
        r0.getCell(3).value = { formula: 'IFERROR(B' + r0.number + '/SUM(' + R_VAL + '),0)', result: pct(aVal, totalVal) }; r0.getCell(3).numFmt = '0.0%';
        r0.getCell(1).border = CELL_BORDER; r0.getCell(2).border = CELL_BORDER; r0.getCell(3).border = CELL_BORDER;
        if (ai % 2 === 1) { r0.getCell(1).fill = r0.getCell(2).fill = r0.getCell(3).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F5EF' } }; }
      }
      if (reasonKeys[ai]) {
        var rVal = reasonMap[reasonKeys[ai]] || 0;
        r0.getCell(6).value = { formula: 'SUMIF(' + R_REASON + ',E' + r0.number + ',' + R_VAL + ')', result: rVal }; r0.getCell(6).numFmt = '#,##0';
        r0.getCell(7).value = { formula: 'IFERROR(F' + r0.number + '/SUM(' + R_VAL + '),0)', result: pct(rVal, totalVal) }; r0.getCell(7).numFmt = '0.0%';
        r0.getCell(5).border = CELL_BORDER; r0.getCell(6).border = CELL_BORDER; r0.getCell(7).border = CELL_BORDER;
        if (ai % 2 === 1) { r0.getCell(5).fill = r0.getCell(6).fill = r0.getCell(7).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F5EF' } }; }
      }
    }
    ...
    var rm = wb.addWorksheet('Return Master', { views: [{ showGridLines: false, state: 'frozen', ySplit: 2 }] });
    rm.columns = [{ width: 8 }, { width: 6 }, { width: 12 }, { width: 20 }, { width: 12 }, { width: 14 }, { width: 14 }, { width: 26 }, { width: 16 }, { width: 34 } ];
    var rmTitle = rm.addRow(['RETURN MASTER DATA (' + rows.length + ' rows)']); styleTitleRow(rmTitle); rm.mergeCells('A1:J1');
    var rmHdr = rm.addRow(['Month', 'Day', 'Warehouse', 'Category', 'BU', 'Value (AED)', 'Area', 'Customer', 'Order No', 'Return Reason']);
    styleHeaderRow(rmHdr, 10);
    rows.forEach(function(r, ri) {
      var r0 = rm.addRow([r.month, r.day, r.warehouse, r.category, r.bu, Math.round(parseFloat(r.value) || 0), r.area || '—', r.customer || '—', r.order_no || '—', r.reason || '—']);
      r0.getCell(6).numFmt = '#,##0';
      for (var ci = 1; ci <= 10; ci++) r0.getCell(ci).border = { bottom: THIN };
      if (ri % 2 === 1) { for (var ci2 = 1; ci2 <= 10; ci2++) r0.getCell(ci2).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: 'FFF7F5EF' } }; }
    });
    rm.autoFilter = { from: { row: 2, column: 1 }, to: { row: 2, column: 10 } };

    // ── Final pass: guarantee NOTHING in this workbook renders smaller than 16pt,
    // and every row is tall enough to actually fit that size — done last, over
    // every sheet/row/cell, so it catches every data cell (like the full Return
    // Master dump above) that wasn't individually styled earlier, without
    // needing to thread a font size through every single addRow() call. ──
    var MIN_FONT_SIZE = 16;
    var MIN_ROW_HEIGHT = 22;
    wb.eachSheet(function (sheet) {
      sheet.eachRow(function (row) {
        if (!row.height || row.height < MIN_ROW_HEIGHT) row.height = MIN_ROW_HEIGHT;
        row.eachCell({ includeEmpty: false }, function (cell) {
          var existing = cell.font || {};
          if (!existing.size || existing.size < MIN_FONT_SIZE) {
            cell.font = Object.assign({}, existing, { size: MIN_FONT_SIZE });
          }
        });
      });
      // Columns were originally sized for ~11pt text; widen proportionally so
      // 16pt+ text has room to breathe instead of overflowing or reading cramped.
      if (sheet.columns) {
        sheet.columns.forEach(function (col) {
          if (col.width) col.width = Math.round(col.width * 1.35);
        });
      }
    });
...
require('./transport_cost_module')(app, pool, requireAuth, requireRole, matchRateViaVehicleMaster);
