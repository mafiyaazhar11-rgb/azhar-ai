...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  var SUPPLY_PERIOD_LIMITS = { day: 30, week: 16, month: 12 };

  app.get('/api/pallets/supply/trend', requireAuth, async function (req, res) {
    try {
      var period = req.query.period;
      if (!['day', 'week', 'month'].includes(period)) period = 'month';
      var limit = SUPPLY_PERIOD_LIMITS[period];
      ...
      var sql = `
        SELECT date_trunc($1, entry_date) AS period_start,
          COALESCE(SUM(CASE WHEN entry_type='inbound' THEN pallet_qty ELSE 0 END),0) AS received,
          COALESCE(SUM(CASE WHEN entry_type='return' THEN pallet_qty ELSE 0 END),0) AS returned,
          COALESCE(SUM(CASE WHEN entry_type='inbound' THEN truck_count ELSE 0 END),0) AS inbound_trucks,
          COALESCE(SUM(CASE WHEN entry_type='return' THEN truck_count ELSE 0 END),0) AS return_trucks
        FROM aujan_supply_log
        ${where}
        GROUP BY 1
        ORDER BY 1 DESC
        LIMIT ${limit}`;
      var r = await pool.query(sql, params);
      var rows = r.rows.reverse().map(function (row) {
        return {
          period_start: row.period_start,
          received: +row.received,
          returned: +row.returned,
          balance: +row.received - +row.returned,
          inbound_trucks: +row.inbound_trucks,
          return_trucks: +row.return_trucks
        };
      });
      res.json({ period: period, points: rows });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
