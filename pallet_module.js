...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.get('/api/pallets/transport-tally/reconcile/export', requireAuth, async function (req, res) {
    try {
      ...
      var legendRow = rs.addRow(['Green = Transport\'s count matches the warehouse ledger for that day. Red = a difference \u2014 ' + mismatchCount + ' of ' + rows.length + ' day(s) shown differ. Transport\'s file mainly tracks what is still retained with customers, not a full mirror of every warehouse movement, so a difference here is informational and worth a quick check, not automatically an error.']);
