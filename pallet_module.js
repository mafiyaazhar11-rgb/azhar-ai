...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.get('/api/pallets/analyzed-report/export', requireAuth, async function (req, res) {
    try {
      ...
      line('Comparing day by day against the warehouse ledger: ' + mismatchDays + ' of ' + reconcileRows.length + ' day(s) have at least one figure (Dispatched, Returned, or Collected from DIP) that doesn\u2019t match. ' + (mismatchDays > 0 ? 'See the Reconciliation sheet for exactly which days and which figure \u2014 useful as a specific, dated list to raise with Transport.' : 'Every day in this period reconciled cleanly.'));
      blank();
      ...
      var recoveryVsLedgerNote = 'Note: this figure (' + ledger.retained_by_customer + ') and the Recovery Dashboard\u2019s Total Outstanding (' + kpi.total_outstanding + ') and Transport\u2019s Retained with Customer (' + tally.retained + ') measure an overlapping but not identical thing from three different files \u2014 they are not expected to match exactly.';
