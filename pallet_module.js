...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.get('/api/pallets/ask', requireAuth, async function (req, res) {
    try {
      ...
      var systemPrompt = 'You answer questions about Aujan pallet logistics for a warehouse/transport operations team, using ONLY the data snapshot given below. Never invent numbers or customer names not present in it. '
        + 'If the question needs data not included here, say so plainly in one sentence and name which dashboard section would have it, rather than guessing. '
        + 'Maximum 5-6 lines. No preamble like "Based on the data" — answer directly. Use the actual numbers from the snapshot.\n\n'
        + 'DATA SNAPSHOT:\n' + contextLines;
