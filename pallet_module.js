...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  async function initPalletDB() {
    try {
      ...
      await pool.query(`CREATE TABLE IF NOT EXISTS aujan_ledger (
        ...
        truck_ref TEXT NOT NULL UNIQUE,
  ...
  app.post('/api/pallets/ledger/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      // Build a field map from whatever headers this file actually has.
      var sampleHeaders = Object.keys(rawRows[0]);
      var fieldByHeader = {};
      sampleHeaders.forEach(function (h) { var m = matchLedgerField(h); if (m) fieldByHeader[h] = m; });

      var batchId = 'LEDGER-' + Date.now();
      var inserted = 0, updated = 0, rejected = 0;
      var errors = [];
      ...
      for (var i = 0; i < rawRows.length; i++) {
        ...
        try {
          var existing = await pool.query('SELECT id FROM aujan_ledger WHERE truck_ref=$1', [rec.truck_ref]);
          if (existing.rows.length) {
            await pool.query(
              `UPDATE aujan_ledger SET entry_date=$1, inbound_pallets=$2, outbound_pallets=$3, return_received_from_delivery=$4,
                damage_pallets=$5, pallets_returned_to_aujan=$6, pallet_retained_by_customer=$7,
                reported_opening_pallets=$8, reported_closing_balance=$9, remarks=$10, upload_batch_id=$11
               WHERE id=$12`,
              [rec.entry_date, rec.inbound_pallets, rec.outbound_pallets, rec.return_received_from_delivery,
              rec.damage_pallets, rec.pallets_returned_to_aujan, rec.pallet_retained_by_customer,
              rec.reported_opening_pallets || null, rec.reported_closing_balance || null, rec.remarks, batchId, existing.rows[0].id]
            );
            updated++;
          } else {
            await pool.query(
      ...
      res.json({ batch_id: batchId, total_rows: rawRows.length, inserted: inserted, updated: updated, rejected: rejected, errors: errors.slice(0, 200) });
    } catch (e) {
  ...
  app.get('/api/pallets/ledger/export', requireAuth, async function (req, res) {
    try {
      ...
      wb.creator = 'AZHAR-AI'; wb.created = new Date();
      var ws = wb.addWorksheet('Ledger');
      ws.columns = [{ width: 12 }, { width: 16 }, { width: 12 }, { width: 12 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 14 }, { width: 12 }, { width: 14 }, { width: 24 }];
      styleTitleRow(ws, 'AUJAN PALLET INBOUND & OUTBOUND TRACKER', 'K');
      ws.addRow(['Downloaded', new Date().toLocaleString('en-AE')]);
      ws.addRow([]);
      styleHeaderRow(ws.addRow(['Date', 'Truck/Tour/Invoice #', 'Opening', 'Inbound', 'Outbound', 'Return Recv.', 'Damage', 'Returned to Aujan', 'Closing', 'Retained by Customer', 'Remarks']));
      r.rows.forEach(function (row) {
        ws.addRow([row.entry_date, row.truck_ref, +row.computed_opening_balance, +row.inbound_pallets, +row.outbound_pallets,
      ...
      var buf = await wb.xlsx.writeBuffer();
      res.setHeader('Content-Disposition', 'attachment; filename="Aujan_Pallet_Ledger_' + Date.now() + '.xlsx"');
      res.setHeader('Content-Type', 'application/vnd.openxmlformats-officedocument.spreadsheetml.sheet');
      res.send(Buffer.from(buf));
