...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  async function buildDirectionalPalletExport(direction, req, res) {
    try {
      ...
      var currentBalance = lrows.length ? +lrows[lrows.length - 1].computed_closing_balance : 0;
      ...
      kpiRow('Customers Holding Pallets', custRows.length);
      kpiRow('Total Pallets Outstanding (Customer-Held)', totalOutstanding);
      kpiRow('Total Value at Risk', 'AED ' + Math.round(totalValueAtRisk).toLocaleString(), '', totalValueAtRisk > 0);
      es.addRow([]);

      // ---- Colored bar visual: Top 10 customers by value at risk ----
      styleHeaderRow(es.addRow(['TOP CUSTOMERS BY COST RISK', '', '']));
      ...
      top10.forEach(function (c) {
        ...
        var row = es.addRow([c.name, 'AED ' + Math.round(+c.value_at_risk).toLocaleString(), bar]);
        row.getCell(3).font = { color: { argb: barLen > 20 ? REDTEXT : (barLen > 10 ? 'FFC9A84C' : GREENTEXT) }, name: 'Arial' };
        row.getCell(1).font = { bold: true };
      });
      es.addRow([]);
      var legendNote = es.addRow(['Bar length is proportional to Value at Risk \u2014 red = highest risk, gold = medium, green = lower.']);
      legendNote.font = { italic: true, size: 9, color: { argb: 'FF888888' } };

      // ---- Sheet 2: Customer Cost Risk Detail ----
  ...
  app.get('/api/pallets/ledger/export', requireAuth, async function (req, res) {
    try {
      ...
      var negRetained = rows.filter(function (r) { return +r.pallet_retained_by_customer < 0; });
      var refCounts = {};
      rows.forEach(function (r) { var k = r.truck_ref || '(blank)'; refCounts[k] = (refCounts[k] || 0) + 1; });
      var dupRefs = Object.keys(refCounts).filter(function (k) { return refCounts[k] > 1; });
      ...
      if (rows.length) {
        ...
        for (var dt = new Date(minD); dt <= maxD; dt.setDate(dt.getDate() + 1)) {
          ...
          if (!presentDates[key]) missingDays.push(key);
        }
      }
      ...
      kpiRow('Data Quality Issues Flagged', negRetained.length + dupRefs.length + missingDays.length, 'See "Data Quality Flags" sheet \u2014 negative retained values, duplicate refs, missing days', (negRetained.length + dupRefs.length + missingDays.length) > 0);
      es.addRow([]);
      ...
      styleHeaderRow(dq.addRow(['NEGATIVE RETAINED-BY-CUSTOMER VALUES', '', 'Physically impossible \u2014 can\u2019t retain a negative number of pallets', '']));
      styleHeaderRow(dq.addRow(['Date', 'Truck/Tour/Invoice #', 'Retained Value (as sent)', 'Closing Balance']));
      negRetained.forEach(function (r) {
        var row = dq.addRow([r.entry_date, r.truck_ref, +r.pallet_retained_by_customer, +r.computed_closing_balance]);
        row.eachCell(function (c) { c.fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: REDFLAG } }; });
      });
      dq.addRow([]);

      styleHeaderRow(dq.addRow(['DUPLICATE TRUCK/TOUR/INVOICE REFERENCES', '', 'Same reference used more than once \u2014 check for double entry', '']));
      styleHeaderRow(dq.addRow(['Reference', 'Times Used', '', '']));
      dupRefs.forEach(function (ref) {
        var row = dq.addRow([ref, refCounts[ref], '', '']);
      ...
      if (!negRetained.length && !dupRefs.length && !missingDays.length) {
