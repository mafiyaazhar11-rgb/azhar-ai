...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.get('/api/pallets/analyzed-report/export', requireAuth, async function (req, res) {
    try {
      ...
      line('Transport delivered ' + kpi.total_delivered + ' pallets on customer orders in this period. Of those, ' + kpi.total_returned + ' (' + recoveryPct + '%) have already come back, leaving ' + kpi.total_outstanding + ' pallets outstanding across ' + kpi.customers_outstanding + ' customers.');
