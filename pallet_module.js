  ...
  app.get('/api/pallets/transport-tally/reconcile/export', requireAuth, async function (req, res) {
    try {
      ...
      rs.columns = [{ width: 14 }, { width: 18 }, { width: 12 }, { width: 12 }, { width: 16 }, { width: 10 },
        { width: 18 }, { width: 14 }, { width: 10 }, { width: 16 }, { width: 12 }, { width: 10 }, { width: 30 }];
      styleTitleRow(rs, 'TRANSPORT vs WAREHOUSE — DAILY RECONCILIATION', 'M');
      styleBigBanner(rs, periodBannerText(req), 'M');
      rs.addRow(['Downloaded', new Date().toLocaleString('en-AE')]);
      rs.addRow([]);
      var explainRow = rs.addRow(['Transport Dispatched compares to WH Outbound + WH Outbound Loop combined (loop pallets leave on the same trucks). Transport Collected from DIP compares to WH Return Loop (loop pallets return separately via DIP, not through Returned to WH).']);
