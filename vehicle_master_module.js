...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, vehicleMasterMap) {
  ...
  async function refreshVehicleMasterMap() {
    try {
      const r = await pool.query(`SELECT vehicle_no, vehicle_no_normalized, veh_type, vehicle_type_raw, partition_flag FROM vehicle_master WHERE active=true`);
      Object.keys(vehicleMasterMap).forEach(k => delete vehicleMasterMap[k]);
      r.rows.forEach(row => {
        const entry = { veh_type: row.veh_type, vehicle_type_raw: row.vehicle_type_raw, partition_flag: row.partition_flag };
