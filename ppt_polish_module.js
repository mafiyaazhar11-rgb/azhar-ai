...
{"slides":[{"title":"...","bullets":["...","..."]}]}`;
      ...
      try { improved = JSON.parse(raw); } catch (e) {
        return res.status(500).json({ error: 'Claude returned a response that could not be parsed. Try again.' });
      }
      if (!improved.slides || improved.slides.length !== slides.length) {
        return res.status(500).json({ error: 'Claude returned an unexpected number of slides. Try again.' });
      }
