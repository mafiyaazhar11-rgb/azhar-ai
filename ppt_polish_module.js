  ...
  async function extractDeck(buffer) {
    ...
    for (const slidePath of slidePaths) {
      try {
        ...
        // One unusual slide (SmartArt, embedded video, odd XML) shouldn't
        // kill the whole deck — keep its place with whatever we could get.
...
{"slides":[{"title":"...","bullets":["...","..."]}]}`;
      ...
      // Be tolerant of any stray text Claude adds outside the JSON object —
      // pull out just the {...} block rather than requiring a perfectly clean response.
      ...
      // Slide count should match, but don't hard-fail a large deck over a
      // minor drift — pad with the original text as a fallback, or trim
      // extras, so the manager still gets a result instead of nothing.
      while (improved.slides.length < slides.length) {
        const idx = improved.slides.length;
        improved.slides.push({ title: (slides[idx].texts[0] || `Slide ${idx + 1}`), bullets: slides[idx].texts.slice(1, 6) });
      }
