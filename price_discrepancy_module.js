...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.get('/api/price-discrepancy/summary/export', requireAuth, requireRole('superadmin', 'subadmin'), async function (req, res) {
    try {
      ...
      var brandSkuSets = {}, brandCustSets = {}, brandRowCount = {}, brandVariance = {};
      detailRows.forEach(function (d) {
        if (!brandSkuSets[d.brand]) { brandSkuSets[d.brand] = new Set(); brandCustSets[d.brand] = new Set(); brandRowCount[d.brand] = 0; brandVariance[d.brand] = 0; }
        brandSkuSets[d.brand].add(d.sku);
        brandCustSets[d.brand].add(d.customer);
        brandRowCount[d.brand]++;
        brandVariance[d.brand] += d.variance;
      });
      ...
      ex.addRow([]);
      headerRow(ex, ['Brand', 'SKUs Affected', '% of Total', 'Customers Affected', 'Open Issues', 'Total Variance (AED)']);
      execSummary.forEach(function (g) {
        var row = ex.addRow([g.brand, g.skuCount, g.pctOfTotal + '%', g.customerCount, g.openIssues, g.totalVariance]);
        row.getCell(2).font = { bold: true };
        row.getCell(3).font = { color: { argb: g.pctOfTotal >= 15 ? 'FFB03A2E' : 'FF3B3B3B' }, bold: g.pctOfTotal >= 15 };
        row.getCell(6).fill = { type: 'pattern', pattern: 'solid', fgColor: { argb: g.totalVariance < 0 ? RED_BG : GREEN_BG } };
        row.getCell(6).font = { color: { argb: g.totalVariance < 0 ? RED_TEXT : GREEN_TEXT }, bold: true };
      });
      ...
      var totalRow = ex.addRow(['TOTAL', totalDistinctSkus, '100%',
        ...
        detailRows.length, +detailRows.reduce(function (s, d) { return s + d.variance; }, 0).toFixed(2)]);
