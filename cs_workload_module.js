...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, anthropic) {
    ...
    try {
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        if (entryType === 'order') {
          ...
          if (mine.rows.length) {
            await pool.query(
              `UPDATE cs_workload_entries SET entry_date=$1, customer_name=$2, resolution_type=$3, booking_status=$4, fob_status=$5, file_name=$6, uploaded_at=NOW() WHERE id=$7`,
              [entryDate, customer, resolution, bookingStatus, fobStatus, req.file.originalname, mine.rows[0].id]
            );
            updated++;
            continue;
          }
      ...
      res.json({ success: true, inserted: inserted, updated: updated });
    } catch (e) {
  ...
  app.get('/api/cs-workload/my-summary', requireAuth, async function (req, res) {
    try {
      ...
      res.json({ success: true, repFullName: req.user.full_name || username, daily: daily.rows, breakdown: breakdown.rows, totals: totals.rows, autoManual: autoManual.rows, fobBreakdown: fobBreakdown.rows, monthly: monthly.rows });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
