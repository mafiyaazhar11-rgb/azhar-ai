...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog) {
  ...
  app.post('/api/cs-workload/baseline/upload', requireAuth, requireRole('superadmin', 'subadmin'), upload.single('file'), async function (req, res) {
    try {
      ...
      var keys = Object.keys(raw[0]);
      var C = {
        person: fc(keys, ['person', 'rep', 'name']),
        account: fc(keys, ['account']),
        total: fc(keys, ['total']),
        auto: fc(keys, ['auto']),
        manual: fc(keys, ['manual'])
      };
      if (!C.person) return res.status(400).json({ error: 'Could not find a Person/Rep column in that file.' });
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        await pool.query(
          ...
          [person, C.account ? String(row[C.account] || '').trim() : null,
           C.total ? (+row[C.total] || 0) : 0, C.auto ? (+row[C.auto] || 0) : 0, C.manual ? (+row[C.manual] || 0) : 0,
           periodLabel, req.user.username]
        );
        inserted++;
      }
      auditLog(req.user.uid, req.user.username, 'CS_WORKLOAD_BASELINE_UPLOAD', 'Replaced baseline with ' + inserted + ' rows', req.headers['x-forwarded-for'] || '');
      res.json({ success: true, inserted: inserted });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
