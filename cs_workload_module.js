...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, anthropic) {
  ...
  app.post('/api/cs-workload/upload', requireAuth, upload.single('file'), async function (req, res) {
    try {
      ...
      // ── Reject the WHOLE file up front if the FOB Status column has any value
      // outside the 3 standard categories — before touching the database, so a
      // bad file never gets partially applied and doesn't burn the rep's
      // once-per-12-hours upload window either. ──
      if (entryType === 'order' && C.status) {
        ...
        for (var vi = 0; vi < raw.length; vi++) {
          ...
          if (!vRaw) continue; // blank is allowed
      ...
      for (var i = 0; i < raw.length; i++) {
        ...
        var fobStatus = null;
        if (C.status) {
          // FOB is authoritative and already validated above — every non-blank value
          // here is guaranteed to match one of the 3 standard categories.
          var rawStatusRaw = String(row[C.status] || '').trim();
          if (rawStatusRaw) {
            fobStatus = classifyFobStatus(rawStatusRaw);
            // "AUTO BOOKED W/ MANUAL INTERVENTION" counts as Manual — manual work was
            // done even though the order was technically auto-booked initially.
