...
module.exports = function (app, pool, requireAuth, requireRole, upload, auditLog, bcrypt) {
  ...
  // ── Guest creates a new trip — same full access as admin except guest
  // account creation, which stays admin-only. The creating guest is
  // automatically granted access to their own new trip. ──
  app.post('/api/trip-guest/trips', requireGuestAuth, async function (req, res) {
    try {
      var name = toStr(req.body.name);
      var members = Array.isArray(req.body.members) ? req.body.members.map(toStr).filter(Boolean) : [];
      if (!name) return res.status(400).json({ error: 'Trip name is required.' });
      ...
      var r = await pool.query(
        `INSERT INTO trip_trips (name, members, created_by_username, created_by_name) VALUES ($1,$2,$3,$4) RETURNING id`,
        [name, JSON.stringify(members), 'guest:' + req.guest.username, req.guest.display_name]
      );
      await pool.query('INSERT INTO trip_guest_access (guest_id, trip_id) VALUES ($1,$2) ON CONFLICT (guest_id, trip_id) DO NOTHING', [req.guest.guest_id, r.rows[0].id]);
      res.json({ success: true, id: r.rows[0].id });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });

  // ── Guest edits trip name / members (status has its own endpoint below) ──
  app.put('/api/trip-guest/trips/:tripId', requireGuestAuth, requireGuestTripAccess, async function (req, res) {
    try {
      var name = toStr(req.body.name) || req.trip.name;
      var members = Array.isArray(req.body.members) ? req.body.members.map(toStr).filter(Boolean) : req.trip.members;
      if (members.length < 2) return res.status(400).json({ error: 'Keep at least 2 members.' });
      await pool.query('UPDATE trip_trips SET name=$1, members=$2, updated_at=NOW() WHERE id=$3', [name, JSON.stringify(members), req.trip.id]);
      res.json({ success: true });
    } catch (e) { res.status(500).json({ error: e.message }); }
  });
